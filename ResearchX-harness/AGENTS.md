...
Pi subagent behavior does **not** live here. The source of truth for bundled Pi subagents is `.researchx/agents/*.md`, which the runtime syncs into the Pi agent directory. If you need to change how `researcher`, `reviewer`, `writer`, or `verifier` behave, edit the corresponding file in `.researchx/agents/` instead of duplicating those prompts here.

## Pi subagents

ResearchX ships four bundled research subagents:
...
They are defined in `.researchx/agents/` and invoked via the Pi `subagent` tool.
...
ResearchX must stay simple yet potent. It is a general-purpose assistant that helps in every field — coding, writing, analysis, math, everyday questions — with research workflows as its flagship strength, not its boundary.

Every new feature must fight for its life before implementation. Keep or add a feature only when it directly improves at least one core job:

- helping with general tasks: code, writing, analysis, planning, questions
...
Reject unrelated product lanes by default. Funding, proposal, sales, and admin workflows do not belong in ResearchX unless the user explicitly asks for them as general assistance.

Before adding a command, prompt, tool, extension, dashboard, document page, or release-note item, state the job it serves and the smallest existing surface that can absorb it. If the value is not concrete and testable, do not add it.

## Pi runtime changes

- ResearchX wraps Pi. Before changing telemetry, tools, extensions, runtime package setup, model/prompt handoff, or child-process env, read the installed Pi package version, `node_modules/@earendil-works/pi-coding-agent/docs/`, and the matching runtime source.
- Before writing a local Pi extension or tool shim, search Pi package docs plus npm/GitHub for an existing Pi extension or plugin; use the existing package, patch it, or record why it fails before adding a ResearchX-owned implementation.
...
- The workbench is a Pi-backed research control plane. For chat, session, project, or reference-product parity work, read Pi's docs/runtime and the reference app's live behavior before coding, then wire the UI to real Pi/ResearchX sessions, messages, streaming or resume state, files, artifacts, execution/provenance, compute, skills/MCP, memory, and verification state before calling the surface functionally complete.
