...
const ignoredPathParts = new Set(["node_modules", "dist", ".git", ".researchx"]);
...
if (debts.length > 0) {
	console.log("[researchx:architecture] known architecture debt:");
	for (const debt of debts) console.log(`  - ${debt}`);
}

if (warnings.length > 0) {
	console.log("[researchx:architecture] files to split before they become core debt:");
	for (const warning of warnings) console.log(`  - ${warning}`);
}

if (failures.length > 0) {
	console.error("[researchx:architecture] failed:");
	for (const failure of failures) console.error(`  - ${failure}`);
	process.exit(1);
}

console.log(`[researchx:architecture] checked ${files.length} source files`);
