...
const runtimePackageLockPath = resolve(researchxDir, "runtime-package-lock.json");
const explicitWorkspaceDir =
	process.env.RESEARCHX_RUNTIME_WORKSPACE_TARGET?.trim();
const workspaceDir = explicitWorkspaceDir
	? resolve(explicitWorkspaceDir)
	: resolve(appRoot, ".researchx", "npm");
...
const workspaceArchivePath = resolve(researchxDir, "runtime-workspace.tgz");
const workspaceArchiveDigestPath = resolve(researchxDir, "runtime-workspace.sha256");
...
function readPackageSpecs() {
	...
	for (const packageName of PINNED_RUNTIME_PACKAGES) {
		...
		if (packageName === RESEARCHX_PI_TELEMETRY_PACKAGE) {
...
function prepareWorkspace(packageSpecs, refreshRuntimeLock) {
	...
	if (!existsSync(runtimePackageLockPath)) {
		throw new Error(
			"Missing .researchx/runtime-package-lock.json. Run npm run runtime:lock to create it.",
		);
	}
...
function patchBundledPiTui() {
	...
	changed = patchScopedPiWorkspaceFile("pi-tui", "dist/components/select-list.js", patchPiSelectListPreviewSource) || changed;
	changed = patchNestedPiWorkspaceFile(
		"pi-coding-agent",
		"pi-tui",
	...
	changed = patchScopedPiWorkspaceFile("pi-tui", "dist/autocomplete.js", patchPiAutocompletePreviewSource) || changed;
	changed = patchNestedPiWorkspaceFile(
		"pi-coding-agent",
		"pi-tui",
...
function patchBundledPiCommandPreview() {
	return patchScopedPiWorkspaceFile(
		"pi-coding-agent",
		"dist/modes/interactive/interactive-mode.js",
		patchPiCommandPreviewSource,
...
function removeGeneratedHiddenRuntimeLock() {
	...
	// npm's hidden lock describes the bytes produced by npm ci. ResearchX then
...
function patchBundledRuntime(
	...
	// Rebrand bundled trees (e.g. upstream pi-web-access baselines) before any
	// patcher reads them, so reviewed pins match on fresh and upgraded installs.
	sweepLegacyBrandMarkers(workspaceNodeModulesDir);
	...
	changed = patchBundledPiCommandPreview() || changed;
...
if (patchExistingWorkspace) {
	if (!explicitWorkspaceDir || !existsSync(workspaceNodeModulesDir)) {
		throw new Error(
			"--patch-existing requires RESEARCHX_RUNTIME_WORKSPACE_TARGET with an installed node_modules tree",
		);
	}
	if (!workspacePackagesMatch(workspaceNodeModulesDir, packageSpecs)) {
		throw new Error(
			"Existing runtime workspace does not match ResearchX's exact package contract",
		);
	}
...
if (!refreshRuntimeLock && !rebuildWorkspace && workspaceIsCurrent(packageSpecs)) {
	...
	console.log("[researchx] vendored runtime workspace already up to date");
	linkLegacyPiRuntimeAliases();
	if (patchBundledRuntime(piCliArgsCandidates)) {
		writeManifest(packageSpecs);
		console.log("[researchx] patched bundled Pi runtime");
	}
	if (archiveIsCurrent(packageSpecs)) {
		process.exit(0);
	}
	console.log("[researchx] refreshing runtime workspace archive...");
	await createWorkspaceArchive(packageSpecs);
	console.log("[researchx] runtime workspace archive ready");
	process.exit(0);
}

console.log("[researchx] preparing vendored runtime workspace...");
...
if (refreshRuntimeLock) {
	...
	console.log("[researchx] refreshed committed runtime lock");
}
writeManifest(packageSpecs);
await createWorkspaceArchive(packageSpecs);
console.log("[researchx] vendored runtime workspace ready");
