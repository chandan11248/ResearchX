...
export function verifyModelAwareSearchRouting(packageRoot) {
	...
	const root = mkdtempSync(join(tmpdir(), "researchx-installed-search-routing-"));
...
export function verifyGitHubCloneSafety(packageRoot) {
	...
	const root = mkdtempSync(join(tmpdir(), "researchx-installed-github-clone-"));
	const configPath = join(root, "web-search.json");
	...
	try {
		...
		writeFileSync(
			...
			`${readFileSync(instrumentedModulePath, "utf8")}\nexport const __researchxVerifier = { cloneCache, cloneDestination };\n`,
			"utf8",
		);
		...
		const cleanup = JSON.parse(runGitHubProbe({
			...
			input: `
				...
				const destination = github.__researchxVerifier.cloneDestination(
				...
				github.__researchxVerifier.cloneCache.set("owner/repo", {
					destination,
