...
export const PI_EXTENSION_HANDLER_TIMEOUT_MARKER =
	"ResearchX Pi 0.84.2 extension handler deadline #8662";
...
const PATCHED_EVENT_AWAIT =
	"await runResearchXExtensionHandler(handler, event, ctx)";
const PATCHED_CURRENT_EVENT_AWAIT =
	"await runResearchXExtensionHandler(handler, currentEvent, ctx)";
...
const RESEARCHX_PI_EXTENSION_HANDLER_TIMEOUT_MS = 30000;
...
const RESEARCHX_PI_INTERACTIVE_UI_METHODS = new Set([
    "select",
    "confirm",
    "input",
    "custom",
...
class ResearchXExtensionHandlerTimeoutError extends Error {
    constructor() {
        super(\`Extension handler timed out after \${RESEARCHX_PI_EXTENSION_HANDLER_TIMEOUT_MS}ms\`);
        this.name = "ResearchXExtensionHandlerTimeoutError";
...
function createResearchXHandlerContext(context, lifecycle) {
    let wrappedUi;
                    ...
                    wrappedUi = new Proxy(ui, {
                        get(uiTarget, uiProperty) {
                            ...
                            if (typeof value !== "function")
                                return value;
                            if (RESEARCHX_PI_INTERACTIVE_UI_METHODS.has(uiProperty)) {
                                return (...args) => {
                                    lifecycle.assertActive();
...
async function runResearchXExtensionHandler(handler, event, context) {
    let deadline;
    let deadlineStartedAt;
    let remainingDeadlineMs = RESEARCHX_PI_EXTENSION_HANDLER_TIMEOUT_MS;
    ...
    const guardedContext = createResearchXHandlerContext(context, lifecycle);
    ...
    try {
        ...
        if (outcome.status === "timed-out")
            throw new ResearchXExtensionHandlerTimeoutError();
...
const ORIGINAL_BOUNDED_TOOL_CALL_METHOD = `    async emitToolCall(event) {
        ...
        for (const ext of this.extensions) {
            ...
            for (const handler of handlers) {
                const handlerResult = await runResearchXExtensionHandler(handler, event, ctx);
                if (handlerResult) {
        ...
        for (const ext of this.extensions) {
            ...
            for (const handler of handlers) {
                try {
                    ...
                    if (!(err instanceof ResearchXExtensionHandlerTimeoutError))
                        throw err;
...
export function assertPiExtensionHandlerTimeoutPatchSource(
	...
	for (const fragment of [
		...
		"class ResearchXExtensionHandlerTimeoutError extends Error {",
		"function createResearchXHandlerContext(context, lifecycle) {",
		"function runResearchXExtensionHandler(handler, event, context) {",
		"let remainingDeadlineMs = RESEARCHX_PI_EXTENSION_HANDLER_TIMEOUT_MS;",
		"remainingDeadlineMs - (Date.now() - deadlineStartedAt)",
		"const deadlineController = new AbortController();",
		"AbortSignal.any([originalSignal, deadlineController.signal])",
