/**
 * ResearchX patches for Pi AgentCore 0.84.2.
 *
 ...
 * with RESEARCHX_PI_STREAM_EVENT_IDLE_TIMEOUT_MS. Zero disables the watchdog.
 *
...
const ABORT_QUEUE_GUARD_MARKER =
	`ResearchX Pi ${PI_AGENT_CORE_PATCH_REQUIRED_VERSION} forward patch: preserve queued input on abort #8658`;

export const PI_AGENT_CORE_PATCH_MARKERS = Object.freeze({
	toolAliases: "function normalizeResearchXToolAlias(",
	streamWatchdog: "ResearchX Pi 0.84.2 forward patch: provider stream watchdog #8331",
});

const HELPER = `
function normalizeResearchXSearchToolArguments(args) {
...
function normalizeResearchXFetchToolArguments(args) {
    if (!args || typeof args !== "object" || Array.isArray(args)) {
        return args;
    }
    const normalized = { ...args };
...
function normalizeResearchXToolAlias(toolCall, tools) {
    ...
    const args = targetName === "fetch_content"
        ? normalizeResearchXFetchToolArguments(toolCall.arguments)
        : normalizeResearchXSearchToolArguments(toolCall.arguments);
    return {
        ...toolCall,
        name: targetName,
        arguments: args,
...
const RESEARCHX_MAX_STREAM_EVENT_IDLE_TIMEOUT_MS = 2147483647;
const RESEARCHX_EMPTY_USAGE = {
    input: 0,
    output: 0,
    cacheRead: 0,
    cacheWrite: 0,
...
function parseResearchXStreamIdleTimeoutMs(value, surface) {
    ...
    if (!Number.isInteger(parsed) ||
        parsed < 0 ||
        parsed > RESEARCHX_MAX_STREAM_EVENT_IDLE_TIMEOUT_MS) {
        throw new Error(\`Invalid \${surface}: expected an integer from 0 to \${RESEARCHX_MAX_STREAM_EVENT_IDLE_TIMEOUT_MS}\`);
    }
    return parsed;
}
function resolveResearchXStreamIdleTimeoutMs(config) {
    ...
    const rawOverride = typeof process !== "undefined"
        ? process.env?.RESEARCHX_PI_STREAM_EVENT_IDLE_TIMEOUT_MS
        : undefined;
    return parseResearchXStreamIdleTimeoutMs(
        rawOverride,
        "RESEARCHX_PI_STREAM_EVENT_IDLE_TIMEOUT_MS",
    ) ?? 0;
}
function sanitizeResearchXPartialMessage(message) {
    return {
        ...message,
...
${PATCHED_STREAM_NEXT_SEQUENCE}
        if (idleTimeoutMs > 0) {
            pending.push(new Promise((resolve) => {
                idleTimer = setTimeout(() => resolve("researchx-idle"), idleTimeoutMs);
                idleTimer.unref?.();
            }));
        }
        if (signal && !signal.aborted) {
            pending.push(new Promise((resolve) => {
                abortListener = () => resolve("researchx-aborted");
        ...
        if (settled === "researchx-idle" || settled === "researchx-aborted") {
            const aborted = settled === "researchx-aborted" || signal?.aborted === true;
            const errorMessage = aborted
                ? undefined
                : \`Provider stream event timeout after \${idleTimeoutMs}ms without a Pi event\`;
            const finalMessage = partialMessage
                ? {
                    ...sanitizeResearchXPartialMessage(partialMessage),
...
function patchToolAliases(source) {
	...
	patched = patched.replace(
		...
		"async function prepareToolCall(currentContext, assistantMessage, toolCall, config, signal) {\n    const effectiveToolCall = normalizeResearchXToolAlias(toolCall, currentContext.tools);\n    const tool = currentContext.tools?.find((t) => t.name === effectiveToolCall.name);",
	);
	patched = patched.replace(
...
function patchStreamWatchdog(source) {
	...
	patched = patched.replace(
		...
		`    const idleTimeoutMs = resolveResearchXStreamIdleTimeoutMs(config);
