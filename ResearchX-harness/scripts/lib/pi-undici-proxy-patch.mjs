...
const PRIOR_RESEARCHX_UNDICI_VERSION = "8.9.0";
const RESEARCHX_UNDICI_LOCK_ENTRY = {
	version: RESEARCHX_UNDICI_VERSION,
	resolved: `https://registry.npmjs.org/undici/-/undici-${RESEARCHX_UNDICI_VERSION}.tgz`,
...
function assertSupportedUndiciVersion(version, surface) {
	if (
		![
			...
			PRIOR_RESEARCHX_UNDICI_VERSION,
...
function lockEntryMatches(entry) {
	return entry?.version === RESEARCHX_UNDICI_LOCK_ENTRY.version &&
		entry?.resolved === RESEARCHX_UNDICI_LOCK_ENTRY.resolved &&
		entry?.integrity === RESEARCHX_UNDICI_LOCK_ENTRY.integrity;
}

function resolveResearchXUndiciPath(nodeModulesPath, fallbackPackagePath) {
	...
	return candidates.find((candidate) =>
		candidate && readPackageVersion(candidate) === RESEARCHX_UNDICI_VERSION
	);
}
...
export function patchPiCodingAgentUndiciPackageJsonSource(source) {
	...
	if (currentVersion === RESEARCHX_UNDICI_VERSION) {
		return source;
	}
	manifest.dependencies.undici = RESEARCHX_UNDICI_VERSION;
...
export function patchPiCodingAgentUndiciShrinkwrapSource(source) {
	...
	if (
		dependencyVersion === RESEARCHX_UNDICI_VERSION &&
		lockEntryMatches(entry)
	) {
		return source;
	}
	shrinkwrap.packages[""].dependencies.undici = RESEARCHX_UNDICI_VERSION;
	shrinkwrap.packages["node_modules/undici"] = RESEARCHX_UNDICI_LOCK_ENTRY;
...
export function assertPiCodingAgentUndiciShrinkwrapSource(source, surface) {
	...
	if (
		...
		throw new Error(
			`Incomplete Pi Undici metadata ${surface}: expected exact ${RESEARCHX_UNDICI_VERSION} dependency, resolved URL, and integrity`,
		);
	}
}
...
export function patchPiUndiciPackageLockSource(source, requiredPiVersion) {
	...
	for (const [packagePath, entry] of Object.entries(lockfile.packages ?? {})) {
		if (/node_modules\/(?:@[^/]+\/)?pi-coding-agent$/.test(packagePath)) {
			...
			if (currentVersion !== RESEARCHX_UNDICI_VERSION) {
				entry.dependencies.undici = RESEARCHX_UNDICI_VERSION;
				changed = true;
			}
			continue;
		}
 ...
 * ResearchX and Pi inherit the current proxy and retrieval reliability fixes.
...
export function patchPiUndiciProxyTree(nodeModulesPath, fallbackPackagePath, requiredPiVersion) {
	...
	for (const piRoot of piRoots) {
		...
		if (nestedVersion !== RESEARCHX_UNDICI_VERSION) {
			if (nestedVersion !== undefined) {
				assertSupportedUndiciVersion(nestedVersion, "installed version");
			}
			const safePackagePath = resolveResearchXUndiciPath(nodeModulesPath, fallbackPackagePath);
			if (!safePackagePath) {
				throw new Error(`Undici ${RESEARCHX_UNDICI_VERSION} package tree is unavailable`);
			}
			const temporaryPath = `${nestedPackagePath}.researchx-proxy-${process.pid}`;
