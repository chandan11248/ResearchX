...
const RESOLVE_PI_AGENT_DIR_HELPER = [
	...
	'	const configured = process.env.RESEARCHX_CODING_AGENT_DIR?.trim() || process.env.PI_CODING_AGENT_DIR?.trim();',
...
const PI_SPAWN_RESEARCHX_ARGV_BLOCK = [
	"\tconst argv1 = deps.argv1 ?? process.argv[1];",
...
const PI_SPAWN_RESEARCHX_ARGV_BLOCK_WITH_WRAPPER_MAIN = [
	"\tconst argv1 = deps.argv1 ?? process.argv[1];",
...
function patchCurrentPiSpawnResolver(source) {
	if (
		...
		source.includes("RESEARCHX_PI_CLI_PATH")
	) {
		return source;
	}
	return source.replace(
		...
		[
			...
			"\tconst researchxPiCliPath = env.RESEARCHX_PI_CLI_PATH?.trim();",
			"\tif (researchxPiCliPath) {",
			"\t\tconst cliPath = normalizePath(researchxPiCliPath);",
			"\t\tif (isRunnableNodeScript(cliPath, existsSync)) return cliPath;",
			"\t}",
...
export function patchPiSubagentsSource(relativePath, source) {
	...
	switch (target) {
		...
		case "pi-spawn.ts":
			patched = applyReplacementGroup(patched, [
				[PI_SPAWN_ORIGINAL_ARGV_BLOCK, PI_SPAWN_RESEARCHX_ARGV_BLOCK_WITH_WRAPPER_MAIN],
				[
					"\texecPath?: string;\n\targv1?: string;",
					"\texecPath?: string;\n\targv1?: string;\n\targv2?: string;",
				],
			...
			patched = applyReplacementGroup(patched, [
				[PI_SPAWN_RESEARCHX_ARGV_BLOCK, PI_SPAWN_RESEARCHX_ARGV_BLOCK_WITH_WRAPPER_MAIN],
				[
					"\texecPath?: string;\n\targv1?: string;",
					"\texecPath?: string;\n\targv1?: string;\n\targv2?: string;",
				],
