 ...
 *
 * Removal condition: delete this patch after ResearchX adopts a released Pi
...
export const PI_AI_FORWARD_FIX_MARKERS = Object.freeze({
	googleGenerativeAi: "ResearchX Pi 0.84.2 forward patch: Google thinking level maps",
	googleShared: "ResearchX Pi 0.84.2 forward patch: resolve Google thinking level maps",
	googleVertex: "ResearchX Pi 0.84.2 forward patch: Vertex thinking level maps",
	bedrock: PI_BEDROCK_RESPONSE_HEADERS_MARKER,
	bedrockToolResultImages: PI_BEDROCK_TOOL_RESULT_IMAGES_MARKER,
	toolChoice: "ResearchX Pi 0.84.2 forward patch: provider-neutral tool choice",
	openAiCompletions: "ResearchX Pi 0.84.2 forward patch: Gemini signatures and bounded tool IDs",
	openAiResponsesNoTools: PI_OPENAI_RESPONSES_NO_TOOLS_MARKER,
	providerRetry: "ResearchX Pi 0.84.2 forward patch: transient in-flight budget retry #8507",
});
	...
	switch (relativePath) {
		...
		case "dist/api/openai-completions.js":
			assertSourceFragments(source, relativePath, [
				...
				"isResearchXSerializedReasoningDetail",
				"if (!block.thoughtSignature ||",
				"isResearchXSerializedReasoningDetail(block.thoughtSignature))",
		...
		case "dist/utils/provider-retry.js":
			assertSourceFragments(source, relativePath, [
				...
				'getResearchXStructuredErrorCode(error) === "in_flight_budget_exhausted"',
				"retryOn",
...
function patchOpenAiCompletions(source) {
		...
		let upgraded = source
			...
			.replace(
				`                            if (compat.supportsGoogleThoughtSignatures &&
                                ...
                                signature.length > 0) {
                                if (isResearchXSerializedReasoningDetail(block.thoughtSignature)) {
                                    appendResearchXEncryptedReasoningDetail(block.thoughtSignature);
                                }
			...
			.replace(
				...
				`                                    else {
                                        if (isResearchXSerializedReasoningDetail(matchingToolCall.thoughtSignature)) {
                                            appendResearchXEncryptedReasoningDetail(matchingToolCall.thoughtSignature);
                                        }
                                        matchingToolCall.thoughtSignature = serializedDetail;
                                    }`,
				);
	...
	patched = replaceRequired(
		...
		`function isResearchXSerializedReasoningDetail(value) {
...
export const stream = (model, context, options) => {`,
	...
	patched = replaceRequired(
		...
		`            const pendingReasoningDetailsByToolCallId = new Map();
            const appendResearchXEncryptedReasoningDetail = (serializedDetail) => {
                try {
	...
	patched = replaceRequired(
		...
		`                        function: {
                        ...
                        ...(compat.supportsGoogleThoughtSignatures &&
                            tc.thoughtSignature &&
                            !isResearchXSerializedReasoningDetail(tc.thoughtSignature)
                            ? { extra_content: { google: { thought_signature: tc.thoughtSignature } } }
                            : {}),
                    };`,
	...
	patched = replaceRequired(
		...
		`                                if (matchingToolCall) {
                                    if (compat.supportsGoogleThoughtSignatures &&
                                        ...
                                        !isResearchXSerializedReasoningDetail(matchingToolCall.thoughtSignature)) {
                                        appendResearchXEncryptedReasoningDetail(serializedDetail);
                                    }
                                    else {
                                        if (isResearchXSerializedReasoningDetail(matchingToolCall.thoughtSignature)) {
                                            appendResearchXEncryptedReasoningDetail(matchingToolCall.thoughtSignature);
...
function getResearchXStructuredErrorCode(error) {
...
export function isTransientInFlightBudgetError(error) {
    if (!(error instanceof Error) ||
        ...
        !error.headers.get("retry-after"))
        return false;
    return getResearchXStructuredErrorCode(error) === "in_flight_budget_exhausted";
}
`;
	let patched = replaceRequired(
		source,
