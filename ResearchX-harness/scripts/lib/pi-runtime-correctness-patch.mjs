...
export const PI_CODING_AGENT_FORWARD_FIX_MARKERS = Object.freeze({
	largeToolRender: "ResearchX Pi 0.84.2 forward patch: large tool render #8036",
	toolReleaseRedirect: "ResearchX Pi 0.84.2 forward patch: GitHub release redirect #8594",
	exifAfterXmp: "ResearchX Pi 0.84.2 forward patch: EXIF after XMP #8616",
});
...
export const PI_RUNTIME_CORRECTNESS_PATCH_MARKERS = Object.freeze({
	agentSession: "ResearchX Pi 0.84.2 correctness patch: issue #7053",
	sessionManager: "ResearchX Pi 0.84.2 correctness patch: restore eager tool results",
	sessionTailRepair: PI_SESSION_TAIL_PATCH_MARKER,
	transformMessages: "ResearchX Pi 0.84.2 correctness patch: order eager tool results",
	githubCopilotDeviceCode: "ResearchX Pi 0.84.2 correctness patch: export abortableSleep for upstream #8121",
	githubCopilotOAuth: "ResearchX Pi 0.84.2 correctness patch: upstream #8121",
	imageQueue: "ResearchX Pi 0.84.2 correctness patch: image-only queue delivery #8581",
	turnEndMessages: "ResearchX Pi 0.84.2 correctness patch: defer custom messages #8166",
...
export const PI_RUNTIME_CORRECTNESS_REQUIRED_FRAGMENTS = Object.freeze({
	agentSession: Object.freeze([
		...
		'const researchxToolResultIdBeforeExtensions = event.type === "message_end" && event.message.role === "toolResult"',
		"const entryId = this.sessionManager.appendMessage(toolResult);",
		"this._researchxEagerlyPersistedToolResults.set(event.toolCallId, {",
		"await this._emitExtensionEvent(event);",
		"event.message.toolCallId = researchxToolResultIdBeforeExtensions;",
		"const eagerToolCallId = researchxToolResultIdBeforeExtensions ?? event.message.toolCallId;",
	...
	sessionManager: Object.freeze([
		...
		"function restoreResearchXToolResultsInSourceOrder(messages) {",
...
${PATCHED_IMAGE_QUEUE_DELIVERY}
        ...
        const researchxToolResultIdBeforeExtensions =`;
...
function createResearchXToolResultMessage(event) {
    return {
        role: "toolResult",
...
function hasSameResearchXToolResultPayload(left, right) {
...
function serializeResearchXToolResultPayload(message) {
...
const AGENT_SESSION_LEGACY_EAGER_PERSISTENCE = `        // A finalized result must be durable before a parallel sibling settles.
        ...
        if (event.type === "tool_execution_end") {
            const toolResult = createResearchXToolResultMessage(event);
            const entryId = this.sessionManager.appendMessage(toolResult);
            this._researchxEagerlyPersistedToolResults.set(event.toolCallId, {
                entryId,
                message: toolResult,
                serializedPayload: this.sessionManager.isPersisted()
                    ? serializeResearchXToolResultPayload(toolResult)
                    : undefined,
            });
        }
`;
...
const AGENT_SESSION_EXTENSION_BOUNDARY = `        const researchxToolResultIdBeforeExtensions = event.type === "message_end" && event.message.role === "toolResult"
...
${AGENT_SESSION_EAGER_PERSISTENCE}        // Emit to extensions first
        ...
        if (researchxToolResultIdBeforeExtensions !== undefined &&
...
const PATCHED_MESSAGE_PERSISTENCE = `            else if (event.message.role === "toolResult") {
                const eagerToolCallId = researchxToolResultIdBeforeExtensions ?? event.message.toolCallId;
                const eagerlyPersisted = this._researchxEagerlyPersistedToolResults.get(eagerToolCallId);
                this._researchxEagerlyPersistedToolResults.delete(eagerToolCallId);
                const payloadUnchanged = eagerlyPersisted?.serializedPayload !== undefined
                    ? eagerlyPersisted.serializedPayload === serializeResearchXToolResultPayload(event.message)
                    : eagerlyPersisted
                        ? hasSameResearchXToolResultPayload(eagerlyPersisted.message, event.message)
                        : false;
...
${AGENT_SESSION_LEGACY_EAGER_PERSISTENCE}`,
			...
			upgraded = replaceRequired(
				upgraded,
				`                const eagerlyPersisted = this._researchxEagerlyPersistedToolResults.get(event.message.toolCallId);
                this._researchxEagerlyPersistedToolResults.delete(event.message.toolCallId);`,
				`                const eagerToolCallId = researchxToolResultIdBeforeExtensions ?? event.message.toolCallId;
                const eagerlyPersisted = this._researchxEagerlyPersistedToolResults.get(eagerToolCallId);
                this._researchxEagerlyPersistedToolResults.delete(eagerToolCallId);`,
	...
	patched = replaceRequired(
		patched,
		"    _eventListeners = [];\n",
		"    _eventListeners = [];\n    _researchxEagerlyPersistedToolResults = new Map();\n",
		"agent-session state",
	);
	patched = replaceRequired(
		patched,
...
const SESSION_MANAGER_REPLACE_MESSAGE = `    /**
     ...
     * ResearchX uses this only when a message_end extension rewrites a finalized tool result.
     */
    replaceMessage(entryId, message) {
