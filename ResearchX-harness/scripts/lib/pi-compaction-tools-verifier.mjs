...
export async function verifyPiCompactionToolsBehavior(packageRoot) {
	...
	const compaction = await import(
		`${pathToFileURL(resolve(codingAgentRoot, "dist", "core", "compaction", "compaction.js")).href}?researchx-tools=${Date.now()}`
	);
	const branch = await import(
		`${pathToFileURL(resolve(codingAgentRoot, "dist", "core", "compaction", "branch-summarization.js")).href}?researchx-tools=${Date.now()}`
	);
	const model = {
		id: "summary-test",
		name: "Summary Test",
