...
const PATCHED_CONFIG_EXPR =
	'process.env.RESEARCHX_WEB_SEARCH_CONFIG ?? process.env.PI_WEB_SEARCH_CONFIG ?? join(homedir(), ".pi", "web-search.json")';
...
const PATCHED_PDF_OUTPUT_DIR = [
	...
	'  process.env.RESEARCHX_FETCH_CACHE_DIR?.trim() || join(process.cwd(), ".researchx", "cache", "fetch-content");',
].join("\n");
...
const PATCHED_CONFIG_PATH_HELPER = [
	"export function getWebSearchConfigPath(): string {",
	"\tconst configuredPath = process.env.RESEARCHX_WEB_SEARCH_CONFIG?.trim() || process.env.PI_WEB_SEARCH_CONFIG?.trim();",
