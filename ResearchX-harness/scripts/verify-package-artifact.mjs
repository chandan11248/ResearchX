...
import {
	...
	RESEARCHX_UNDICI_VERSION,
} from "./lib/pi-undici-proxy-patch.mjs";
import {
	RESEARCHX_LITEPARSE_GIT_HEAD,
	RESEARCHX_LITEPARSE_VERSION,
...
const RESEARCHX_BRACE_EXPANSION_VERSION = "5.0.9";
const RESEARCHX_IP_ADDRESS_VERSION = "10.5.0";
const RESEARCHX_PI_DOCPARSER_VERSION = "4.0.0";
const PI_INTERACTIVE_UPDATE_NOTICE_MARKER = "// ResearchX: package update notices use the full update command.";
...
function fail(message) {
	throw new Error(`[researchx artifact] ${message}`);
}
...
if (manifest.dependencies?.["brace-expansion"] !== RESEARCHX_BRACE_EXPANSION_VERSION) {
	fail(`package.json does not pin brace-expansion ${RESEARCHX_BRACE_EXPANSION_VERSION}`);
}
if (manifest.overrides?.["brace-expansion"] !== RESEARCHX_BRACE_EXPANSION_VERSION) {
	fail(`package.json does not override brace-expansion ${RESEARCHX_BRACE_EXPANSION_VERSION}`);
}
if (manifest.overrides?.["ip-address"] !== RESEARCHX_IP_ADDRESS_VERSION) {
	fail(`package.json does not override ip-address ${RESEARCHX_IP_ADDRESS_VERSION}`);
}
if (
	readJson(packageRequire.resolve("ip-address/package.json"), "bundled ip-address manifest").version !==
	RESEARCHX_IP_ADDRESS_VERSION
) {
	fail(`bundled package does not resolve ip-address ${RESEARCHX_IP_ADDRESS_VERSION}`);
}
...
requireMarkers(
	...
	"installed runtime verifier",
	[
		"EXPECTED_RESEARCHX_COMMANDS",
		"EXPECTED_RESEARCHX_TOOLS",
		'message: "/tools"',
		"valid-typebox-probe",
		"null-typebox-probe",
...
for (const [label, path] of [
	...
	if (readJson(path, `${label} manifest`).version !== RESEARCHX_BRACE_EXPANSION_VERSION) {
		fail(`${label} is not ${RESEARCHX_BRACE_EXPANSION_VERSION}`);
	}
}
...
if (manifest.dependencies?.undici !== RESEARCHX_UNDICI_VERSION) {
	fail(`package.json does not pin Undici ${RESEARCHX_UNDICI_VERSION}`);
}
if (bundledPiManifest.dependencies?.undici !== RESEARCHX_UNDICI_VERSION) {
	fail(`bundled Pi does not pin Undici ${RESEARCHX_UNDICI_VERSION}`);
}
for (const [label, path] of [
	["ResearchX", packageRequire.resolve("undici/package.json")],
	[
		"bundled Pi",
		resolve(packageRoot, "node_modules", "@earendil-works", "pi-coding-agent", "node_modules", "undici", "package.json"),
	],
]) {
	if (readJson(path, `${label} Undici manifest`).version !== RESEARCHX_UNDICI_VERSION) {
		fail(`${label} does not resolve Undici ${RESEARCHX_UNDICI_VERSION}`);
	}
}

const archivePath = resolve(packageRoot, ".researchx", "runtime-workspace.tgz");
const digestPath = resolve(packageRoot, ".researchx", "runtime-workspace.sha256");
const runtimeLockPath = resolve(packageRoot, ".researchx", "runtime-package-lock.json");
...
if (expectedPiDocparserVersion !== RESEARCHX_PI_DOCPARSER_VERSION) {
	fail(`committed runtime lock does not pin pi-docparser ${RESEARCHX_PI_DOCPARSER_VERSION}`);
}
if (
...
if (runtimeLock.packages?.["node_modules/ip-address"]?.version !== RESEARCHX_IP_ADDRESS_VERSION) {
	fail(`committed runtime lock does not resolve ip-address ${RESEARCHX_IP_ADDRESS_VERSION}`);
}
verifyLiteparseRuntimeLockContract(runtimeLock, fail);
if (runtimeLock.packages?.[""]?.dependencies?.undici !== RESEARCHX_UNDICI_VERSION) {
	fail(`committed runtime lock does not pin Undici ${RESEARCHX_UNDICI_VERSION}`);
}
if (runtimeLock.packages?.["node_modules/undici"]?.version !== RESEARCHX_UNDICI_VERSION) {
	fail(`committed runtime lock does not resolve Undici ${RESEARCHX_UNDICI_VERSION}`);
}
if (
	runtimeLock.packages?.[""]?.dependencies?.["brace-expansion"] !== RESEARCHX_BRACE_EXPANSION_VERSION
) {
	fail(`committed runtime lock does not pin brace-expansion ${RESEARCHX_BRACE_EXPANSION_VERSION}`);
}
if (
	runtimeLock.packages?.["node_modules/brace-expansion"]?.version !== RESEARCHX_BRACE_EXPANSION_VERSION
) {
	fail(`committed runtime lock does not resolve brace-expansion ${RESEARCHX_BRACE_EXPANSION_VERSION}`);
}
for (const [packagePath, entry] of Object.entries(runtimeLock.packages ?? {})) {
	if (
		...
		entry?.version !== RESEARCHX_BRACE_EXPANSION_VERSION
	) {
		fail(`committed runtime lock does not pin Pi brace-expansion ${RESEARCHX_BRACE_EXPANSION_VERSION}`);
	}
	if (
		packagePath.endsWith("/pi-coding-agent/node_modules/undici") &&
		entry?.version !== RESEARCHX_UNDICI_VERSION
	) {
		fail(`committed runtime lock does not pin Pi Undici ${RESEARCHX_UNDICI_VERSION}`);
	}
}
...
if (
	...
	fail(`runtime archive does not resolve ip-address ${RESEARCHX_IP_ADDRESS_VERSION}`);
}
...
requireMarkers(
	...
	["probeEndpoint(cfg.endpoint, 300, cfg.headers)", "if (!process.env.RESEARCHX_POSTHOG_KEY)"],
);
...
requireMarkers(
	...
	["RESEARCHX_WEB_SEARCH_CONFIG", "PI_WEB_SEARCH_CONFIG", "configuredPath || join(getWebSearchConfigDir()"],
);
requireMarkers(
	readArchivedText(
		archivePath,
	...
	[
		"RESEARCHX_FETCH_CACHE_DIR",
		'join(process.cwd(), ".researchx", "cache", "fetch-content")',
...
for (const [label, entryPath] of [
	...
	if (readArchivedJson(archivePath, entryPath).version !== RESEARCHX_BRACE_EXPANSION_VERSION) {
		fail(`${label} is not ${RESEARCHX_BRACE_EXPANSION_VERSION}`);
	}
}
...
if (runtimePiManifest.dependencies?.undici !== RESEARCHX_UNDICI_VERSION) {
	fail(`runtime Pi does not pin Undici ${RESEARCHX_UNDICI_VERSION}`);
}
for (const [label, entryPath] of [
	...
	if (readArchivedJson(archivePath, entryPath).version !== RESEARCHX_UNDICI_VERSION) {
		fail(`${label} does not resolve Undici ${RESEARCHX_UNDICI_VERSION}`);
	}
}
...
console.log(JSON.stringify({
	...
	ipAddressVersion: RESEARCHX_IP_ADDRESS_VERSION,
	liteparseGitHead: RESEARCHX_LITEPARSE_GIT_HEAD,
	liteparseVersion: RESEARCHX_LITEPARSE_VERSION,
	piWebAccessVersion: expectedPiWebAccessVersion,
	undiciVersion: RESEARCHX_UNDICI_VERSION,
