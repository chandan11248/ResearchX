...
VERSION="latest"
SCOPE="${RESEARCHX_SKILLS_SCOPE:-codex}"
TARGET_DIR="${RESEARCHX_SKILLS_DIR:-}"
...
download_file() {
  ...
  echo "curl or wget is required to install ResearchX skills." >&2
  exit 1
}
...
resolve_target_dir() {
  ...
  case "$SCOPE" in
    repo)
      printf '%s/.agents/skills/researchx\n' "$PWD"
      ;;
    opencode)
      printf '%s/.opencode/skills/researchx\n' "$PWD"
      ;;
    codex | user)
      codex_home="${CODEX_HOME:-$HOME/.codex}"
      printf '%s/skills/researchx\n' "$codex_home"
      ;;
    *)
...
archive_url="${RESEARCHX_INSTALL_SKILLS_ARCHIVE_URL:-}"
if [ -z "$archive_url" ]; then
  case "$git_ref" in
    main)
      archive_url="https://github.com/chandan11248/ReSearchX/archive/refs/heads/main.tar.gz"
      ;;
    v*)
      archive_url="https://github.com/chandan11248/ReSearchX/archive/refs/tags/${git_ref}.tar.gz"
      ;;
  esac
fi
...
step "Installing ResearchX skills ${resolved_version} (${SCOPE})"
...
archive_path="$tmp_dir/researchx-skills.tar.gz"
...
printf 'ResearchX skills %s installed successfully.\n' "$resolved_version"
