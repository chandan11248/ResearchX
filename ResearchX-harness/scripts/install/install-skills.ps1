...
function Resolve-LatestReleaseVersion {
  $page = Invoke-WebRequest -Uri "https://github.com/chandan11248/ReSearchX/releases/latest"
...
function Resolve-VersionMetadata {
  ...
  return [PSCustomObject]@{
    ...
    DownloadUrl = if ($env:RESEARCHX_INSTALL_SKILLS_ARCHIVE_URL) { $env:RESEARCHX_INSTALL_SKILLS_ARCHIVE_URL } else { "https://github.com/chandan11248/ReSearchX/archive/refs/tags/v$resolvedVersion.zip" }
  }
}
...
function Resolve-InstallDir {
  ...
  if ($ResolvedScope -eq "Repo") {
    return Join-Path (Get-Location) ".agents\skills\researchx"
  }
  if ($ResolvedScope -eq "OpenCode") {
    return Join-Path (Get-Location) ".opencode\skills\researchx"
  }

  $codexHome = if ($env:CODEX_HOME) { $env:CODEX_HOME } else { Join-Path $HOME ".codex" }
  return Join-Path $codexHome "skills\researchx"
}
...
$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("researchx-skills-install-" + [System.Guid]::NewGuid().ToString("N"))
New-Item -ItemType Directory -Path $tmpDir | Out-Null

try {
  $archivePath = Join-Path $tmpDir "researchx-skills.zip"
  $extractDir = Join-Path $tmpDir "extract"

  Write-Host "==> Downloading ResearchX skills $resolvedVersion"
  ...
  if (-not $sourceRoot) {
    throw "Could not find extracted ResearchX archive."
  }
  ...
  Write-Host "ResearchX skills $resolvedVersion installed successfully."
} finally {
  if (Test-Path $tmpDir) {
