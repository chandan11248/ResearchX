...
import { RESEARCHX_LOGO_HTML } from "../logo.mjs";
...
import { sweepLegacyBrandMarkers } from "./lib/legacy-brand-sweep.mjs";
import {
	patchPiEditorSource,
...
const researchxHome = resolve(process.env.RESEARCHX_HOME ?? homedir(), ".researchx");
const researchxNpmPrefix = resolve(researchxHome, "npm-global");
process.env.RESEARCHX_NPM_PREFIX = researchxNpmPrefix;
process.env.NPM_CONFIG_PREFIX = researchxNpmPrefix;
process.env.npm_config_prefix = researchxNpmPrefix;
...
if (!piPackageRoot) {
	console.warn("[researchx] pi-coding-agent not found, skipping Pi patches");
}
...
const workspaceRoot = resolve(appRoot, ".researchx", "npm", "node_modules");
for (const legacyRoot of [resolve(appRoot, "node_modules"), workspaceRoot]) sweepLegacyBrandMarkers(legacyRoot); // rebrand pre-rename markers first
...
const settingsPath = resolve(appRoot, ".researchx", "settings.json");
const workspaceDir = resolve(appRoot, ".researchx", "npm");
const workspaceArchivePath = resolve(appRoot, ".researchx", "runtime-workspace.tgz");
const workspaceArchiveDigestPath = resolve(appRoot, ".researchx", "runtime-workspace.sha256");
const workspaceSetupLockDir = resolve(appRoot, ".researchx", ".workspace-setup.lock");
const globalNodeModulesRoot = process.platform === "win32"
	? resolve(researchxNpmPrefix, "node_modules")
	: resolve(researchxNpmPrefix, "lib", "node_modules");
...
function ensurePackageWorkspace() {
	...
	const heartbeat = () => {
		if (!heartbeatRuntimeWorkspaceSetupLock(workspaceSetupLockDir, lockToken)) {
			throw new Error(
				"ResearchX lost ownership of the runtime workspace setup lock.",
			);
		}
	};
	try {
...
function ensurePackageWorkspaceUnlocked(heartbeat) {
	...
	if (!installSeed) {
		throw new Error(
			"ResearchX could not read an authenticated package-lock restore seed.",
		);
	}
	const frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
	let frame = 0;
	...
	const spinner = setInterval(() => {
		...
		process.stderr.write(`\r${frames[frame++ % frames.length]} setting up researchx... ${elapsed}s`);
	}, 80);
	...
	if (!result) {
		...
		throw new Error("ResearchX could not restore its bundled research runtime.");
	} else {
		process.stderr.write("\r\x1b[2K");
		if (!workspaceMatchesRuntime(supportedPackageSpecs)) {
			throw new Error(
				"ResearchX restored an incomplete bundled research runtime.",
			);
		}
		ensureBundledPackageLinks(supportedPackageSpecs);
	}
...
function ensurePandoc() {
	...
	if ((process.env.RESEARCHX_SKIP_PANDOC_INSTALL) === "1") return;
	...
	console.log("[researchx] installing pandoc...");
	const result = spawnSync(brewPath, ["install", "pandoc"], {
		stdio: "inherit",
		timeout: 300000,
	});
	if (result.status !== 0) {
		console.warn("[researchx] warning: pandoc install failed, run `researchx --setup-preview` later");
	}
}
...
const agentNpmPiSubagentsRoot = resolve(researchxHome, "agent", "npm", "node_modules", "pi-subagents");
const globalPiOtelRoot = resolve(globalNodeModulesRoot, "pi-otel");
const agentNpmPiOtelRoot = resolve(researchxHome, "agent", "npm", "node_modules", "pi-otel");
const globalPiBtwRoot = resolve(globalNodeModulesRoot, "pi-btw");
const agentNpmPiBtwRoot = resolve(researchxHome, "agent", "npm", "node_modules", "pi-btw");
...
const agentNpmPiDocparserRoot = resolve(researchxHome, "agent", "npm", "node_modules", "pi-docparser");
...
if (packageJsonPath && existsSync(packageJsonPath)) {
	...
	if (pkg.piConfig?.name !== "researchx" || pkg.piConfig?.configDir !== ".researchx") {
		pkg.piConfig = {
			...(pkg.piConfig || {}),
			name: "researchx",
			configDir: ".researchx",
...
for (const entryPath of [cliPath, bunCliPath].filter(Boolean)) {
	...
	if (cliSource.includes('process.title = "pi";')) {
		cliSource = cliSource.replace('process.title = "pi";', 'process.title = "researchx";');
	}
	const stdinErrorGuard = [
		"const researchxHandleStdinError = (error) => {",
		...
		"    }",
		"};",
		'process.stdin?.on?.("error", researchxHandleStdinError);',
	].join("\n");
	if (!cliSource.includes('process.stdin?.on?.("error", researchxHandleStdinError);')) {
	...
	if (interactiveModeSource.includes("`π - ${sessionName} - ${cwdBasename}`")) {
		writeFileSync(
			...
			interactiveModeSource
				.replace("`π - ${sessionName} - ${cwdBasename}`", "`researchx - ${sessionName} - ${cwdBasename}`")
				.replace("`π - ${cwdBasename}`", "`researchx - ${cwdBasename}`"),
			"utf8",
		);
	}
}
...
const researchxUndiciPath = resolve(appRoot, "node_modules", "undici");
...
if (oauthPagePath && existsSync(oauthPagePath)) {
	...
	let changed = false;
	const target = `const LOGO_SVG = \`${RESEARCHX_LOGO_HTML}\`;`;
...
if (existsSync(piMemoryPath)) {
	...
	const memoryReplacement =
		'const MEMORY_DIR = process.env.RESEARCHX_MEMORY_DIR ?? process.env.PI_MEMORY_DIR ?? join(homedir(), ".pi", "memory");';
	...
	const execReplacement = [
		'const execBinary = process.env.RESEARCHX_NODE_EXECUTABLE || process.env.RESEARCHX_EXECUTABLE || "pi";',
		'      const execArgs = process.env.RESEARCHX_BIN_PATH',
		'        ? [process.env.RESEARCHX_BIN_PATH, "--prompt", prompt]',
		'        : ["-p", prompt, "--print"];',
