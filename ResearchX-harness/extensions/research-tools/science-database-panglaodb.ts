...
async function loadPanglaoMarkers(): Promise<PanglaoMarkerRow[]> {
	if (!panglaoMarkerRowsPromise) {
		panglaoMarkerRowsPromise = (async () => {
			const localPath = process.env.RESEARCHX_PANGLAODB_MARKERS_PATH?.trim();
			...
			const expectedDigest = process.env.RESEARCHX_PANGLAODB_MARKERS_SHA256?.trim() || PANGLAODB_MARKERS_SHA256;
...
export async function searchPanglaoDb(params: SearchParams): Promise<Record<string, unknown>> {
	...
	if (parsed.mode === "options") {
		...
		return {
			...
			provenance: {
				docs: panglaoDocs(),
				endpoints: [process.env.RESEARCHX_PANGLAODB_MARKERS_PATH?.trim() || PANGLAODB_MARKERS_URL],
			},
		};
	}
