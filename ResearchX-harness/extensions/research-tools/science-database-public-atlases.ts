	...
	try {
		const response = await fetch(url, {
			...
			headers: {
				...
				"user-agent": "researchx-science-database-search/1.0 (https://github.com/companion-ai/researchx)",
				...(init?.headers ?? {}),
			},
...
async function exactEqtlCatalogue(params: SearchParams, command: string, rest: string): Promise<Record<string, unknown>> {
	...
	if (command === "eqtl_list_datasets") {
		...
		return {
			schema: "researchx.scienceDatabaseSearch.v1",
...
async function exactGwasCatalog(params: SearchParams, command: string, rest: string): Promise<Record<string, unknown>> {
	...
	if (command === "gwas_get_study") {
		...
		try {
			...
			return { schema: "researchx.scienceDatabaseSearch.v1", source: "gwascatalog", query: params.query, searchMode: command, found: true, accession_id: accessionId, study, provenance: { docs: "https://www.ebi.ac.uk/gwas/rest/api/v2/docs/reference", endpoints: [url.toString()] } };
		} catch (error) {
			if (error instanceof Error && error.message.includes("404")) return { schema: "researchx.scienceDatabaseSearch.v1", source: "gwascatalog", query: params.query, searchMode: command, found: false, accession_id: accessionId, study: null, provenance: { docs: "https://www.ebi.ac.uk/gwas/rest/api/v2/docs/reference", endpoints: [url.toString()] } };
			throw error;
		}
	}
	if (command === "gwas_get_variant") {
		...
		try {
			...
			return { schema: "researchx.scienceDatabaseSearch.v1", source: "gwascatalog", query: params.query, searchMode: command, found: true, rs_id: rsId, variant, provenance: { docs: "https://www.ebi.ac.uk/gwas/rest/api/v2/docs/reference", endpoints: [url.toString()] } };
		} catch (error) {
			if (error instanceof Error && error.message.includes("404")) return { schema: "researchx.scienceDatabaseSearch.v1", source: "gwascatalog", query: params.query, searchMode: command, found: false, rs_id: rsId, variant: null, provenance: { docs: "https://www.ebi.ac.uk/gwas/rest/api/v2/docs/reference", endpoints: [url.toString()] } };
			throw error;
		}
	}
