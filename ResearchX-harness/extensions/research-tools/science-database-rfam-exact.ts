...
export async function searchRfamExact(params: SearchParams): Promise<Record<string, unknown> | undefined> {
	...
	if (command === "get_family") {
		...
		return { schema: "researchx.scienceDatabaseSearch.v1", source: "rfam", query, mode: command, ...result, provenance: { docs: rfamDocs(), endpoints } };
	}
	...
	if (command === "get_tree") {
		...
		return { schema: "researchx.scienceDatabaseSearch.v1", source: "rfam", query, mode: command, family, num_leaf_labels: leafCount, sha256: sha256Text(tree), tree, provenance: { docs: rfamDocs(), endpoints } };
	}
	if (command === "get_sequence_regions") {
		const url = rfamTextUrl(family, "/regions");
		endpoints.push(url.toString());
		const parsed = parseRegions(await fetchText(url), limit);
		return { schema: "researchx.scienceDatabaseSearch.v1", source: "rfam", query, mode: command, family, declared_count: parsed.declared_count, num_regions: parsed.regions.length, regions: parsed.regions, provenance: { docs: rfamDocs(), endpoints } };
	}
	if (command === "get_structure_mapping") {
		...
		return { schema: "researchx.scienceDatabaseSearch.v1", source: "rfam", query, mode: command, family, num_mappings: normalized.mapping.length, num_pdb_ids: normalized.pdb_ids.length, pdb_ids: normalized.pdb_ids, mapping: normalized.mapping, provenance: { docs: rfamDocs(), endpoints } };
	}
