...
function resolveResearchXSettingsPath(): string {
	const configured = (process.env.PI_CODING_AGENT_DIR ?? process.env.RESEARCHX_CODING_AGENT_DIR)?.trim();
...
function isResearchXLocalTool(tool: ToolInfo): boolean {
...
function isPublicTool(tool: ToolInfo): boolean {
	return isResearchXLocalTool(tool) || isPublicLivePackageToolName(tool.name);
}
...
export function registerDiscoveryCommands(pi: ExtensionAPI): void {
	pi.registerCommand("commands", {
		description: "Browse ResearchX workflow, project, and approved live runtime commands.",
