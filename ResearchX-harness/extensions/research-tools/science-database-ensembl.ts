...
async function ensemblHomology(params: SearchParams): Promise<Record<string, unknown>> {
	...
	if (geneSymbol) {
		...
		if (!geneId) {
			return { schema: "researchx.scienceDatabaseSearch.v1", source: "ensembl", searchMode: "ensembl-homology", found: false, geneSymbol, species, returned: 0, results: [], provenance: { docs: ENSEMBL_DOCS, endpoints } };
		}
	}
...
async function ensemblSequence(params: SearchParams): Promise<Record<string, unknown>> {
	...
	try {
		...
		if (error instanceof EnsemblRequestError && [400, 404].includes(error.status)) {
			return { schema: "researchx.scienceDatabaseSearch.v1", source: "ensembl", searchMode: "ensembl-sequence", species, stableId, region, found: false, returned: 0, results: [], provenance: { docs: ENSEMBL_DOCS, endpoints: [url.toString()] } };
		}
		throw error;
	}
}
