...
function sanitizeWorkbenchContext(cwd: string, includeDiagnostics: boolean | undefined): Record<string, unknown> {
	...
	const context = {
		schema: "researchx.workbenchContext.v1",
		...
		...(includeDiagnostics ? {
			diagnostics: [
				...
				"Streamable HTTP, SSE, and local command custom MCP connectors can be discovered with researchx_connector_tools and called through researchx_connector_call.",
				"Scientific model endpoints can be called with researchx_model_endpoint_call and save output artifacts under outputs/model-endpoints.",
...
export function registerWorkbenchContextTool(pi: ExtensionAPI): void {
	pi.registerTool({
		name: "researchx_workbench_context",
		label: "ResearchX Workbench Context",
		description:
			"Inspect sanitized in-app ResearchX science workbench resources: custom connectors, credential refs, permission grants, allowed domains, compute hosts, model endpoints, and memory categories. Never returns secret values.",
		promptSnippet: "Inspect configured ResearchX workbench resources and policies without exposing secret values.",
		promptGuidelines: [
			"Use researchx_workbench_context when a workbench chat needs to know which custom connectors, credential refs, grants, compute hosts, model endpoints, allowed domains, or memory categories are configured.",
			"Use researchx_connector_tools and researchx_connector_call for Streamable HTTP, SSE, or local command connector discovery and execution.",
			"Use researchx_model_endpoint_call for hosted ESMFold or self-hosted AlphaFold2 NIM inference calls.",
