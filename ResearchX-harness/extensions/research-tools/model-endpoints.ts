...
type ModelEndpointResult = {
	schema: "researchx.modelEndpointCall.v1";
	provider: ModelEndpointProvider;
	model: ModelEndpointModel;
	endpoint: string;
...
async function postModelEndpoint(params: ModelEndpointParams, signal?: AbortSignal): Promise<ModelEndpointResult> {
	...
	try {
		...
		const resultWithoutArtifacts = {
			schema: "researchx.modelEndpointCall.v1" as const,
...
export function registerModelEndpointTools(pi: ExtensionAPI): void {
	pi.registerTool({
		name: "researchx_model_endpoint_call",
		label: "ResearchX Model Endpoint Call",
		description:
			"Call configured ResearchX-owned scientific model endpoints. Supports NVIDIA BioNeMo/NIM ESMFold hosted calls through NVIDIA_API_KEY and self-hosted AlphaFold2 NIM sequence prediction endpoints.",
		promptSnippet: "Run a ResearchX scientific model endpoint call and save the returned structure or response as a provenance-backed artifact.",
		promptGuidelines: [
			"Use researchx_model_endpoint_call when a research chat needs a model-backed biology inference result, such as ESMFold or AlphaFold2 structure prediction.",
