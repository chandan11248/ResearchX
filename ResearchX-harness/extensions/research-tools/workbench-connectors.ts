...
function createPendingToolGrant(cwd: string, connector: NormalizedConnector, toolName: string): WorkbenchPermissionGrant {
	...
	const record = {
		...
		scope,
		decision: "ask",
		description: `Pending approval requested by researchx_connector_call for ${connector.name}.`,
	};
...
async function openLocalMcpSession(connector: NormalizedConnector, cwd: string): Promise<LocalMcpSession> {
	...
	const init = await sendLocalMcpMessage(session, {
		id: "researchx_initialize",
		jsonrpc: "2.0",
		method: "initialize",
		params: {
			capabilities: {},
			clientInfo: {
				name: "researchx-workbench",
				version: process.env.RESEARCHX_VERSION ?? "dev",
			},
			protocolVersion: MCP_PROTOCOL_VERSION,
		},
	}, { expectResponse: true });
...
async function listConnectorTools(cwd: string, connectorIdOrName?: string, options: ListConnectorToolsOptions = {}): Promise<{
	...
	schema: "researchx.connectorTools.v1";
}> {
...
async function callConnectorTool(cwd: string, connectorIdOrName: string, toolName: string, args: Record<string, unknown>, options: CallConnectorToolOptions = {}): Promise<{
	...
	grantDecision: ConnectorToolGrantDecision;
	result: unknown;
	schema: "researchx.connectorCall.v1";
	toolName: string;
}> {
	const settings = readSettings(cwd);
...
export function registerWorkbenchConnectorTools(pi: ExtensionAPI): void {
	pi.registerTool({
		name: "researchx_connector_tools",
		label: "ResearchX Connector Tools",
		description:
			"Discover tools exposed by configured ResearchX workbench Streamable HTTP, SSE, or local stdio MCP connectors and show their per-tool grant decision.",
		promptSnippet: "List configured ResearchX connector tools before using a custom research connector.",
		promptGuidelines: [
			"Use researchx_connector_tools before researchx_connector_call when you need the connector's exact tool names or schemas.",
			...
			"Tools with allow grants, wildcard allow grants, or connector skip-approvals can be called. Missing and ask grants create a pending approval request when researchx_connector_call is attempted.",
	...
	pi.registerTool({
		name: "researchx_connector_call",
		label: "ResearchX Connector Call",
		description:
			"Call one approved tool on a configured ResearchX workbench Streamable HTTP, SSE, or local stdio MCP connector, or create a pending approval request before execution.",
		promptSnippet: "Call an approved ResearchX connector tool with structured arguments.",
		promptGuidelines: [
			"Call researchx_connector_tools first when the exact connector tool name or input schema is unknown.",
			"When the workbench prompt names an active specialist, pass it as the specialist parameter so connector assignments are enforced.",
			"Use researchx_connector_call when the tool has an allow grant for connector:<connector-id>:<tool-name>, connector:<connector-id>:*, or connector skip-approvals. Missing or ask grants stop before execution and add a pending permission request.",
			"Do not call tools omitted from researchx_connector_tools because they are excluded for the active connector or specialist.",
