...
- Use visible ResearchX alpha tools such as `alpha_search` when present. For shell access, call `researchx alpha ...`; do not call the user's bare global `alpha` binary.
...
- `--window-size <chars>` or `RESEARCHX_SUMMARIZE_WINDOW_CHARS` (default: `6000`)
- `--overlap <chars>` or `RESEARCHX_SUMMARIZE_OVERLAP_CHARS` (default: `500`)
- `--tier1-threshold <chars>` or `RESEARCHX_SUMMARIZE_TIER1_THRESHOLD` (default: `8000`)
- `--tier2-threshold <chars>` or `RESEARCHX_SUMMARIZE_TIER2_THRESHOLD` (default: `60000`)
