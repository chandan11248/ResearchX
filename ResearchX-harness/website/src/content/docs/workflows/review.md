...
```bash
researchx review arxiv:2401.12345
researchx review my-draft.md
```

You can pass an arXiv ID, a URL, or a local file path. For arXiv papers, ResearchX fetches the source paper directly when the paper tools are available. For local PDFs, ResearchX attempts document extraction and records blocked checks if extraction fails.
...
The workflow reads or fetches the artifact, records evidence notes in `outputs/.drafts/<slug>-review-evidence.md`, and then writes exactly one final review to `outputs/<slug>-review.md`. For larger artifacts it can delegate evidence gathering or review synthesis to ResearchX's bundled research agents; for smaller artifacts it performs the review directly to avoid unnecessary orchestration.
