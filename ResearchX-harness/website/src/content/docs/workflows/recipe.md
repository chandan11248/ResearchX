...
The workflow borrows the useful idea from Hugging Face's open-source [`ml-intern`](https://github.com/huggingface/ml-intern) repo: make ML research outputs recipe-shaped and implementation-ready. ResearchX keeps the implementation native to Pi prompts, bundled skills, and read-only Hub inspection tools rather than copying `ml-intern`'s runtime loop or frontend.
...
ResearchX uses `verified`, `unverified`, `blocked`, and `inferred` labels precisely. It should not call a recipe state of the art, replicated, or production-ready unless the checks actually support that claim.
