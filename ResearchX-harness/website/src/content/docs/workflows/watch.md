...
```bash
researchx watch "New developments in state space models for sequence modeling"
```

After creating a watch, ResearchX writes the baseline artifact and the watch plan. Scheduled recurrence is created only when the `schedule_prompt` tool is visible.
