...
Slash commands are available inside the ResearchX REPL. They map to research workflows, research-session utilities, and setup utilities. Type `/help` inside the REPL for ResearchX's curated live command list.
...
| `/help` | Show grouped ResearchX commands and prefill the editor with a selected command |
| `/researchx-model` | Open the approved research model picker for the main default and per-subagent overrides |
...
The `/researchx-model` command opens an interactive picker that lets you either change the main approved research model or assign a different approved model to a bundled subagent like `researcher`, `reviewer`, `writer`, or `verifier`.
