...
function Resolve-LatestReleaseVersion {
  $page = Invoke-WebRequest `
    -Uri "https://github.com/chandan11248/ReSearchX/releases/latest" `
    -UseBasicParsing
  $match = [regex]::Match($page.Content, 'releases/tag/v([0-9][^"''<>\s]*)')
  if (-not $match.Success) {
    throw "Failed to resolve the latest ResearchX release version."
  }
...
function Resolve-ReleaseMetadata {
  ...
  $bundleName = "researchx-$resolvedVersion-$AssetTarget"
  $archiveName = "$bundleName.$BundleExtension"
  $baseUrl = if ($env:RESEARCHX_INSTALL_BASE_URL) { $env:RESEARCHX_INSTALL_BASE_URL } else { "https://github.com/chandan11248/ReSearchX/releases/download/v$resolvedVersion" }
...
function New-SameVolumeStagingRoot {
  ...
  for ($attempt = 1; $attempt -le 32; $attempt += 1) {
    $name = "researchx-stage-" + [System.Guid]::NewGuid().ToString("N").Substring(0, 12)
...
"@
  ...
  $candidateCmd = Join-Path $extractedBundleDir "researchx.cmd"
  $candidatePs1 = Join-Path $extractedBundleDir "researchx.ps1"
  ...
  $shimCandidate = Join-Path $stagedBinDir "researchx.cmd"
  @"
@echo off
CALL "$bundleDir\researchx.cmd" %*
"@ | Set-Content -Path $shimCandidate -Encoding ASCII
  ...
  try {
    ...
    if ($env:RESEARCHX_INSTALL_TEST_FAIL_AFTER_BUNDLE_BACKUP -eq "1") {
    ...
    Write-Host "==> Linking researchx into $installBinDir"
    if ($env:RESEARCHX_INSTALL_TEST_FAIL_AFTER_BUNDLE_SWAP -eq "1") {
  ...
  $staleBundleIndex = 0
  Get-ChildItem -LiteralPath $installRoot -Directory -Filter "researchx-*" |
  ...
  if (-not $alreadyOnPath) {
    ...
    Write-Host "Updated user PATH. Open a new shell to run researchx."
  } else {
    Write-Host "$installBinDir is already on PATH."
  }

  $resolvedCommand = Get-Command researchx -ErrorAction SilentlyContinue
  ...
  $expectedShimPaths = @(
    [System.IO.Path]::GetFullPath((Join-Path $installBinDir "researchx.cmd"))
  )
