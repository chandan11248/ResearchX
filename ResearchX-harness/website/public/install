...
VERSION="${1:-latest}"
INSTALL_BIN_DIR="${RESEARCHX_INSTALL_BIN_DIR:-$HOME/.local/bin}"
INSTALL_APP_DIR="${RESEARCHX_INSTALL_APP_DIR:-$HOME/.local/share/researchx}"
SKIP_PATH_UPDATE="${RESEARCHX_INSTALL_SKIP_PATH_UPDATE:-0}"
path_action="already"
path_profile=""
...
download_file() {
  ...
  echo "curl or wget is required to install ResearchX." >&2
  exit 1
}
...
add_to_path() {
  ...
  profile="${RESEARCHX_INSTALL_SHELL_PROFILE:-$HOME/.profile}"
  if [ -z "${RESEARCHX_INSTALL_SHELL_PROFILE:-}" ]; then
    case "${SHELL:-}" in
      */zsh)
        profile="$HOME/.zshrc"
        ;;
...
require_command() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "$1 is required to install ResearchX." >&2
    exit 1
  fi
}
...
sha256_file() {
  ...
  echo "sha256sum or shasum is required to verify the ResearchX download." >&2
  exit 1
}

warn_command_conflict() {
  expected_path="$INSTALL_BIN_DIR/researchx"
  resolved_path="$(command -v researchx 2>/dev/null || true)"
  ...
  if [ "$resolved_path" != "$expected_path" ]; then
    step "Warning: current shell resolves researchx to $resolved_path"
    step "Run now: export PATH=\"$INSTALL_BIN_DIR:\$PATH\" && hash -r && researchx"
...
resolve_release_metadata() {
  ...
  if [ "$normalized_version" = "latest" ]; then
    release_page="$(download_text "https://github.com/chandan11248/ReSearchX/releases/latest")"
    resolved_version="$(printf '%s\n' "$release_page" | sed -n 's@.*releases/tag/v\([0-9][^"<>[:space:]]*\).*@\1@p' | head -n 1)"

    if [ -z "$resolved_version" ]; then
      echo "Failed to resolve the latest ResearchX release version." >&2
      exit 1
    fi
  else
  ...
  bundle_name="researchx-${resolved_version}-${asset_target}"
  archive_name="${bundle_name}.${archive_extension}"
  release_base_url="${RESEARCHX_INSTALL_BASE_URL:-https://github.com/chandan11248/ReSearchX/releases/download/v${resolved_version}}"
...
step "Installing ResearchX ${resolved_version} for ${asset_target}"
...
if [ ! -x "$candidate_bundle/researchx" ]; then
  echo "Downloaded archive did not contain the expected executable: ${bundle_name}/researchx" >&2
  exit 1
fi
candidate_version="$("$candidate_bundle/researchx" --version | tail -n 1)"
...
"$candidate_bundle/researchx" --help >/dev/null
...
if ! mv "$candidate_bundle" "$bundle_dir"; then
  ...
  echo "Failed to install the validated ResearchX bundle." >&2
  exit 1
fi

step "Linking researchx into $INSTALL_BIN_DIR"
shim_candidate="$tmp_dir/researchx-shim"
if ! cat >"$shim_candidate" <<EOF
#!/bin/sh
set -eu
exec "$INSTALL_APP_DIR/$bundle_name/researchx" "\$@"
EOF
then
  rm -rf "$bundle_dir"
  if [ "$had_previous" -eq 1 ]; then
...
if ! mv "$shim_candidate" "$INSTALL_BIN_DIR/researchx"; then
  rm -rf "$bundle_dir"
  if [ "$had_previous" -eq 1 ]; then
    mv "$backup_dir" "$bundle_dir"
  fi
...
for old_bundle in "$INSTALL_APP_DIR"/researchx-*; do
...
case "$path_action" in
  ...
  *)
    ...
    step "Run: hash -r && researchx"
    ;;
esac

warn_command_conflict

printf 'ResearchX %s installed successfully.\n' "$resolved_version"
