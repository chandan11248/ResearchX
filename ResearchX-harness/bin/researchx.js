...
if (compareNodeVersions(parsedNodeVersion, parseNodeVersion(MIN_NODE_VERSION)) < 0 || parsedNodeVersion.major > MAX_NODE_MAJOR) {
  ...
  console.error(`researchx supports Node.js ${MIN_NODE_VERSION} through ${MAX_NODE_MAJOR}.x (detected ${process.versions.node}).`);
  ...
  console.error(isWindows
    ? "See https://github.com/chandan11248/ResearchX-harness/releases for a standalone build"
    : "See https://github.com/chandan11248/ResearchX-harness/releases for a standalone build");
  process.exit(1);
}
