...
const onboardingPermissionOptions = [
	...
	{ id: "science-databases", title: "Science databases", description: "Use ResearchX Bio Tools for public papers, genes, variants, proteins, structures, trials, and datasets." },
...
async function fetchJson<T>(path: string, init?: RequestInit): Promise<T> {
	const response = await fetch(path, {
		...
		headers: {
			...
			...(clientToken ? { "x-researchx-token": clientToken } : {}),
...
function commandPaletteGroups(state: WorkbenchState, query: string): CommandPaletteGroup[] {
	...
	const newProject: CommandPaletteItem[] = commandMatches(query, ["new project", "create project", "start project"])
		? [{
			...
			kind: "new-project",
			label: "New project",
			sublabel: "Create a ResearchX science project",
...
function App() {
	...
	async function steerMessage(trimmed: string) {
		...
		const queuedMessage: WorkbenchChatMessage = {
			...
			toolEvents: [{
				...
				status: "running",
				output: "Sending to the running ResearchX session",
			}],
		};
		setSession({
			...session,
	...
	async function sendChatText(
		...
		setBusy(true);
		setStatus(options.status ?? "ResearchX is working");
		setError(null);
		if (options.clearComposer !== false) setMessage("");
		setSession({
			...existing,
			...
			messages: [
				...
				},
				{
					...
					role: "assistant",
					content: "Starting ResearchX inside this workspace...",
					createdAt,
					status: "running",
					toolEvents: [{
						id: "pi-turn",
						label: "ResearchX Pi turn",
						status: "running",
					}],
				},
			],
	...
	if (error && !data) {
		return (
			<div className="boot-screen">
				<div>
					<p className="eyebrow">ResearchX Science</p>
							...
							<div className="topbar-menu-control">
								...
								{modelMenuOpen ? (
									<div className="topbar-menu-panel model-menu" role="menu" aria-label="Model menu">
										...
										>
											<span>Default</span>
											<small>{data?.modelStatus?.current ? `Uses ${data.modelStatus.current}` : "Uses the configured ResearchX default"}</small>
										</button>
						...
						{session?.messages.length ? (
									...
									<article key={chatMessage.id} className={cx("message", chatMessage.role)} data-transcript-message-id={chatMessage.id}>
										...
										<div className="message-body">
											<div className="message-meta">
												<span>{chatMessage.role === "assistant" ? "ResearchX" : chatMessage.role}</span>
							...
							runArtifacts.length ? (
								...
								<div className="empty-state">
									...
									<h2>Ask ResearchX to research, verify, reproduce, or synthesize.</h2>
											...
											<div className="cloud-storage-details">
												...
												<div className="cloud-storage-detail-actions">
													...
													<button
														type="button"
														onClick={() => {
															if (window.confirm(`Remove ${credential.name} from ResearchX cloud storage?`)) onRemoveCredential(credential.settingsRecordId);
														}}
													>
														Delete credential
													</button>
		...
		<div className="entry-shell onboarding-shell">
			...
			<section className="entry-panel onboarding-panel">
				<div className="entry-brand">
					...
					<div>
						<div className="brand-title">ResearchX</div>
				...
				<section className="launcher-columns">
					...
					<div className="launcher-section">
						...
						<div className="launcher-list seed-list">
							...
							{!seeds.length ? <div><strong>No seed workflows found</strong><span>ResearchX loads packaged open-science fixtures into workspace outputs on serve.</span></div> : null}
						</div>
					</div>
				</section>
			</section>
				...
				<div className="command-palette-list" role="listbox" aria-label="Search results">
					{groups.length ? groups.map((group) => (
						...
						<div className="command-palette-empty">
							...
							<span>Search across ResearchX projects, sessions, and artifacts.</span>
						</div>
					)}
				</div>
					...
					<Tabs.Content value="env" className="radix-tab-content">
						<div className="environment-manager">
							<header>
								...
								<span>Python venv and R library inside this ResearchX workspace.</span>
							</header>
							<div className="control-stack">
...
(function(nonce) {
	if (window.__researchxHtmlAnnotationBridge) return;
	window.__researchxHtmlAnnotationBridge = true;
	var mode = false;
	var hoverBox = null;
	...
	function makeOverlay(className) {
		...
		node.setAttribute("data-researchx-html-annotation", "1");
	...
	function setHighlights(items) {
		...
		(items || []).forEach(function(item) {
			try {
				var element = document.querySelector(item.selector);
				if (!element) return;
				var box = makeOverlay("researchx-html-annotation-box");
	...
	document.addEventListener("mousemove", function(event) {
		...
		if (!element || element.closest("[data-researchx-html-annotation]")) return;
		hoverTarget = element;
		if (!hoverBox) hoverBox = makeOverlay("researchx-html-hover-box");
...
})(__RESEARCHX_HTML_ANNOTATION_NONCE__);
`;
...
function injectHtmlAnnotationBridge(content: string, nonce: string): string {
	const script = `<script data-researchx-html-annotation="1">(function(){var __RESEARCHX_HTML_ANNOTATION_NONCE__=${JSON.stringify(nonce)};${htmlAnnotationBridgeScript}})();<\/script>`;
...
const jsonViewerStyles = {
	...defaultStyles,
	container: `${defaultStyles.container} researchx-json-tree`,
	childFieldsContainer: `${defaultStyles.childFieldsContainer} researchx-json-children`,
	basicChildStyle: `${defaultStyles.basicChildStyle} researchx-json-row`,
	label: `${defaultStyles.label} researchx-json-label`,
	clickableLabel: `${defaultStyles.clickableLabel} researchx-json-clickable-label`,
	stringValue: `${defaultStyles.stringValue} researchx-json-string`,
	numberValue: `${defaultStyles.numberValue} researchx-json-number`,
	booleanValue: `${defaultStyles.booleanValue} researchx-json-boolean`,
	nullValue: `${defaultStyles.nullValue} researchx-json-null`,
	otherValue: `${defaultStyles.otherValue} researchx-json-other`,
	punctuation: `${defaultStyles.punctuation} researchx-json-punctuation`,
	collapseIcon: `${defaultStyles.collapseIcon} researchx-json-toggle`,
	expandIcon: `${defaultStyles.expandIcon} researchx-json-toggle`,
	collapsedContent: `${defaultStyles.collapsedContent} researchx-json-collapsed`,
