...
test("Pi Undici patch updates the direct dependency and shrinkwrap", () => {
	...
	assert.equal(manifest.dependencies.undici, RESEARCHX_UNDICI_VERSION);

	const shrinkwrap = JSON.parse(patchPiCodingAgentUndiciShrinkwrapSource(piShrinkwrap()));
	assert.equal(shrinkwrap.packages[""].dependencies.undici, RESEARCHX_UNDICI_VERSION);
	assert.equal(shrinkwrap.packages["node_modules/undici"].version, RESEARCHX_UNDICI_VERSION);
	...
	for (const corrupted of [
		exactShrinkwrap.replace(RESEARCHX_UNDICI_VERSION, upstreamVersion),
		exactShrinkwrap.replace(
			`undici-${RESEARCHX_UNDICI_VERSION}.tgz`,
			"undici-wrong.tgz",
		),
...
test("Pi Undici patch updates the owning package lock", () => {
	...
	assert.equal(entry.version, RESEARCHX_UNDICI_VERSION);
...
test("Pi Undici patch replaces the nested package tree and is idempotent", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-pi-undici-"));
	...
	writeFileSync(join(safePackage, "package.json"), JSON.stringify({ name: "undici", version: RESEARCHX_UNDICI_VERSION }));
	...
	assert.equal(JSON.parse(readFileSync(join(nestedPackage, "package.json"), "utf8")).version, RESEARCHX_UNDICI_VERSION);
	assert.equal(JSON.parse(readFileSync(join(piRoot, "package.json"), "utf8")).dependencies.undici, RESEARCHX_UNDICI_VERSION);
	assert.equal(
...
test("embedded Pi resolves the patched Undici package", () => {
	...
	assert.equal(shrinkwrap.packages[""].dependencies.undici, RESEARCHX_UNDICI_VERSION);
	assert.equal(shrinkwrap.packages["node_modules/undici"].version, RESEARCHX_UNDICI_VERSION);
	assert.equal(nested.version, RESEARCHX_UNDICI_VERSION);
});
