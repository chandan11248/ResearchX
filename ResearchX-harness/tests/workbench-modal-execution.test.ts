...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-modal-"));
...
test("notebook Modal cloud mode writes and runs a Modal app script", async () => {
	...
	try {
		...
		writeFileSync(fakeModal, [
			...
			"grep -q \"RESEARCHX_MODAL_ARTIFACT_DIR\" \"$2\" || exit 5",
			"echo \"View run at https://modal.com/apps/companion/main/ap-test123\"",
			"printf '%s\\n' '__RESEARCHX_MODAL_RESULT__{\"artifacts\":[{\"contentBase64\":\"Y2xvdWQgYXJ0aWZhY3QK\",\"path\":\"outputs/modal-result.md\",\"size\":15}],\"exitCode\":0,\"stderr\":\"\",\"stdout\":\"remote ok\\n\"}'",
			"",
		].join("\n"));
