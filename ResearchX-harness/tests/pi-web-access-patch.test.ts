...
test("package artifact verification checks every pi-web-access patch target", () => {
	...
	assert.match(installedVerifier, /"\$encoded=\$env:RESEARCHX_DPAPI_FIXTURE_INPUT;"/);
	assert.match(installedVerifier, /RESEARCHX_DPAPI_FIXTURE_INPUT: value\.toString\("base64"\)/);
...
test("patchPiWebAccessSource rewrites legacy Pi web-search config paths", () => {
	...
	assert.match(patched, /RESEARCHX_WEB_SEARCH_CONFIG/);
	assert.match(patched, /PI_WEB_SEARCH_CONFIG/);
});

test("patchPiWebAccessSource keeps current upstream config helpers on ResearchX's exact config file", () => {
	const input = [
		'import { join } from "node:path";',
	...
	assert.match(patched, /process\.env\.RESEARCHX_WEB_SEARCH_CONFIG\?\.trim\(\)/);
...
test("exact pi-web-access fixture binds config reads and writes to ResearchX's path", () => {
...
test("runtime 0.25.0 normalizes PDF limits and honors OpenAI search provider priority", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-web-0241-config-"));
		...
		["--import", "tsx", "--input-type=module"],
		{
			...
			input: `
				...
				console.log(JSON.stringify({
					valid: parseGitHubUrl("https://github.com/chandan11248/ReSearchX"),
...
test("patched Linux browser launcher propagates immediate spawn failures", async () => {
	...
	const tempRoot = mkdtempSync(join(tmpdir(), "researchx-xdg-open-"));
...
test("patchPiWebAccessSources rejects unreviewed partial config-path patch state", () => {
	...
	for (const partial of [
		...
		},
		{
			...
			binding:
				'const WEB_SEARCH_CONFIG_PATH = process.env.RESEARCHX_WEB_SEARCH_CONFIG ?? process.env.PI_WEB_SEARCH_CONFIG ?? join(homedir(), ".pi", "web-search.json");',
			directory: "const dir = getWebSearchConfigDir();",
			helperImport:
				'import { formatSeconds, getWebSearchConfigDir, getWebSearchConfigPath, resolveCuratorNetworkConfig } from "./utils.ts";',
		},
...
test("patchPiWebAccessSource binds nested web model calls to Pi's resolved session scope", async () => {
	...
	const fixtureRoot = mkdtempSync(join(tmpdir(), "researchx-web-model-scope-"));
...
test("patchPiWebAccessSource keeps current fetched PDF scratch files inside the project", () => {
	...
	assert.match(patched, /RESEARCHX_FETCH_CACHE_DIR/);
	assert.match(patched, /process\.cwd\(\).*\.researchx.*cache.*fetch-content/);
