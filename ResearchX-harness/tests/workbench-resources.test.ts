...
test("resource directory searches and filters the full capabilities catalog", () => {
	const connectors = Array.from({ length: 9 }, (_, index) => ({
		...
		tools: index === 7 ? [{ name: "researchx_science_database_search", description: "Search Open Targets" }] : [],
	...
	assert.equal(filterResourceGroups(groups, { groupId: "connectors", query: "researchx_science_database_search" })[0]?.resources[0]?.name, "Open Targets");
});
