...
test("printSearchStatus marks config path and prints setup hint when web-search.json is missing", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-search-cmds-"));
	...
	assert.ok(
		output.some((line) => line.toLowerCase().includes("researchx search set")),
		"expected a hint referencing `researchx search set`",
	);
	assert.ok(
