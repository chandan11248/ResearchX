...
test("manual post-release gates exercise the live native installers", () => {
	...
	assert.match(installerJob[0], /https:\/\/researchx\.is\/install\b/);
	assert.match(installerJob[0], /https:\/\/researchx\.is\/install\.ps1/);
	assert.match(installerJob[0], /RESEARCHX_INSTALL_BIN_DIR/);
...
test("PR and publish workflows require clean package and consumer audits", () => {
	for (const workflow of [e2eWorkflow, publishWorkflow]) {
		assert.match(workflow, /npm audit --omit=dev --prefix \.researchx\/npm/);
		assert.match(workflow, /npm audit --omit=dev --prefix "\$consumer"/);
		assert.match(workflow, /\.researchx\/runtime-workspace\.tgz/);
		assert.match(workflow, /npm audit --omit=dev --prefix "\$runtime_audit\/npm"/);
		assert.doesNotMatch(
			workflow,
			/npm audit --omit=dev --prefix\s+\\?\s*"\$consumer\/node_modules\/@companion-ai\/researchx\/\.researchx\/npm"/,
		);
...
test("installed package gates verify ResearchX commands, tools, and TypeBox schemas across launchers", () => {
	...
	for (const workflow of [e2eWorkflow, publishWorkflow]) {
		assert.match(workflow, /bin="\$consumer\/node_modules\/\.bin\/researchx\.cmd"/);
		assert.match(
			workflow,
			/global_node_modules\/@companion-ai\/researchx\/scripts\/verify-installed-runtime\.mjs" "\$global_bin"/,
		);
...
test("package gates exercise the global npm install path", () => {
	...
	for (const workflow of [e2eWorkflow, publishWorkflow]) {
		assert.match(workflow, /global_bin="\$global_prefix\/researchx\.cmd"/);
		assert.match(workflow, /global_bin="\$global_prefix\/bin\/researchx"/);
	...
	assert.match(publishWorkflow, /"@companion-ai\/researchx@\$VERSION"/);
});
...
test("publish uses the exact verified tarball after native bundles pass", () => {
	...
	assert.match(
		consumerJob[0],
		/runtime_archive="\$consumer\/node_modules\/@companion-ai\/researchx\/\.researchx\/runtime-workspace\.tgz"/,
	);
...
test("version reconciliation and post-publish verification cover all release surfaces", () => {
	for (const id of ["linux-x64", "linux-arm64", "darwin-x64", "darwin-arm64", "win32-x64"]) {
		assert.match(publishWorkflow, new RegExp(`researchx-.*-${id.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}`));
	}
	...
	assert.match(
		publishWorkflow,
		/npm install --prefix "\$consumer" --omit=dev --no-audit \\\s+"@companion-ai\/researchx@\$VERSION"/,
	);
	...
	assert.doesNotMatch(
		publishWorkflow,
		/npm view "@companion-ai\/researchx@\$VERSION" version 2>\/dev\/null \|\| true/,
	);
