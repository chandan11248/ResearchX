...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-chat-context-"));
...
test("workbench chat prompt includes configured science runtime resources", () => {
	...
	try {
		...
		assert.match(prompt, /Pass specialist "Researcher" to researchx_connector_tools and researchx_connector_call/);
		...
		assert.match(prompt, /NVIDIA BioNeMo NIM \| nvidia-bionemo \| models esmfold, alphafold2 \| tool researchx_model_endpoint_call/);
...
test("workbench chat prompt includes built-in science database search without custom setup", () => {
	...
	try {
		...
		assert.match(prompt, /Configured science runtime resources:/);
		assert.match(prompt, /researchx_science_database_search/);
		...
		assert.match(prompt, /researchx_model_endpoint_call/);
...
test("notebook executions persist sanitized workbench environment snapshots", async () => {
	...
	try {
		...
		assert.equal(executed.environmentSnapshot?.schema, "researchx.workbenchEnvironmentSnapshot.v1");
