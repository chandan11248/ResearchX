...
function createInstalledProjectPackage(workingDir: string, packageName: string, version = "1.0.0"): void {
	createInstalledPackage(resolve(workingDir, ".researchx", "npm", "node_modules", packageName), packageName, version);
}
...
test("installPackageSources filters noisy npm chatter but preserves meaningful output", async () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-package-ops-"));
...
test("installPackageSources patches installed Pi packages before returning", async () => {
	...
	assert.match(patched, /RESEARCHX_SESSION_DIR/);
...
test("reconcileManagedCorePackageInstalls repairs stale user installs from the bundled exact presets", () => {
	...
	const bundledRoot = resolve(appRoot, ".researchx", "npm");
	const packageSource = "npm:pi-web-access@0.25.0";
	...
	createInstalledPackage(resolve(bundledRoot, "node_modules", "pi-web-access"), "pi-web-access", "0.25.0");
	mkdirSync(resolve(appRoot, ".researchx"), { recursive: true });
	writeFileSync(
		resolve(appRoot, ".researchx", "runtime-package-lock.json"),
		JSON.stringify({
			name: "researchx-pi-runtime",
			lockfileVersion: 3,
			packages: {
				"": { dependencies: { "pi-web-access": "0.25.0" } },
				"node_modules/pi-web-access": { version: "0.25.0" },
			},
		}, null, 2) + "\n",
	);
	writeFileSync(
...
test("reconcileManagedCorePackageInstalls removes a broken stale managed symlink", () => {
	...
	const bundledPackagePath = resolve(appRoot, ".researchx", "npm", "node_modules", "pi-web-access");
	const packageSource = "npm:pi-web-access@0.25.0";
	...
	createInstalledPackage(bundledPackagePath, "pi-web-access", "0.25.0");
	mkdirSync(resolve(appRoot, ".researchx"), { recursive: true });
	writeFileSync(
		resolve(appRoot, ".researchx", "runtime-package-lock.json"),
		JSON.stringify({
...
test("reconcileManagedCorePackageInstalls replaces a stale usable prefix copy", () => {
	...
	assert.deepEqual(reconcileManagedCorePackageInstalls(agentDir, appRoot), [packageSource]);
	assert.equal(readInstalledPackageVersion(globalPackagePath), "0.25.0");
	assert.equal(existsSync(managedPackagePath), false);
});
...
test("reconcileManagedCorePackageInstalls preserves a usable current prefix copy", () => {
	...
	createInstalledManagedPackage(agentDir, "pi-web-access", "0.21.0");
	createInstalledPackage(bundledPackagePath, "pi-web-access", "0.25.0");
	createInstalledPackage(globalPackagePath, "pi-web-access", "0.25.0");
...
test("updateConfiguredPackages updates project packages in the project install root", async () => {
	...
	const agentDir = resolve(root, "agent");
	const logPath = resolve(root, "npm-invocations.jsonl");
	mkdirSync(resolve(workingDir, ".researchx"), { recursive: true });
	...
	writeFileSync(
		resolve(workingDir, ".researchx", "settings.json"),
	...
	const invocation = invocations[0] ?? [];
	assert.ok(!invocation.includes("-g"));
	assert.equal(invocation[invocation.indexOf("--prefix") + 1], resolve(workingDir, ".researchx", "npm"));
	assert.equal(
		readInstalledPackageVersion(resolve(workingDir, ".researchx", "npm", "node_modules", "test-project")),
		"2.0.0",
	);
});
...
test("updateConfiguredPackages targets the effective project package when a source exists in both scopes", async () => {
	...
	assert.equal(invocation[invocation.indexOf("--prefix") + 1], resolve(workingDir, ".researchx", "npm"));
	assert.equal(
		readInstalledPackageVersion(resolve(workingDir, ".researchx", "npm", "node_modules", "test-duplicate")),
		"2.0.0",
	);
	assert.equal(
