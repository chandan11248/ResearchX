...
test("science database tool exposes bioRxiv and medRxiv parity modes", async () => {
	...
	const categories = await tools.get("researchx_science_database_search")?.execute("call-categories", {
		source: "biorxiv",
		query: "categories",
	});
	const dateWindow = await tools.get("researchx_science_database_search")?.execute("call-date-window", {
		source: "biorxiv",
		query: "cell date_from=2025-03-21 date_to=2025-03-28 category=\"cell biology\"",
		limit: 1,
	});
	const published = await tools.get("researchx_science_database_search")?.execute("call-published", {
		source: "medrxiv",
		query: "published 2020-03-01..2020-03-30",
		limit: 1,
	});
	const funder = await tools.get("researchx_science_database_search")?.execute("call-funder", {
		source: "biorxiv",
		query: "funder:00k4n6c32 2025-04-10..2025-07-10",
		limit: 1,
	});
	const contentStats = await tools.get("researchx_science_database_search")?.execute("call-content-stats", {
		source: "biorxiv",
		query: "content-stats interval=yearly through=2025",
	});
	const usageStats = await tools.get("researchx_science_database_search")?.execute("call-usage-stats", {
		source: "medrxiv",
