...
function makeFrameMessageWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-frame-messages-"));
...
test("workbench state exposes Claude-style frame message rows from ResearchX chat sessions", async () => {
	...
	try {
		await submitWorkbenchChatMessage({
			...
			executor: async (request) => ({
				...
				toolEvents: [{
					...
					label: "search papers",
					status: "complete",
					toolName: "researchx_science_database_search",
					input: "BRCA1",
					output: "one result",
				}],
			}),
		...
		const firstJson = JSON.parse(first.msgJson) as { _uuid: string; role: string; content: Array<{ type: string; text: string }>; researchx: { message_index: number; message_count: number } };
		...
		assert.equal(firstJson.researchx.message_index, 0);
		assert.equal(firstJson.researchx.message_count, 2);

		const assistantJson = JSON.parse(state.frameMessages[1]!.msgJson) as { role: string; researchx: { tool_events: Array<{ tool_name: string; output: string }> } };
		assert.equal(assistantJson.role, "assistant");
		assert.equal(assistantJson.researchx.tool_events[0]?.tool_name, "researchx_science_database_search");
		assert.equal(assistantJson.researchx.tool_events[0]?.output, "one result");
	} finally {
