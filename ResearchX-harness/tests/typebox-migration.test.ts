...
test("research extension source and direct locks use Pi's coordinated TypeBox package", () => {
	...
	const runtimeLock = readJson(join(repoRoot, ".researchx", "runtime-package-lock.json"));
...
test("Pi runtime validation omits null AlphaFold2 databases without losing optional arrays", () => {
	...
	const tool = tools.get("researchx_model_endpoint_call");
	assert.ok(tool);
	const validate = (arguments_: Record<string, unknown>) =>
		validateToolArguments(tool, {
			type: "toolCall",
	...
	assert.throws(
		...
		/Validation failed for tool "researchx_model_endpoint_call":[\s\S]*databases: must be array/,
	);
});
