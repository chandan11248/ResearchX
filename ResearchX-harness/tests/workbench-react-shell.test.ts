...
function makeAppRoot(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-react-shell-"));
...
function sessionFixture(): WorkbenchChatSession {
	return {
		...
		piSession: {
			id: "researchx-workbench-scaling-laws",
			status: "pending",
			messageCount: 0,
			userMessages: 0,
			assistantMessages: 0,
...
test("workbench server serves the authenticated React app shell on default and app-shell routes", async () => {
	...
	const workingDir = mkdtempSync(join(tmpdir(), "researchx-react-shell-workspace-"));
...
test("React shell tool activity helpers classify connector approvals", () => {
	...
	const pending = {
		...
		status: "error",
		toolName: "researchx_connector_call",
		input: "{\"query\":\"BRCA1\"}",
		output: approvalText,
		isError: true,
	...
	const groups = [
		{
			id: "connectors",
			title: "Connectors",
			description: "ResearchX-owned connectors",
			resources: [{
		...
		},
		{
			...
			resources: [{
				...
				description: "Pending approval requested by researchx_connector_call.",
				status: "available",
	...
	const uploads = [
		{
			...
			sizeBytes: 42,
			createdAt: "2026-07-02T00:00:00.000Z",
			storagePath: ".researchx/workbench/uploads/scaling-laws/upload-1-cells.csv",
		...
		},
		{
			...
			sizeBytes: 100,
			createdAt: "2026-07-02T00:00:01.000Z",
			storagePath: ".researchx/workbench/uploads/scaling-laws/upload-2-figure.png",
			truncated: true,
		},
...
test("React shell resource helpers expose actionable Customize controls", () => {
	...
	const packageResource = {
		...
		packageSources: ["@researchx/biomart"],
		tags: ["connector"],
	} satisfies WorkbenchResource;
	const storedRecord = {
		id: "grant-ask",
