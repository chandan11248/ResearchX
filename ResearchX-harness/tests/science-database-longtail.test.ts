...
const originalPanglaoChecksum = process.env.RESEARCHX_PANGLAODB_MARKERS_SHA256;
const originalPanglaoPath = process.env.RESEARCHX_PANGLAODB_MARKERS_PATH;

afterEach(() => {
	globalThis.fetch = originalFetch;
	if (originalPanglaoChecksum === undefined) delete process.env.RESEARCHX_PANGLAODB_MARKERS_SHA256;
	else process.env.RESEARCHX_PANGLAODB_MARKERS_SHA256 = originalPanglaoChecksum;
	if (originalPanglaoPath === undefined) delete process.env.RESEARCHX_PANGLAODB_MARKERS_PATH;
	else process.env.RESEARCHX_PANGLAODB_MARKERS_PATH = originalPanglaoPath;
});
...
test("science database tool searches PanglaoDB marker genes and options from the pinned TSV", async () => {
	...
	process.env.RESEARCHX_PANGLAODB_MARKERS_SHA256 = createHash("sha256").update(gz).digest("hex");
	const requests: string[] = [];
	globalThis.fetch = async (input) => {
		const url = String(input);
		requests.push(url);
