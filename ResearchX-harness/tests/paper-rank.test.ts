...
test("resolvePaperAccess writes Markdown-safe provider titles and links", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-markdown-safe-"));
...
test("runPaperRank full-text enrichment discovers Europe PMC content from DOI-only OpenAlex papers", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-rank-europe-pmc-"));
	const calls: string[] = [];
	const openAlexWork = {
...
test("resolvePaperAccess writes bounded paper-access artifacts", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-output-"));
	const result = await resolvePaperAccess({
...
test("resolvePaperAccess escapes provider full-text source labels in Markdown reports", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-source-safe-"));
	const result = await resolvePaperAccess({
		identifier: "10.0000/foundation",
		sourceFixture: fixturePath,
		outputDir,
...
test("resolvePaperAccess rejects unrelated OpenAlex hits for arXiv identifiers", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-arxiv-output-"));
	const calls: string[] = [];
	const fetchImpl = async (input: string | URL | Request) => {
		const url = String(input);
		calls.push(url);
...
test("resolvePaperAccess enriches arXiv fallback records from the arXiv API", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-arxiv-metadata-output-"));
	const calls: string[] = [];
	const fetchImpl = async (input: string | URL | Request) => {
		const url = String(input);
		calls.push(url);
...
test("resolvePaperAccess accepts OpenAlex hits whose arXiv id is only in secondary locations", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-secondary-arxiv-output-"));
	const fetchImpl = async () => new Response(JSON.stringify({
		meta: { count: 1 },
		results: [
			{
...
test("resolvePaperAccess checks multiple OpenAlex candidates for arXiv identifiers", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-arxiv-candidates-output-"));
...
test("resolvePaperAccess treats title-like arXiv-shaped numbers as title search text", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-title-number-output-"));
...
test("resolvePaperAccess checks multiple OpenAlex candidates for title searches", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-title-candidates-output-"));
...
test("resolvePaperAccess rejects unrelated OpenAlex title search results", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-title-unrelated-output-"));
	const identifier = "Sparse Autoencoders Find Highly Interpretable Features in Language Models";
	const calls: string[] = [];
	const fetchImpl = async (input: string | URL | Request) => {
		const url = String(input);
...
test("resolvePaperAccess treats title-like DOI substrings as title search text", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-title-doi-output-"));
...
test("resolvePaperAccess keeps explicit DOI inputs on the DOI lookup path", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-explicit-doi-output-"));
	const calls: string[] = [];
	const fetchImpl = async (input: string | URL | Request) => {
		const url = String(input);
		calls.push(url);
...
test("resolvePaperAccess keeps explicit OpenAlex IDs on the OpenAlex lookup path", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-explicit-openalex-output-"));
	const calls: string[] = [];
	const fetchImpl = async (input: string | URL | Request) => {
		const url = String(input);
		calls.push(url);
...
test("resolvePaperAccess keeps explicit PMID inputs on the PMID lookup path", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-explicit-pmid-output-"));
	const calls: string[] = [];
	const fetchImpl = async (input: string | URL | Request) => {
		const url = String(input);
		calls.push(url);
...
test("resolvePaperAccess keeps explicit PMCID inputs on the PMCID lookup path", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-access-explicit-pmcid-output-"));
	const calls: string[] = [];
	const fetchImpl = async (input: string | URL | Request) => {
		const url = String(input);
		calls.push(url);
...
test("runPaperRank writes durable report, data, graph, and provenance artifacts", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-rank-"));
	const result = await runPaperRank({
		topic: "mechanistic interpretability sparse autoencoders",
		limit: 4,
		outputDir,
	...
	assert.equal(researchRun.schemaVersion, "researchx.researchRun.v1");
	...
	assert.equal(calibrationTemplate.schemaVersion, "researchx.paperRank.preferenceTemplate.v1");
	...
	assert.equal(reproductionTemplate.schemaVersion, "researchx.paperRank.reproductionNotesTemplate.v1");
	...
	assert.match(graphExplorerHtml, /ResearchX PaperRank Graph Explorer/);
	...
	assert.match(synthesisPrompt, /ResearchX PaperRank Model Synthesis Prompt/);
...
test("runPaperRank writes Markdown-safe topic headings and shell snippets", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-rank-safe-topic-"));
	...
	const guideCommand = guide.split("\n").find((line) => line.startsWith("researchx rank"));
	...
	for (const line of [guideCommand, calibrationInstruction, reproductionInstruction]) {
		assert.ok(line);
		assert.match(line, /researchx rank '/);
...
test("runPaperRank escapes script-breaking graph artifact data", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-rank-html-safe-"));
...
test("runPaperRank redacts model synthesis failure messages from artifacts", async () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-rank-synthesis-failure-"));
	const result = await runPaperRank({
		topic: "mechanistic interpretability sparse autoencoders",
		limit: 2,
		outputDir,
...
test("researchx rank works end to end through the CLI with a fixture source", () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-rank-cli-output-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-rank-cli-home-"));
...
test("researchx rank default output is concise and decision-first", () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-rank-cli-concise-output-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-rank-cli-concise-home-"));
	const result = spawnSync(
		process.execPath,
		[
			"--import",
...
test("researchx rank writes default outputs under --cwd", () => {
	const callerDir = mkdtempSync(join(tmpdir(), "researchx-rank-caller-"));
	const workingDir = mkdtempSync(join(tmpdir(), "researchx-rank-workspace-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-rank-cwd-home-"));
...
test("researchx paper works end to end through the CLI with a fixture source", () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-cli-output-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-paper-cli-home-"));
	const result = spawnSync(
		process.execPath,
		[
			"--import",
...
test("researchx paper default output names the best access route", () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-paper-cli-route-output-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-paper-cli-route-home-"));
	const result = spawnSync(
		process.execPath,
		[
			"--import",
...
test("researchx paper writes default outputs under --cwd", () => {
	const callerDir = mkdtempSync(join(tmpdir(), "researchx-paper-caller-"));
	const workingDir = mkdtempSync(join(tmpdir(), "researchx-paper-workspace-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-paper-cwd-home-"));
	cpSync(fixturePath, join(workingDir, "openalex-rank.json"));
	const result = spawnSync(
		process.execPath,
		[
...
test("researchx rank accepts a preference file through the CLI", () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-rank-cli-calibration-output-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-rank-cli-calibration-home-"));
	const result = spawnSync(
		process.execPath,
		[
			"--import",
...
test("researchx rank accepts reproduction notes through the CLI", () => {
	const outputDir = mkdtempSync(join(tmpdir(), "researchx-rank-cli-reproduction-output-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-rank-cli-reproduction-home-"));
	const result = spawnSync(
		process.execPath,
		[
			"--import",
