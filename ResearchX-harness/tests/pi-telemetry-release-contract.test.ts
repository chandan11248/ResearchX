...
import {
	RESEARCHX_PI_TELEMETRY_INTEGRITY,
	RESEARCHX_PI_TELEMETRY_PACKAGE,
	RESEARCHX_PI_TELEMETRY_RESOLVED,
...
test("Pi telemetry runtime pin falls back only when package-lock is not shipped", () => {
	...
	assert.equal(
		resolvePiTelemetryRuntimeVersion(RESEARCHX_PI_TELEMETRY_VERSION, true),
		RESEARCHX_PI_TELEMETRY_VERSION,
	);
	assert.throws(
...
function makeRuntimeLock(): PiTelemetryRuntimeLock {
	return {
		packages: {
			...
			[TOP_LEVEL_LOCK_PATH]: {
				version: RESEARCHX_PI_TELEMETRY_VERSION,
				resolved: RESEARCHX_PI_TELEMETRY_RESOLVED,
				integrity: RESEARCHX_PI_TELEMETRY_INTEGRITY,
			},
...
function makeArchiveEntries() {
	return new Map<string, unknown>([
		...
		[
			...
			{
				name: RESEARCHX_PI_TELEMETRY_PACKAGE,
				version: RESEARCHX_PI_TELEMETRY_VERSION,
			},
		],
		[
