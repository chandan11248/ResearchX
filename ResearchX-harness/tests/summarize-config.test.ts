...
test("summarize prompt exposes configurable window and threshold knobs", () => {
	...
	assert.match(summarizePrompt, /RESEARCHX_SUMMARIZE_WINDOW_CHARS/i);
	assert.match(summarizePrompt, /RESEARCHX_SUMMARIZE_OVERLAP_CHARS/i);
	assert.match(summarizePrompt, /RESEARCHX_SUMMARIZE_TIER1_THRESHOLD/i);
	assert.match(summarizePrompt, /RESEARCHX_SUMMARIZE_TIER2_THRESHOLD/i);
