...
test("science database tool summarizes ResearchX-owned ClinGen gene curation records", async () => {
	...
	const result = await tools.get("researchx_science_database_search")?.execute("call-clingen-summary", {
		source: "clingen",
		query: "BRCA1",
		limit: 2,
	});
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /ClinGen/);
});
...
test("science database tool returns bounded ClinGen warnings for transient actionability failures", async () => {
	...
	const result = await registerTools().get("researchx_science_database_search")?.execute("call-clingen-actionability", {
		source: "clingen",
