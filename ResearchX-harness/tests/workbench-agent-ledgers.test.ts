...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-agent-ledgers-"));
...
test("buildWorkbenchState exposes Claude-style agent and bundled-agent setting rows", () => {
	...
	try {
		...
		assert.equal(researcher?.url, "researchx://agents/researcher");
		...
		assert.equal(parameters.source, "researchx-bundled-agent");
		assert.equal(parameters.path, ".researchx/agents/researcher.md");
...
test("workbench server returns agent ledgers through state", async () => {
	...
	try {
		...
		assert.equal(payload.agents[0]?.url, "researchx://agents/researcher");
		assert.match(payload.agents[0]?.parameters ?? "", /researchx-bundled-agent/);
