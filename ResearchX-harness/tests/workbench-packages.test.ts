...
function writePackage(root: string, name: string, manifest: Record<string, unknown>): void {
	const packageRoot = join(root, ".researchx", "npm", "node_modules", ...name.split("/"));
...
test("workbench connector resources expose package state and lifecycle metadata", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-packages-"));
	try {
		...
		assert.ok(webAccess?.diagnostics?.some((item) => item.includes(".researchx/settings.json")), "expected project config diagnostic");
		assert.ok(webAccess?.diagnostics?.some((item) => item.includes(".researchx/npm/node_modules/pi-web-access")), "expected install path diagnostic");
...
test("workbench package setting updates enable and disable project package sources", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-package-settings-"));
	try {
		mkdirSync(join(root, ".researchx"), { recursive: true });
		const settingsPath = join(root, ".researchx", "settings.json");
