...
test("runtime Windows Chrome handles large expiries and decrypts DPAPI-backed v10 cookies", () => {
	const home = mkdtempSync(join(tmpdir(), "researchx-cookie-windows-"));
	const bin = mkdtempSync(join(tmpdir(), "researchx-cookie-bin-"));
	try {
...
test("runtime Windows Edge reports unsupported v20 app-bound Gemini cookies", () => {
	const home = mkdtempSync(join(tmpdir(), "researchx-cookie-edge-"));
	const bin = mkdtempSync(join(tmpdir(), "researchx-cookie-bin-"));
	try {
