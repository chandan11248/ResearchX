...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-new-session-"));
...
test("workbench state exposes ResearchX model status for the model menu", async () => {
	const root = makeWorkspace();
...
test("workbench onboarding creates project, session, profile, and setup settings", async () => {
	...
	try {
		...
		assert.deepEqual(payload.onboarding.suggestedConnectors, ["ResearchX Bio Tools"]);
		...
		assert.equal(settings.permissionGrants.some((grant) => grant.id === "onboarding-researchx-bio-tools" && grant.scope === "builtin:researchx_science_database_search"), true);
		...
		assert.ok(intents.includes("connector:ResearchX Bio Tools"));
...
test("ResearchX-owned open-science seed fixtures are packaged with all reference-shaped workflows", () => {
...
test("open-science seed fixture sync materializes missing files without overwriting workspace edits", () => {
	const appRoot = mkdtempSync(join(tmpdir(), "researchx-seed-fixture-app-"));
	const root = makeWorkspace();
	try {
