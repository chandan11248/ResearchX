...
function makeSettingsWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-settings-"));
	...
	mkdirSync(join(root, ".researchx", "agents"), { recursive: true });
...
test("buildWorkbenchState exposes mutable science settings resources", () => {
	...
	const previousClaudeScienceReference = process.env.RESEARCHX_DEBUG_CLAUDE_SCIENCE_REFERENCE;
	try {
		delete process.env.RESEARCHX_DEBUG_CLAUDE_SCIENCE_REFERENCE;
		...
		const network = state.resources.find((group) => group.id === "network")?.resources ?? [];
			...
			assert.equal(connectors[0]?.name, "ResearchX Bio Tools");
			assert.equal(connectors[0]?.source, "ResearchX built-in science tools");
			...
			assert.equal(biomart?.tools?.some((tool) => tool.name === "researchx_science_database_search"), true);
			...
			assert.equal(zinc?.tools?.some((tool) => tool.name === "researchx_science_database_search"), true);
		...
		assert.equal(pubmed?.tools?.some((tool) => tool.name === "researchx_science_database_search"), true);
			...
			assert.equal(resource?.tools?.some((tool) => tool.name === "researchx_science_database_search"), true);
		}
			...
			assert.equal(ketcher?.tools?.some((tool) => tool.name === "researchx_open_chemistry_sketcher"), true);
		...
		if (previousClaudeScienceReference === undefined) delete process.env.RESEARCHX_DEBUG_CLAUDE_SCIENCE_REFERENCE;
		else process.env.RESEARCHX_DEBUG_CLAUDE_SCIENCE_REFERENCE = previousClaudeScienceReference;
...
test("buildWorkbenchState reflects science provider credential sources", () => {
	const root = makeSettingsWorkspace();
	const previousKaggleUser = process.env.KAGGLE_USERNAME;
	const previousKaggleKey = process.env.KAGGLE_KEY;
	const previousNvidiaKey = process.env.NVIDIA_API_KEY;
	try {
		process.env.KAGGLE_USERNAME = "kaggle-test-user";
		process.env.KAGGLE_KEY = "kaggle-test-key";
		process.env.NVIDIA_API_KEY = "nvidia-test-key";

		const state = buildWorkbenchState({ workingDir: root, version: "0.0.0-test" });
		const compute = state.resources.find((group) => group.id === "compute")?.resources ?? [];
		const credentials = state.resources.find((group) => group.id === "credentials")?.resources ?? [];
		const kaggleProvider = state.compute.find((provider) => provider.id === "kaggle");
		const bionemoProvider = state.compute.find((provider) => provider.id === "nvidia-bionemo");
		assert.equal(kaggleProvider?.status, "configured");
		assert.equal(kaggleProvider?.enabled, true);
		assert.equal(bionemoProvider?.status, "configured");
		assert.equal(bionemoProvider?.enabled, true);
		assert.equal(bionemoProvider?.tools?.some((tool) => tool.name === "researchx_model_endpoint_call"), true);

		const kaggleCompute = compute.find((resource) => resource.id === "compute-kaggle");
		assert.equal(kaggleCompute?.status, "configured");
		assert.equal(kaggleCompute?.detail, "KAGGLE_USERNAME / KAGGLE_KEY");
		assert.equal(kaggleCompute?.diagnostics?.some((item) => item.includes("KAGGLE_USERNAME")), true);
		...
		assert.equal(bionemoCompute?.diagnostics?.some((item) => item.includes("researchx_model_endpoint_call")), true);
		assert.equal(bionemoCompute?.diagnostics?.some((item) => item.includes("outputs/model-endpoints")), true);
		assert.equal(bionemoCompute?.tools?.some((tool) => tool.name === "researchx_model_endpoint_call"), true);

		const kaggleCredential = credentials.find((resource) => resource.id === "credential-kaggle");
		assert.equal(kaggleCredential?.status, "configured");
		assert.equal(kaggleCredential?.detail, "KAGGLE_USERNAME / KAGGLE_KEY");
		assert.equal(kaggleCredential?.diagnostics?.some((item) => item.includes("KAGGLE_USERNAME and KAGGLE_KEY are present")), true);
		...
		if (previousKaggleUser === undefined) delete process.env.KAGGLE_USERNAME;
		else process.env.KAGGLE_USERNAME = previousKaggleUser;
		if (previousKaggleKey === undefined) delete process.env.KAGGLE_KEY;
		else process.env.KAGGLE_KEY = previousKaggleKey;
...
test("workbench server mutates science settings resources through the authenticated API", async () => {
	...
	try {
		const headers = {
			...
			cookie: "researchx_workbench=test-token",
		};
