...
const runtimeRoot = resolve(appRoot, ".researchx", "npm");
...
function createPatchedPackageRoot(): string {
	const root = mkdtempSync(resolve(tmpdir(), "researchx-web-025-patched-"));
...
test("pi-web-access 0.25 renders bounded GitHub PR research documents", async () => {
	...
	assert.deepEqual(
		github.parseGitHubIssuePrUrl(
			"https://github.com/chandan11248/ReSearchX/pull/259#discussion_r123",
		),
		{
			owner: "chandan11248",
			repo: "ReSearchX",
	...
	const rendered = github.renderGitHubPrIssue({
		url: "https://github.com/chandan11248/ReSearchX/pull/259",
		owner: "chandan11248",
		repo: "ReSearchX",
		kind: "pull",
		number: 259,
		view: {
			...
			headRepositoryOwner: { login: "chandan11248" },
			additions: 10,
			deletions: 2,
			changedFiles: 3,
			commits: [],
...
test("patched curl proxy transport keeps credentials and headers out of process argv", () => {
	const packageRoot = createPatchedPackageRoot();
	const root = mkdtempSync(resolve(tmpdir(), "researchx-web-curl-stdin-"));
	try {
		...
		const fakeCurlSource = `
			...
			process.stdin.on("end", () => {
				...
				fs.writeFileSync(process.env.RESEARCHX_CURL_LOG, JSON.stringify({ args, stdin: input }));
...
test("patched GitHub issue and PR gh calls inherit the per-call proxy and bypass it via NO_PROXY", () => {
	const packageRoot = createPatchedPackageRoot();
	const root = mkdtempSync(resolve(tmpdir(), "researchx-web-gh-proxy-"));
	try {
		const bin = resolve(root, "bin");
		...
		const fakeGhSource = `
			const fs = require("node:fs");
			const args = process.argv.slice(2);
			fs.appendFileSync(process.env.RESEARCHX_GH_PROXY_LOG, JSON.stringify({
				args,
		...
		const child = spawnSync(
			...
			["--import", "tsx", "--input-type=module"],
			{
				...
				env: {
					...process.env,
					PATH: `${bin}${delimiter}${process.env.PATH ?? ""}`,
					RESEARCHX_GH_PROXY_LOG: logPath,
					RESEARCHX_WEB_SEARCH_CONFIG: resolve(root, "web-search.json"),
					NO_PROXY: "",
					no_proxy: "",
					HTTP_PROXY: "http://inherited.invalid:1",
					HTTPS_PROXY: "http://inherited.invalid:1",
...
test("patched GitHub clones propagate explicit, bypassed, and forced-direct proxy decisions", () => {
	const packageRoot = createPatchedPackageRoot();
	const root = mkdtempSync(resolve(tmpdir(), "researchx-web-clone-proxy-"));
	try {
		const bin = resolve(root, "bin");
		...
		const fakeCommandSource = `
			...
			fs.appendFileSync(process.env.RESEARCHX_CLONE_PROXY_LOG, JSON.stringify({
				scenario: process.env.RESEARCHX_SCENARIO,
				command,
				args,
				HTTP_PROXY: process.env.HTTP_PROXY ?? null,
				HTTPS_PROXY: process.env.HTTPS_PROXY ?? null,
			...
			if (command === "gh" && args[0] === "--version") {
				if (process.env.RESEARCHX_GH_AVAILABLE === "1") {
					console.log("gh version test");
					process.exit(0);
				}
				process.exit(1);
			...
			const child = spawnSync(
				...
				["--import", "tsx", "--input-type=module"],
				{
					...
					env: {
						...process.env,
						PATH: `${bin}${delimiter}${process.env.PATH ?? ""}`,
						RESEARCHX_CLONE_PROXY_LOG: logPath,
						RESEARCHX_SCENARIO: scenario.name,
						RESEARCHX_GH_AVAILABLE: scenario.ghAvailable ? "1" : "0",
						RESEARCHX_WEB_SEARCH_CONFIG: configPath,
...
test("pi-web-access 0.25 loads explicit Gemini ADC project and location config", (t) => {
	const root = mkdtempSync(resolve(tmpdir(), "researchx-web-adc-"));
	t.after(() => {
