...
test("science database tool searches ResearchX-owned cBioPortal studies and details", async () => {
	...
	const search = await tools.get("researchx_science_database_search")?.execute("call-cbio-search", {
		source: "cbioportal",
		query: "melanoma",
		limit: 2,
	});
	const detail = await tools.get("researchx_science_database_search")?.execute("call-cbio-detail", {
		source: "cbioportal",
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /cBioPortal/);
});
...
test("science database tool fetches bounded cBioPortal gene mutation rows", async () => {
	...
	const tools = registerTools();
	const result = await tools.get("researchx_science_database_search")?.execute("call-cbio-mutations", {
		source: "cbioportal",
...
test("science database tool exposes cBioPortal reference cancer-model frequency and CNA modes", async () => {
	...
	const tools = registerTools();
	const frequency = await tools.get("researchx_science_database_search")?.execute("call-cbio-frequency", {
		source: "cbioportal",
		query: "cbioportal_mutation_frequency:KRAS@paad_qcmg_uq_2016,msk_impact_2017,unknown_study",
	});
	const cna = await tools.get("researchx_science_database_search")?.execute("call-cbio-cna", {
		source: "cbioportal",
