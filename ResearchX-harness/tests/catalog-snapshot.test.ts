...
test("buildModelStatusSnapshotFromRecords emits guidance when no models are available", () => {
	...
	assert.ok(snapshot.guidance.some((line) => line.includes("researchx model login <provider>")));
	assert.ok(snapshot.guidance.some((line) => line.includes("researchx model list")));
	assert.ok(snapshot.guidance.every((line) => !line.includes("researchx setup model")));
});
...
test("buildModelStatusSnapshotFromRecords emits guidance when no default model is set", () => {
	...
	assert.ok(snapshot.guidance.some((line) => line.includes("researchx model set <provider/model>")));
	assert.ok(snapshot.guidance.some((line) => line.includes("researchx model list")));
	assert.ok(snapshot.guidance.every((line) => !line.includes("researchx setup model")));
});
...
test("DeepSeek V4 Pro IDs remain available while genuine premium Pro models stay blocked", () => {
	...
	const previousBypass = process.env.RESEARCHX_ALLOW_PRO_MODELS;
	try {
		process.env.RESEARCHX_ALLOW_PRO_MODELS = "google/gemini-3.1-pro";
		assert.equal(isProClassModelSpec("google/gemini-3.1-pro"), true);
	} finally {
		if (previousBypass === undefined) delete process.env.RESEARCHX_ALLOW_PRO_MODELS;
		else process.env.RESEARCHX_ALLOW_PRO_MODELS = previousBypass;
	}
