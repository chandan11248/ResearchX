...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-endpoint-"));
...
test("workbench promotes saved model endpoint artifacts into compute and execution history", () => {
	...
	try {
		...
		assert.match(computeJob?.command ?? "", /researchx_model_endpoint_call --provider nvidia-bionemo --model esmfold/);
		...
		assert.equal(managed?.skillName, "researchx_model_endpoint_call");
		...
		assert.equal(managed?.registeredBy, "researchx-runtime-context");
