...
function makeWorkspace(): { authPath: string; root: string } {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-setup-decisions-"));
...
test("workbench state exposes contact email and credential ask decision rows", () => {
	...
	try {
		...
		assert.equal(contact?.noticeVersion, "researchx-contact-email-v1");
		assert.match(contact?.noticeText ?? "", /ResearchX Bio Tools/);
