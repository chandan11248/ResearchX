...
test("ensureSupportedNodeVersion throws a guided upgrade message", () => {
	assert.throws(
		...
		(error: unknown) =>
			...
			error.message.includes("github.com/chandan11248/ResearchX-harness/releases"),
	);
});
...
test("unsupported version guidance reports the detected version", () => {
	...
	assert.equal(lines[0], `researchx supports Node.js ${MIN_NODE_VERSION} through ${MAX_NODE_MAJOR}.x (detected 18.17.0).`);
	assert.ok(lines.some((line) => line.includes("github.com/chandan11248/ResearchX-harness/releases")));
});
...
test("unsupported version guidance explains upper-bound failures", () => {
	...
	assert.equal(lines[0], `researchx supports Node.js ${MIN_NODE_VERSION} through ${MAX_NODE_MAJOR}.x (detected ${tooNewVersion}).`);
