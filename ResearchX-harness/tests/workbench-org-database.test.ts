...
import { getResearchXOrgDatabasePath } from "../src/config/paths.js";
...
function emptyState(partial: Partial<WorkbenchState>): WorkbenchState {
	return {
		workspacePath: "/tmp/researchx-workspace",
		workspaceName: "researchx-workspace",
...
test("materializeWorkbenchOrgDatabase writes ResearchX-owned reference-shaped tables", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-org-db-"));
	try {
		withResearchXHome(join(root, "home-parent"), () => {
			...
			const state = emptyState({
				...
				computeProviders: [{
					...
					probedAt: now,
					probedAtMs: nowMs,
					egressPolicy: "ResearchX Settings Network policy",
				...
				directoryAttachments: [{
					...
					section: "Featured",
					excludedTools: [],
					toolNames: ["researchx_science_database_search"],
					createdAt: now,
					createdAtMs: nowMs,
				}],
				mcpToolGrants: [{
			...
			assert.match(summary.path, /\/\.researchx\/orgs\/[0-9a-f-]{36}\/researchx-workbench\.db$/);
			...
			try {
				...
				assert.deepEqual(visibleTableNames.filter((name) => !REFERENCE_LEDGER_TABLE_NAMES.includes(name)), [
					...
					"notes",
					"projects",
					"researchx_state_payloads",
					"routine_schedules",
					"verification_checks",
				]);
				assert.equal((database.prepare("SELECT value FROM __researchx_mirror_meta WHERE key = 'schema_version'").get() as { value: string }).value, "1");
				...
				assert.equal(localProviderPolicy.egress_policy, "ResearchX Settings Network policy");
				...
				assert.equal((database.prepare("SELECT server_id FROM mcp_tool_grants WHERE id = 'grant-1'").get() as { server_id: string }).server_id, "researchx-bio-tools");
				...
				assert.equal((database.prepare("SELECT enabled FROM capability_settings WHERE id = 'local:connector:researchx-bio-tools'").get() as { enabled: number }).enabled, 1);
				...
				assert.match((database.prepare("SELECT payload_json FROM researchx_state_payloads WHERE table_name = 'artifacts' AND row_id = 'outputs/report.md'").get() as { payload_json: string }).payload_json, /"starred":true/);
			} finally {
				database.close();
			}
		});
...
test("built workbench state can refresh the org database on demand", () => {
	...
	try {
		...
		withResearchXHome(join(root, "home-parent"), () => {
			...
			try {
				assert.equal((database.prepare("SELECT value FROM __researchx_mirror_meta WHERE key = 'workspace_path'").get() as { value: string }).value, state.workspacePath);
...
test("materializeWorkbenchOrgDatabase upgrades existing compute provider policy columns", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-org-db-upgrade-"));
	const path = join(root, "existing-researchx-workbench.db");
...
`);
	...
	try {
		...
		try {
			...
			assert.equal(modalProviderPolicy.egress_policy, "ResearchX Settings Network policy");
