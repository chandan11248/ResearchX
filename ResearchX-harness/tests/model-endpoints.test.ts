...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-model-endpoints-"));
...
test("researchx_model_endpoint_call runs hosted ESMFold with NVIDIA auth and writes PDB artifacts", async () => {
	...
	try {
		...
		const tool = registerTools().get("researchx_model_endpoint_call");
		...
		assert.equal(tool?.name, "researchx_model_endpoint_call");
