...
test("science database tool searches PubMed through ESearch and ESummary", async () => {
	...
	const result = await tools.get("researchx_science_database_search")?.execute("call-1", {
		source: "pubmed",
		query: "CRISPR cancer",
		limit: 2,
		sort: "pub_date",
	...
	assert.equal(tools.has("researchx_science_database_search"), true);
	...
	assert.equal(new URL(requests[0]!).searchParams.get("tool"), "researchx");
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /PubMed/);
});
...
test("science database tool searches ClinicalTrials.gov and ChEMBL", async () => {
	...
	const tools = registerTools();
	const trials = await tools.get("researchx_science_database_search")?.execute("call-2", {
		source: "clinicaltrials",
		query: "glioblastoma targeted therapy",
		limit: 1,
	});
	const chembl = await tools.get("researchx_science_database_search")?.execute("call-3", {
		source: "chembl",
		query: "HER2",
		chemblEntity: "target",
		limit: 1,
...
test("science database tool searches public literature and DOI metadata sources", async () => {
	...
	const tools = registerTools();
	const crossref = await tools.get("researchx_science_database_search")?.execute("call-crossref", {
		source: "crossref",
		query: "crispr metadata",
		limit: 1,
	});
	const europepmc = await tools.get("researchx_science_database_search")?.execute("call-epmc", {
		source: "europepmc",
		query: "crispr",
		limit: 1,
	});
	const arxiv = await tools.get("researchx_science_database_search")?.execute("call-arxiv", {
		source: "arxiv",
		query: "crispr",
		limit: 1,
		sort: "pub_date",
	});
	const datacite = await tools.get("researchx_science_database_search")?.execute("call-datacite", {
		source: "datacite",
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /Europe PMC/);
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /DataCite/);
});
...
test("science database tool searches specialty protein structure gene and preprint sources", async () => {
	...
	const tools = registerTools();
	const uniprot = await tools.get("researchx_science_database_search")?.execute("call-uniprot", {
		source: "uniprot",
		query: "gene_exact:BRCA1 AND organism_id:9606",
		limit: 1,
	});
	const pdb = await tools.get("researchx_science_database_search")?.execute("call-pdb", {
		source: "pdb",
		query: "hemoglobin",
		limit: 1,
	});
	const ensembl = await tools.get("researchx_science_database_search")?.execute("call-ensembl", {
		source: "ensembl",
		query: "BRCA2",
		ensemblSpecies: "homo_sapiens",
		limit: 1,
	});
	const biorxiv = await tools.get("researchx_science_database_search")?.execute("call-biorxiv", {
		source: "biorxiv",
		query: "kinase",
		limit: 1,
	});
	const medrxiv = await tools.get("researchx_science_database_search")?.execute("call-medrxiv", {
		source: "medrxiv",
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /UniProt/);
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /RCSB PDB/);
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /Ensembl/);
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /bioRxiv/);
});

test("science database tool searches ResearchX-owned clinical variant databases", async () => {
	...
	const tools = registerTools();
	const clinvar = await tools.get("researchx_science_database_search")?.execute("call-clinvar", {
		source: "clinvar",
		query: "APOE rs7412",
		limit: 1,
	});
	const dbsnp = await tools.get("researchx_science_database_search")?.execute("call-dbsnp", {
		source: "dbsnp",
		query: "rs7412",
		limit: 1,
	});
	const cadd = await tools.get("researchx_science_database_search")?.execute("call-cadd", {
		source: "cadd",
		query: "GRCh38-v1.7 19-44908822-C-T",
		limit: 1,
	});
	const variation = await tools.get("researchx_science_database_search")?.execute("call-variation", {
		source: "variation",
		query: "NM_000518.4:c.27dupG",
		limit: 1,
	});
	const variationSpdiFallback = await tools.get("researchx_science_database_search")?.execute("call-variation-spdi", {
		source: "variation",
		query: "NC_000001.10:12345:0:C",
		limit: 1,
	});
	const variationBatch = await tools.get("researchx_science_database_search")?.execute("call-variation-batch", {
		source: "variation",
	...
	assert.equal(new URL(requests[0]!.url).searchParams.get("tool"), "researchx");
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /ClinVar/);
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /dbSNP/);
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /CADD/);
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /NCBI Variation Services/);
});

test("science database tool searches ResearchX-owned specialty bio databases", async () => {
	const requests: Array<{ body?: string; method: string; url: string }> = [];
	globalThis.fetch = async (input, init) => {
		const url = String(input);
		requests.push({ url, method: init?.method ?? "GET", body: typeof init?.body === "string" ? init.body : undefined });
	...
	const tools = registerTools();
	const encode = await tools.get("researchx_science_database_search")?.execute("call-encode", {
		source: "encode",
		query: "CTCF K562",
		limit: 1,
	});
	const geo = await tools.get("researchx_science_database_search")?.execute("call-geo", {
		source: "geo",
		query: "single cell tumor",
		limit: 1,
	});
	const gtex = await tools.get("researchx_science_database_search")?.execute("call-gtex", {
		source: "gtex",
		query: "BRCA1",
		limit: 1,
	});
	const gnomadSearch = await tools.get("researchx_science_database_search")?.execute("call-gnomad-search", {
		source: "gnomad",
		query: "rs7412",
		limit: 1,
	});
	const gnomadVariant = await tools.get("researchx_science_database_search")?.execute("call-gnomad-variant", {
		source: "gnomad",
		query: "19-44908822-C-T",
		limit: 1,
	});
	const gnomadConstraint = await tools.get("researchx_science_database_search")?.execute("call-gnomad-constraint", {
		source: "gnomad",
		query: "constraint:TP53",
		limit: 1,
	});
	const gnomadStructuralGene = await tools.get("researchx_science_database_search")?.execute("call-gnomad-structural-gene", {
		source: "gnomad",
		query: "sv:TP53",
		limit: 1,
	});
	const gnomadStructuralVariant = await tools.get("researchx_science_database_search")?.execute("call-gnomad-structural-variant", {
		source: "gnomad",
		query: "sv-id:DEL_chr17_599b1512",
		limit: 1,
	});
	const gnomadMitoGene = await tools.get("researchx_science_database_search")?.execute("call-gnomad-mito-gene", {
		source: "gnomad",
		query: "mito:MT-TL1",
		limit: 1,
	});
	const gnomadMitoRegion = await tools.get("researchx_science_database_search")?.execute("call-gnomad-mito-region", {
		source: "gnomad",
		query: "mito:1-200",
		limit: 1,
	});
	const gnomadMitoVariant = await tools.get("researchx_science_database_search")?.execute("call-gnomad-mito-variant", {
		source: "gnomad",
		query: "M-3243-A-G",
		limit: 1,
	});
	const interpro = await tools.get("researchx_science_database_search")?.execute("call-interpro", {
		source: "interpro",
		query: "kringle",
		limit: 1,
	});
	const ols = await tools.get("researchx_science_database_search")?.execute("call-ols", {
		source: "ols",
		query: "DNA repair",
		limit: 1,
	});
	const pride = await tools.get("researchx_science_database_search")?.execute("call-pride", {
		source: "pride",
		query: "proteome",
		limit: 1,
	});
	const quickgo = await tools.get("researchx_science_database_search")?.execute("call-quickgo", {
		source: "quickgo",
		query: "P38398",
		limit: 1,
	});
	const reactome = await tools.get("researchx_science_database_search")?.execute("call-reactome", {
		source: "reactome",
		query: "BRCA1 TP53",
		limit: 1,
	});
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /Reactome/);
});
