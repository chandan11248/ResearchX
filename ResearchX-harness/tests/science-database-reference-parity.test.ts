...
test("science database tool resolves PubChem compounds with properties and synonyms", async () => {
	...
	const result = await tools.get("researchx_science_database_search")?.execute("call-pubchem", {
		source: "pubchem",
		query: "aspirin",
		limit: 1,
	});
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /PubChem/);
	assert.equal(requests.some((request) => request.url.includes("tool=researchx")), true);
});
...
test("science database tool accepts PubChem reference chemistry query names", async () => {
	...
	const tool = registerTools().get("researchx_science_database_search");
...
test("science database tool searches BindingDB and STRING reference endpoints", async () => {
	...
	const tools = registerTools();
	const binding = await tools.get("researchx_science_database_search")?.execute("call-bindingdb", {
		source: "bindingdb",
		query: "uniprot:P35355 cutoff=100",
		limit: 2,
	});
	const string = await tools.get("researchx_science_database_search")?.execute("call-string", {
		source: "string",
...
test("science database tool searches KEGG, Rhea, and Rfam reference endpoints", async () => {
	...
	const tools = registerTools();
	const kegg = await tools.get("researchx_science_database_search")?.execute("call-kegg", {
		source: "kegg",
		query: "find:compound aspirin",
		limit: 1,
	});
	const rhea = await tools.get("researchx_science_database_search")?.execute("call-rhea", {
		source: "rhea",
		query: "rhea_search_reactions:glucose",
		limit: 1,
	});
	const rfam = await tools.get("researchx_science_database_search")?.execute("call-rfam", {
		source: "rfam",
		query: "RF00005",
		limit: 1,
	});
