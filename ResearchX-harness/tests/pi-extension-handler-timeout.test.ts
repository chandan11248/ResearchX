...
function restoreOfficialRunnerSource(source: string): string {
	...
	return restored
		.split("await runResearchXExtensionHandler(handler, currentEvent, ctx)")
		.join("await handler(currentEvent, ctx)")
		.split("await runResearchXExtensionHandler(handler, event, ctx)")
...
async function loadFastPatchedRunner(
	...
	const root = mkdtempSync(join(tmpdir(), "researchx-pi-extension-timeout-"));
	...
	source = source
		...
		.replace(
			...
			`const RESEARCHX_PI_EXTENSION_HANDLER_TIMEOUT_MS = ${timeoutMs};`,
		);
...
test("Pi 0.84.2 extension timeout patch is exact, idempotent, and fail closed", () => {
	...
	assert.equal(
		once.match(/runResearchXExtensionHandler\(handler, (?:currentEvent|event), ctx\)/g)?.length,
		12,
	);
...
test("a hung handler aborts scoped work, reports safely, and cannot starve later handlers", async (t) => {
	...
	assert.doesNotMatch(errors[0]?.error ?? "", /private|prompt|system|researchx/i);
