...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-read-cursors-"));
...
test("workbench frame read cursors persist and appear in state", () => {
	...
	try {
		...
		assert.match(readFileSync(storePath, "utf8"), /researchx\.workbenchReadCursors\.v1/);
		assert.equal(existsSync(join(root, ".researchx", "workbench", "read-cursors.json")), false);
