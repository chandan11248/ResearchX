...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-user-agents-"));
	...
	writeFileSync(join(root, ".researchx", "SYSTEM.md"), [
		"You are ResearchX, a general-purpose AI assistant.",
		"",
...
test("buildWorkbenchState exposes Claude-style ResearchX user agent rows", () => {
	...
	try {
		...
		assert.equal(agent?.name, "RESEARCHX");
		assert.equal(agent?.displayName, "ResearchX");
		assert.equal(agent?.description, "Default ResearchX general-purpose agent profile");
		assert.match(agent?.systemPrompt ?? "", /general-purpose AI assistant/);
		assert.equal(agent?.iconKey, "lightning");
		assert.equal(agent?.colorKey, "researchx-green");
...
test("workbench server returns ResearchX user agents through state", async () => {
	const root = makeWorkspace();
	...
	try {
		...
		assert.equal(payload.userAgents[0]?.name, "RESEARCHX");
		assert.equal(payload.userAgents[0]?.displayName, "ResearchX");
