...
test("researchx_workbench_context returns sanitized configured resources", async () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-context-tool-"));
	...
	try {
		...
		assert.equal(tool?.name, "researchx_workbench_context");
		...
		assert.match(text, /researchx_model_endpoint_call/);
