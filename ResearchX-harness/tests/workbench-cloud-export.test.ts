...
function makeWorkspace(): { exportRoot: string; root: string } {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-cloud-export-"));
	const exportRoot = mkdtempSync(join(tmpdir(), "researchx-workbench-cloud-target-"));
	mkdirSync(join(root, "outputs"), { recursive: true });
	writeFileSync(join(root, "outputs", "alpha.md"), "# Alpha\n\nExport me.\n");
	writeFileSync(workbenchDataPath(root, "settings.json"), JSON.stringify({
		schema: "researchx.workbenchSettings.v1",
		credentialRefs: [{
			id: "local-export",
			name: "Local export target",
			provider: "local",
			envVar: "RESEARCHX_TEST_EXPORT_TARGET",
			...
			provider: "s3",
			envVar: "RESEARCHX_TEST_MISSING_EXPORT_TARGET",
			createdAt: "2026-07-01T00:00:00.000Z",
			updatedAt: "2026-07-01T00:00:00.000Z",
		}],
		updatedAt: "2026-07-01T00:00:00.000Z",
...
test("workbench state exposes configured credential-backed cloud export targets", () => {
	...
	const original = process.env.RESEARCHX_TEST_EXPORT_TARGET;
	process.env.RESEARCHX_TEST_EXPORT_TARGET = `file://${exportRoot}`;
	try {
		...
		assert.equal(localCredential?.encryptedCredentials, "researchx-env-ref:RESEARCHX_TEST_EXPORT_TARGET");
		...
		if (original === undefined) {
			delete process.env.RESEARCHX_TEST_EXPORT_TARGET;
		} else {
			process.env.RESEARCHX_TEST_EXPORT_TARGET = original;
		}
...
test("workbench server exports an artifact through the authenticated cloud export API", async () => {
	...
	try {
		...
		assert.equal(existsSync(join(root, ".researchx", "workbench", "cloud-exports.jsonl")), false);
