...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-mcp-tool-grants-"));
...
function seedGrants(root: string): void {
	...
	upsertWorkbenchSettingsRecord(root, {
		...
		record: {
			...
			scope: "builtin:researchx_science_database_search",
			decision: "ask",
		},
	});
	upsertWorkbenchSettingsRecord(root, {
...
test("buildWorkbenchState exposes Claude-style MCP tool grant rows", () => {
	...
	try {
		...
		assert.equal(builtinGrant?.toolName, "researchx_science_database_search");
...
test("workbench server returns MCP tool grants through state", async () => {
	...
	try {
		...
		assert.deepEqual(
			...
			[
				"builtin:researchx_science_database_search:ask",
