...
test("Pi 0.84.2 correctness patch is applied, idempotent, and documents its removal condition", () => {
	...
	assert.match(agentSessionSource, /_researchxEagerlyPersistedToolResults/);
	assert.match(agentSessionSource, /researchxToolResultIdBeforeExtensions/);
	...
	assert.match(sessionManagerSource, /restoreResearchXToolResultsInSourceOrder/);
	...
	assert.match(transformMessagesSource, /flushResearchXToolResults/);
	assert.match(nestedTransformMessagesSource, /order eager tool results/);
	assert.match(nestedTransformMessagesSource, /flushResearchXToolResults/);
	for (const source of [
	...
	const reorderedAgentSession = agentSessionSource
		.replace(appendFragment, "__RESEARCHX_APPEND__")
		.replace(extensionFragment, appendFragment)
		.replace("__RESEARCHX_APPEND__", extensionFragment);
	assert.throws(
...
test("Pi 0.84.2 correctness patch migrates the pre-review eager persistence layout", () => {
	...
	const currentBoundary = `        const researchxToolResultIdBeforeExtensions = event.type === "message_end" && event.message.role === "toolResult"
...
`;
	const currentLookup = `                const eagerToolCallId = researchxToolResultIdBeforeExtensions ?? event.message.toolCallId;
	...
	const legacyLookup = `                const eagerlyPersisted = this._researchxEagerlyPersistedToolResults.get(event.message.toolCallId);
	...
	assert.doesNotMatch(legacy, /researchxToolResultIdBeforeExtensions/);
	assert.throws(
...
test("package artifact verification rejects a mixed Pi runtime train", () => {
	const packageRoot = mkdtempSync(resolve(tmpdir(), "researchx-mixed-pi-artifact-"));
	try {
...
test("interleaved research text and images retain order for idle and queued delivery", async (t) => {
	...
	const tempRoot = mkdtempSync(resolve(tmpdir(), "researchx-pi-8615-"));
...
test("completed parallel results persist before a slow sibling and restore in tool-call order", async (t) => {
	...
	const tempRoot = mkdtempSync(resolve(tmpdir(), "researchx-pi-7053-"));
	const faux = registerFauxProvider();
...
test("provider transformation orders eager results and synthesizes only unresolved calls", async () => {
	const moduleUrl = `${pathToFileURL(nestedTransformMessagesPath).href}?researchx-7053`;
