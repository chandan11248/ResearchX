...
test("Europe PMC full-text mode fetches OA JATS sections through the database tool", async () => {
	...
	const result = await tools.get("researchx_science_database_search")?.execute("call-fulltext", {
		source: "europepmc",
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /full-text sections/);
	assert.match(tools.get("researchx_science_database_search")?.promptGuidelines?.join("\n") ?? "", /section inventories/);
});
...
test("Europe PMC full-text mode reports not-open-access records without fetching XML", async () => {
	...
	const tools = registerTools();
	const result = await tools.get("researchx_science_database_search")?.execute("call-fulltext-pmid", {
		source: "europepmc",
		query: "pmid:29456894",
		limit: 1,
	});
...
test("Europe PMC full-text mode returns invalid ID status records", async () => {
	const tools = registerTools();
	const result = await tools.get("researchx_science_database_search")?.execute("call-fulltext-invalid", {
		source: "europepmc",
