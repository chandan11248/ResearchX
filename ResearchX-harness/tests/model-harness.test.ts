...
function createAuthPath(contents: Record<string, unknown>): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-auth-"));
...
test("researchx alpha reaches Alpha Hub help when cwd is supplied before alpha", () => {
	const workingDir = mkdtempSync(join(tmpdir(), "researchx-alpha-cwd-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-alpha-home-"));
...
test("unknown CLI flags point users to ResearchX help", () => {
	const workingDir = mkdtempSync(join(tmpdir(), "researchx-unknown-option-cwd-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-unknown-option-home-"));
	...
	assert.match(`${result.stdout}\n${result.stderr}`, /Run `researchx help` to see available commands and flags/);
});

test("packages CLI hides removed UI and bulk extras", () => {
	const workingDir = mkdtempSync(join(tmpdir(), "researchx-packages-cwd-"));
	const homeDir = mkdtempSync(join(tmpdir(), "researchx-packages-home-"));
	const env = {
		...process.env,
		RESEARCHX_HOME: homeDir,
		RESEARCHX_TELEMETRY: "0",
...
test("setDefaultModelSpec accepts a unique bare model id from authenticated models", async () => {
	...
	const settingsPath = join(mkdtempSync(join(tmpdir(), "researchx-settings-")), "settings.json");
...
test("buildLocalModelWorkflowNotice names the configured model and the workflow", () => {
	...
	assert.ok(notice.includes("researchx model set"));
});
