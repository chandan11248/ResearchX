...
test("Pi command discovery normalizes live RPC commands into resources", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-command-resources-"));
	try {
			...
			commands: [
				{
					...
					sourceInfo: {
						...
						source: "@companion-ai/researchx-research",
						scope: "project",
						origin: "package",
		...
		assert.equal(group.resources[1]?.detail, "project / package / @companion-ai/researchx-research");
		assert.deepEqual(group.resources[1]?.tags, [
			"live",
			"skill",
			"project",
			"package",
			"@companion-ai/researchx-research",
		]);
