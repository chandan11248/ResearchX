...
test("vendored runtime uses a committed exact dependency lock", () => {
	const settings = JSON.parse(
		readFileSync(join(repoRoot, ".researchx", "settings.json"), "utf8"),
	) as { packages: string[] };
	...
	const runtimeLock = JSON.parse(
		readFileSync(
			join(repoRoot, ".researchx", "runtime-package-lock.json"),
			"utf8",
		),
	) as {
...
test("workspace setup locks preserve active owners and release only matching tokens", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-runtime-lock-"));
...
test("setup lock heartbeats preserve a live owner when process-start lookup is unavailable", async () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-runtime-heartbeat-"));
	const lockDir = join(root, ".workspace-setup.lock");
...
test("setup lock acquisition bounds cleanup of crashed ownership tombstones", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-runtime-lock-tombstones-"));
	const lockDir = join(root, ".workspace-setup.lock");
	const now = Date.now();
	try {
		const oldPaths = [
