...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-compute-"));
	...
	try {
		const executePromise = fetch(`${handle.url}api/notebook/execute`, {
			...
			body: JSON.stringify({
				...
				purpose: "exploration",
				code: "mkdir -p .researchx\nif [ -f .researchx/retry-ready ]; then printf 'retried\\n'; else touch .researchx/retry-ready; sleep 30; fi",
			}),
		});
		...
		await waitFor(() => existsSync(join(root, ".researchx", "retry-ready")), "retry marker");
...
test("canceling an active Modal notebook job records a pending terminate entry", async () => {
	const root = makeWorkspace();
	const previousModalCli = process.env.RESEARCHX_MODAL_CLI;
	try {
		...
		process.env.RESEARCHX_MODAL_CLI = fakeModal;
		...
		if (previousModalCli === undefined) delete process.env.RESEARCHX_MODAL_CLI;
		else process.env.RESEARCHX_MODAL_CLI = previousModalCli;
		rmSync(root, { recursive: true, force: true });
	}
});
