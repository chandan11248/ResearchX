...
test("external fetched-content cache follows ResearchX's exact web-search config path", async () => {
	const fixtureRoot = mkdtempSync(join(tmpdir(), "researchx-web-content-cache-"));
	...
	const originalConfigPath = process.env.RESEARCHX_WEB_SEARCH_CONFIG;
	...
	try {
		...
		const responseId = "researchx-cache-proof";
		...
		if (originalConfigPath === undefined) {
			...
			process.env.RESEARCHX_WEB_SEARCH_CONFIG = originalConfigPath;
		}
...
test("fetched-content cache evicts oldest entries and removes only stale owned temp files", async () => {
	const fixtureRoot = mkdtempSync(join(tmpdir(), "researchx-web-cache-limits-"));
...
test("fetched-content cache rejects directory and entry symlinks", { skip: process.platform === "win32" }, async () => {
	const fixtureRoot = mkdtempSync(join(tmpdir(), "researchx-web-cache-symlink-"));
	const configPath = join(fixtureRoot, "config", "web-search.json");
	const cacheDir = join(fixtureRoot, "config", "web-search-cache");
