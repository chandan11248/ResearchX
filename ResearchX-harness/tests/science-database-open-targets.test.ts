...
test("science database tool searches ResearchX-owned Open Targets records", async () => {
	...
	const search = await tools.get("researchx_science_database_search")?.execute("call-ot-search", {
		source: "opentargets",
		query: "BRAF melanoma",
		limit: 3,
	});
	const target = await tools.get("researchx_science_database_search")?.execute("call-ot-target", {
		source: "opentargets",
		query: "target:ENSG00000157764",
		limit: 3,
	});
	const drug = await tools.get("researchx_science_database_search")?.execute("call-ot-drug", {
		source: "opentargets",
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /Open Targets/);
});
...
test("science database tool fetches Open Targets disease and evidence rows", async () => {
	...
	const tools = registerTools();
	const disease = await tools.get("researchx_science_database_search")?.execute("call-ot-disease", {
		source: "opentargets",
		query: "disease-targets:MONDO_0004992",
		limit: 2,
	});
	const evidence = await tools.get("researchx_science_database_search")?.execute("call-ot-evidence", {
		source: "opentargets",
