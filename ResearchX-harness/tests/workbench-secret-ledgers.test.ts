...
function makeWorkspace(): { authPath: string; root: string } {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-secret-ledgers-"));
...
test("workbench state exposes Claude-style redacted user secret and Anthropic key ledgers", () => {
	...
	try {
		...
		assert.equal(authAnthropic?.encryptedValue, "researchx-auth-storage-ref:auth:anthropic");
		...
		assert.equal(openAiEnv?.encryptedValue, "researchx-environment-ref:env:openai:OPENAI_API_KEY");
