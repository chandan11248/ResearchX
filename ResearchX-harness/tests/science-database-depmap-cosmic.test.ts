...
test("science database tool searches ResearchX-owned DepMap model and dependency records", async () => {
	...
	const search = await tools.get("researchx_science_database_search")?.execute("call-depmap-search", {
		source: "depmap",
		query: "SNU-1033",
		limit: 2,
	});
	const dependency = await tools.get("researchx_science_database_search")?.execute("call-depmap-dependency", {
		source: "depmap",
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /DepMap/);
});
...
test("science database tool accepts DepMap reference cancer-model query names", async () => {
	...
	const tools = registerTools();
	const getModel = await tools.get("researchx_science_database_search")?.execute("depmap-get-model", {
		source: "depmap",
		query: "get_model:SIDM00192",
	});
	const listModels = await tools.get("researchx_science_database_search")?.execute("depmap-list-models", {
		source: "depmap",
		query: "list_models:tissue=Large Intestine;cancer_type=Colorectal Carcinoma",
	});
	const searchModels = await tools.get("researchx_science_database_search")?.execute("depmap-search-models", {
		source: "depmap",
		query: "search_models:SNU",
	});
	const searchGenes = await tools.get("researchx_science_database_search")?.execute("depmap-search-genes", {
		source: "depmap",
		query: "search_genes:BRA",
	});
	const dependencies = await tools.get("researchx_science_database_search")?.execute("depmap-gene-dependencies", {
		source: "depmap",
...
test("science database tool fetches bounded COSMIC mutation records through NLM Clinical Tables", async () => {
	...
	const result = await registerTools().get("researchx_science_database_search")?.execute("call-cosmic", {
		source: "cosmic",
	...
	assert.match(registerTools().get("researchx_science_database_search")?.promptSnippet ?? "", /COSMIC/);
});
