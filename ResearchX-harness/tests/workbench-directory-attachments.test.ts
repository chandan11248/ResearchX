...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-directory-attachments-"));
...
test("buildWorkbenchState exposes ResearchX-owned directory attachment rows", () => {
	...
	try {
		...
		const pubmed = state.directoryAttachments.find((attachment) =>
			attachment.connectorName === "PubMed" && attachment.agentName === "researchx"
		);
		...
		assert.equal(pubmed?.toolNames.includes("researchx_science_database_search"), true);

		const bioTools = state.directoryAttachments.find((attachment) => attachment.connectorId === "researchx-bio-tools");
		assert.equal(bioTools?.agentName, "researchx");
		assert.equal(bioTools?.connectorKind, "featured");
		assert.equal(bioTools?.source, "ResearchX built-in science tools");
		...
		for (const [connectorId, connectorName] of [
			...
			assert.equal(attachment?.toolNames.includes("researchx_science_database_search"), true, `expected ResearchX database tool on ${connectorId}`);
		}
...
test("workbench server returns directory attachments through state", async () => {
	...
	try {
		const response = await fetch(`${handle.url}api/state`, {
			headers: { cookie: "researchx_workbench=test-token" },
		});
