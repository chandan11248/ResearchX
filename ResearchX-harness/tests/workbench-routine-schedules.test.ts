...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-routine-schedules-"));
...
test("buildWorkbenchState exposes Claude-style routine schedule rows for watch plans", () => {
	...
	try {
		...
		assert.equal(onTick.schema, "researchx.routineTick.v1");
...
test("workbench server returns routine schedules through state", async () => {
	...
	try {
		...
		assert.match(payload.routineSchedules[0]?.onTick ?? "", /researchx\.routineTick\.v1/);
