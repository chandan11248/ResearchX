...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-r-"));
...
function writeFakeR(root: string): string {
	...
	writeFileSync(fakeR, [
		...
		"    const marker = line.match(/(__RESEARCHX_NOTEBOOK_[A-Za-z0-9_]+_DONE__):/);",
...
test("notebook isolated R execution uses configured Rscript runtime", async () => {
	const root = makeWorkspace();
	const previousRscript = process.env.RESEARCHX_RSCRIPT;
	try {
		const fakeRscript = writeFakeRscript(root);
		process.env.RESEARCHX_RSCRIPT = fakeRscript;
		...
		if (previousRscript === undefined) delete process.env.RESEARCHX_RSCRIPT;
		else process.env.RESEARCHX_RSCRIPT = previousRscript;
		rmSync(root, { recursive: true, force: true });
	}
});
