...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-custom-mcp-ledgers-"));
...
test("buildWorkbenchState exposes Claude-style custom MCP server and assignment rows", () => {
	...
	try {
		...
		assert.deepEqual(localAssignments.map((assignment) => assignment.agentName), ["researchx"]);
