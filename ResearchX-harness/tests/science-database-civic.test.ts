...
test("science database tool searches ResearchX-owned CIViC cancer curation records", async () => {
	...
	const civic = await tools.get("researchx_science_database_search")?.execute("call-civic", {
		source: "civic",
		query: "BRAF V600E",
		limit: 1,
	});
	const civicGene = await tools.get("researchx_science_database_search")?.execute("call-civic-gene", {
		source: "civic",
		query: "gene:BRAF",
		limit: 1,
	});
	...
	assert.match(tools.get("researchx_science_database_search")?.promptSnippet ?? "", /CIViC/);
});
