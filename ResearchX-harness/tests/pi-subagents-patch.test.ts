...
for (const scenario of CASES) {
	test(`patchPiSubagentsSource rewrites ${scenario.name}`, () => {
		...
		assert.match(patched, /process\.env\.RESEARCHX_CODING_AGENT_DIR\?\.trim\(\) \|\| process\.env\.PI_CODING_AGENT_DIR\?\.trim\(\)/);
...
test("installed pi-subagents verifier launches mock Pi scripts through Node on every platform", () => {
	...
	assert.match(source, /process\.env\.RESEARCHX_PI_CLI_PATH = mockPath/);
...
test("patchPiSubagentsSource registers split prompt metadata by default", () => {
	...
	assert.match(patchedDescription, /researchx-pi-subagents-prompt-metadata-v2/);
	assert.match(patchedDescription, /Delegate to configured research subagents/);
	assert.match(patchedDescription, /run `researchx model list` first/);
	...
	assert.throws(
		() => patchPiSubagentsSource(
			"src/extension/tool-description.ts",
			`${patchedDescription}\n// researchx-pi-subagents-prompt-metadata-v1`,
		),
...
test("patchPiSubagentsSource upgrades v1 prompt metadata with approved model guidance", () => {
	...
	assert.match(patched, /researchx-pi-subagents-prompt-metadata-v2/);
	assert.doesNotMatch(patched, /researchx-pi-subagents-prompt-metadata-v1/);
	assert.match(patched, /run `researchx model list` first/);
	assert.equal(
...
test("patchPiSubagentsSource makes pi-spawn prefer the real Pi CLI over ResearchX wrapper", () => {
	const input = [
	...
	assert.match(patched, /process\.env\.RESEARCHX_PI_CLI_PATH/);
...
test("patchPiSubagentsSource preserves the exact ResearchX Pi path in the current package resolver", () => {
	const input = [
		'import * as fs from "node:fs";',
		'import * as path from "node:path";',
	...
	assert.match(patched, /env\.RESEARCHX_PI_CLI_PATH\?\.trim\(\)/);
...
test("patchPiSubagentsSource upgrades old ResearchX pi-spawn patch to derive cli.js from wrapper main arg", () => {
	const input = [
		"export interface PiSpawnDeps {",
		"\texecPath?: string;",
		"\targv1?: string;",
		...
		'\tconst readFileSync = deps.readFileSync ?? ((filePath, encoding) => fs.readFileSync(filePath, encoding));',
		"\tconst argv1 = deps.argv1 ?? process.argv[1];",
		"\tconst researchxPiCliPath = process.env.RESEARCHX_PI_CLI_PATH;",
		"\tif (researchxPiCliPath) {",
		"\t\tconst cliPath = normalizePath(researchxPiCliPath);",
