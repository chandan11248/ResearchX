...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-connectors-"));
	return root;
}
...
test("researchx connector tools discover and call allowed Streamable HTTP MCP tools", async () => {
...
test("researchx connector tools send stored OAuth bearer tokens to remote MCP servers", async () => {
	const root = makeWorkspace();
	const mcp = await startMcpServer();
	try {
		upsertWorkbenchSettingsRecord(root, {
...
test("researchx connector tools discover and call allowed SSE MCP tools", async () => {
	const root = makeWorkspace();
...
test("researchx connector tools discover and call allowed local stdio MCP tools", async () => {
	const root = makeWorkspace();
	try {
