...
test("loadPiWebAccessConfig returns empty config when Pi web config is missing", () => {
	const root = mkdtempSync(join(tmpdir(), "researchx-pi-web-"));
	const configPath = getPiWebSearchConfigPath(root);

	assert.deepEqual(loadPiWebAccessConfig(configPath), {});
});

test("getPiWebSearchConfigPath respects RESEARCHX_HOME semantics", () => {
	assert.equal(getPiWebSearchConfigPath("/tmp/custom-home"), "/tmp/custom-home/.researchx/web-search.json");
});

test("getPiWebSearchConfigPath honors an exact custom ResearchX config path", () => {
	...
	try {
		...
		assert.equal(
			...
			"/tmp/explicit-home/.researchx/web-search.json",
		);
	} finally {
...
test("formatPiWebAccessDoctorLines marks missing config path and includes a setup hint", () => {
	...
	assert.ok(
		lines.some((line) => line.includes("hint:") && line.includes("researchx search set")),
		"expected a hint line pointing to `researchx search set`",
	);
});
