...
const originalEndpoints = process.env.RESEARCHX_BIOMART_ENDPOINTS;

afterEach(() => {
	globalThis.fetch = originalFetch;
	if (originalEndpoints === undefined) delete process.env.RESEARCHX_BIOMART_ENDPOINTS;
	else process.env.RESEARCHX_BIOMART_ENDPOINTS = originalEndpoints;
});
...
test("science database tool lists BioMart registry rows", async () => {
	process.env.RESEARCHX_BIOMART_ENDPOINTS = "https://biomart.example/martservice";
...
test("science database tool queries BioMart gene rows with endpoint fallback and completion stamp", async () => {
	process.env.RESEARCHX_BIOMART_ENDPOINTS = "https://bad.example/martservice,https://ok.example/martservice";
