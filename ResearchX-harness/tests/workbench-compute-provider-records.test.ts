...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-compute-providers-"));
...
test("buildWorkbenchState exposes Claude-style compute provider rows", () => {
	...
	try {
		...
		assert.equal(local?.family, "ResearchX");
		...
		assert.deepEqual(nvidia?.inferConfig, { hosted: "ESMFold", selfHosted: "AlphaFold2 NIM", tool: "researchx_model_endpoint_call" });
		...
		assert.equal(ssh?.egressPolicy, "ResearchX Settings Network policy");
		assert.equal(ssh?.scratchRoot, "/scratch/researchx");
		...
		const kaggle = state.computeProviders.find((provider) => provider.name === "kaggle");
		assert.equal(kaggle?.egressPolicy, "ResearchX Settings Network policy");
		assert.equal(kaggle?.enabled, true);
	} finally {
...
test("workbench server returns compute provider rows through state", async () => {
	...
	try {
		...
		assert.equal(payload.computeProviders.some((provider) =>
			...
			provider.scratchRoot === "/scratch/researchx"
		), true);
	} finally {
		await handle.close();
		rmSync(root, { recursive: true, force: true });
