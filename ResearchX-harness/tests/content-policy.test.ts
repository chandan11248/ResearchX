...
test("research writing prompts forbid fabricated results and unproven figures", () => {
	...
	const systemPrompt = readFileSync(join(repoRoot, ".researchx", "SYSTEM.md"), "utf8");
	const writerPrompt = readFileSync(join(repoRoot, ".researchx", "agents", "writer.md"), "utf8");
	const verifierPrompt = readFileSync(join(repoRoot, ".researchx", "agents", "verifier.md"), "utf8");
...
test("research workflows use real web-search tool names and grant them to evidence agents", () => {
	const systemPrompt = readFileSync(join(repoRoot, ".researchx", "SYSTEM.md"), "utf8");
	const deepResearchPrompt = readFileSync(join(repoRoot, "prompts", "deepresearch.md"), "utf8");
	const researcherPrompt = readFileSync(join(repoRoot, ".researchx", "agents", "researcher.md"), "utf8");
	const verifierPrompt = readFileSync(join(repoRoot, ".researchx", "agents", "verifier.md"), "utf8");
	...
	assert.match(deepResearchPrompt, /Use visible ResearchX alpha tools such as `alpha_search`/i);
	assert.match(deepResearchPrompt, /call `researchx alpha \.\.\.`/i);
...
test("repo agent contract keeps ResearchX general-purpose with research flagship", () => {
	const agentsPrompt = readFileSync(join(repoRoot, "AGENTS.md"), "utf8");

	assert.match(agentsPrompt, /simple yet potent/i);
	assert.match(agentsPrompt, /general-purpose assistant/i);
	assert.match(agentsPrompt, /Every new feature must fight for its life/i);
	assert.match(agentsPrompt, /core job/i);
	assert.match(agentsPrompt, /Reject unrelated product lanes by default/i);
...
test("review surfaces frame critique as internal research review, not external peer review", () => {
	...
	const reviewerPrompt = readFileSync(join(repoRoot, ".researchx", "agents", "reviewer.md"), "utf8");
	const systemPrompt = readFileSync(join(repoRoot, ".researchx", "SYSTEM.md"), "utf8");
...
test("compute copy stays scoped to explicit research experiments", () => {
	const homePage = readFileSync(join(repoRoot, "website", "src", "pages", "index.astro"), "utf8");
	const readme = readFileSync(join(repoRoot, "README.md"), "utf8");
	const kaggleSkill = readFileSync(join(repoRoot, "skills", "kaggle", "SKILL.md"), "utf8");
	...
	assert.match(kaggleSkill, /Kaggle is the default compute backend/i);
	assert.match(kaggleSkill, /Never run heavy compute locally when Kaggle is available/i);
	assert.match(kaggleSkill, /Monitor every 10 minutes/i);
...
test("setup docs and LiteLLM fallback do not pin a stale OpenAI model slug", () => {
	...
	assert.match(setupSource, /researchx model set <provider\/model>/i);
...
test("alphaXiv active docs and skills route shell commands through bundled ResearchX alpha", () => {
	const alphaDocs = readFileSync(join(repoRoot, "website", "src", "content", "docs", "tools", "alphaxiv.md"), "utf8");
	const alphaSkill = readFileSync(join(repoRoot, "skills", "alpha-research", "SKILL.md"), "utf8");
	const activeAlphaCopy = `${alphaDocs}\n${alphaSkill}`;

	assert.match(activeAlphaCopy, /researchx alpha login/i);
	assert.match(activeAlphaCopy, /researchx alpha annotate/i);
