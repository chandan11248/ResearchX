...
test("ResearchX help omits generic scheduler and process package commands", async () => {
...
test("ResearchX command browser omits generic scheduler and process package commands", async () => {
	type RegisteredCommand = {
		description?: string;
		handler: (_args: string[], ctx: unknown) => Promise<void>;
	};
	const repoRoot = "/tmp/researchx";
...
test("ResearchX tool browser omits generic scheduler and process package tools", async () => {
	type RegisteredCommand = {
		description?: string;
		handler: (_args: string[], ctx: unknown) => Promise<void>;
	};
	...
	const windowsLocalSourceInfo = {
		...localSourceInfo,
		path: "C:\\researchx\\extensions\\research-tools.ts",
	};
	const packageSourceInfo = (source: string) => ({
		source,
		path: `${repoRoot}/node_modules/${source.replace(/^npm:/, "")}/dist/index.js`,
