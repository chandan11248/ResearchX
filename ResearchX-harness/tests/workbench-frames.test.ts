...
function makeFrameWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-frames-"));
...
test("workbench state exposes Claude-style frame records from ResearchX sessions, runs, and uploads", async () => {
	...
	try {
		...
		writeFileSync(workbenchDataPath(root, "frame-backfill-poison.json"), JSON.stringify({
			schema: "researchx.frameBackfillPoison.v1",
			frameBackfillPoison: [
		...
		assert.match(contextData.pi_session?.id ?? "", /^researchx-workbench-kinase-session$/);
		...
		assert.equal(runFrame?.agentName, "RESEARCHX");
