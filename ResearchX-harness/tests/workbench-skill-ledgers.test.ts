...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-skill-ledgers-"));
	...
	writeFileSync(join(root, ".researchx", "agents", "reviewer.md"), [
		"---",
		"name: reviewer",
...
test("buildWorkbenchState exposes Claude-style custom skill and agent prompt rows", () => {
	...
	try {
		...
		assert.equal(assignment?.agentName, "researchx");
		...
		assert.equal(prompt?.path, ".researchx/agents/reviewer.md");
		...
		assert.equal(source?.slug, "researchx-science-skill-pack");
		assert.equal(source?.kind, "local");
		assert.equal(source?.marketplaceName, "ResearchX Science Skill Pack");
		...
		assert.equal(assent?.noticeVersion, "researchx-owned-skill-pack-v1");
		assert.match(assent?.noticeText ?? "", /ResearchX-owned local project skill/);
...
test("workbench server returns skill ledgers through state", async () => {
	...
	try {
		...
		]), [[payload.customSkills[0]?.id, "researchx"]]);
		...
		assert.equal(payload.marketplaceSources[0]?.slug, "researchx-science-skill-pack");
