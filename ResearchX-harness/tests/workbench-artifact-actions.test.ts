...
function makeWorkspace(): string {
	const root = mkdtempSync(join(tmpdir(), "researchx-workbench-artifact-actions-"));
...
test("artifact actions persist star, rename, hide, and reversible delete state", () => {
	...
	try {
		...
		assert.equal(existsSync(join(root, ".researchx", "workbench", "artifact-actions.json")), false);
