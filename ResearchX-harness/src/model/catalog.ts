...
export function buildModelStatusSnapshotFromRecords(
	...
	if (nonProAvailable.length === 0) {
		if (proClassAvailableCount > 0) {
			guidance.push("No approved authenticated Pi models are available. Premium Pro-class models are disabled in ResearchX.");
			guidance.push("Configure an approved research model, then rerun `researchx model list`.");
		} else {
			guidance.push("No authenticated Pi models are available yet.");
			guidance.push(
				"Run `researchx model login <provider>` (OAuth) or configure an API key (env var, auth.json, or models.json for custom providers).",
			);
			guidance.push("After auth is in place, rerun `researchx model list`.");
		}
	} else if (!current) {
		if (recommended) {
		...
		guidance.push("Run `researchx model set <provider/model>` after choosing from `researchx model list`.");
