...
async function promptLiteLlmProviderSetup(): Promise<CustomProviderSetup | undefined> {
	...
	printInfo("Start the LiteLLM proxy first. ResearchX uses the OpenAI-compatible chat-completions API.");
	...
	if (hasKey) {
		printInfo("Set LITELLM_MASTER_KEY in your shell or .env before using ResearchX.");
	}
...
async function configureBedrockProvider(authPath: string): Promise<boolean> {
	...
	printInfo("ResearchX will verify the AWS SDK credential chain used by Pi's Bedrock provider.");
	...
	try {
		...
		printInfo("Use `researchx model list` to see available Bedrock models.");
		return true;
	} catch (error) {
...
async function configureApiKeyProvider(authPath: string, providerId?: string): Promise<boolean> {
	...
	if (!apiKey) {
		if (provider.envVar) {
			printInfo(`Set ${provider.envVar} and rerun setup (or run \`researchx model list\`).`);
		} else {
...
export async function setDefaultModelSpec(settingsPath: string, authPath: string, spec: string): Promise<void> {
	...
	if (!resolvedSpec) {
		throw new Error(`Model not available in Pi auth storage: ${spec}. Run \`researchx model list\` first.`);
	}
