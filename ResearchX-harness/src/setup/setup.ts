...
import { normalizeResearchXSettings } from "../pi/settings.js";
...
function printNonInteractiveSetupGuidance(): void {
	...
	printInfo("  researchx model login <provider>");
	printInfo("  researchx model set <provider/model>");
	printInfo("  # or configure API keys via env vars/auth.json and rerun `researchx model list`");
	printInfo("  researchx alpha login");
...
async function maybeInstallBundledPackages(options: SetupOptions): Promise<void> {
	...
	const shouldInstall = await promptConfirm("Install missing ResearchX packages now?", true);
	if (!shouldInstall) {
		printInfo("Skipping package install. ResearchX may install missing packages later if needed.");
		return;
	}
...
export async function runSetup(options: SetupOptions): Promise<void> {
	...
	try {
		await promptIntro("ResearchX setup");
		...
		await normalizeResearchXSettings(
		...
		await promptOutro("ResearchX is ready");
	} catch (error) {
