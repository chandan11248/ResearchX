...
export const PAPER_RANK_SOURCES = [
	{
		...
		reason: "Defines Works as scholarly documents and documents the search/list surface used by researchx rank.",
	},
	{
...
export type OpenAlexWork = {
	...
	researchx_full_text?: string | null;
	researchx_full_text_source?: string | null;
};
...
export async function fetchOpenAlexWorks(
	...
	const response = await fetchWithTimeout(fetchImpl, url, {
		headers: {
			...
			"User-Agent": "ResearchX PaperRank local research workflow",
		},
...
export async function fetchOpenAlexWorksByIds(
	...
	const response = await fetchWithTimeout(fetchImpl, url, {
		headers: {
			Accept: "application/json",
			"User-Agent": "ResearchX PaperRank citation expansion workflow",
		},
...
export function buildFullTextAccessPlan(paper: PaperRecord, generatedAt?: string): FullTextAccessPlan {
	...
	if (paper.arxivId) {
		add({
			...
			isOpenAccess: true,
			canFetch: true,
			note: "ResearchX can fetch arXiv paper text through the bundled alphaXiv client when available.",
		});
		add({
			source: "arXiv",
			kind: "pdf",
			...
			isOpenAccess: true,
			canFetch: false,
			note: "Direct PDF access is exposed as an access link; ResearchX does not parse arbitrary PDFs in this resolver.",
		});
	}
	if (paper.pmcid) {
		add({
	...
	for (const url of paper.urls) {
		if (url.type === "pdf") {
			...
			add({
				...
				canFetch: false,
				note: arxivUrl
					? "Direct arXiv PDF access is exposed as an access link; ResearchX does not parse arbitrary PDFs in this resolver."
...
function reproductionEvidenceLimits(status: ReproductionEvidenceStatus): string[] {
	...
	return [
		...
		"A reproduced or failed status is a recorded outcome, not a claim that ResearchX itself executed the experiment during ranking.",
...
function nextResearchActionsBasis(): string[] {
	return [
		...
		"Replication actions are triggered by missing, partial, failed, or not-runnable completed reproduction evidence and do not claim that ResearchX executed experiments during ranking.",
...
export function renderModelSynthesisPrompt(packet: ModelSynthesisPacket): string {
	...
	return [
		`# ResearchX PaperRank Model Synthesis Prompt`,
		"",
...
export async function fetchEuropePmcPaperContent(
	...
	const response = await fetchWithTimeout(fetchImpl, url, {
		headers: {
			...
			"User-Agent": "ResearchX full-text resolver",
		},
...
async function fetchEuropePmcRecord(
	...
	const response = await fetchWithTimeout(fetchImpl, url, {
		headers: {
			Accept: "application/json",
			"User-Agent": "ResearchX Europe PMC resolver",
		},
	...
	for (const paper of selected) {
		try {
			...
			const erroredPaper: PaperRecord = {
				...
				fullTextSource: "ResearchX full-text resolver",
...
function createFixturePaperContentFetcher(works: OpenAlexWork[]): PaperContentFetcher {
	...
	return async (paper) => {
		...
		const content = cleanString(work?.researchx_full_text);
		if (!content) return undefined;
		return {
			content,
			source: cleanString(work?.researchx_full_text_source) ?? "fixture full text",
		};
	};
}
...
export async function runPaperRank(options: PaperRankOptions): Promise<PaperRankRunResult> {
	...
	if (!topic) throw new Error("Usage: researchx rank <topic>");
...
export async function fetchOpenAlexWorkByIdentifier(
	...
	if (!normalized) throw new Error("Usage: researchx paper <doi|arxiv-id|openalex-id|pmid|pmcid|title>");
	...
	const response = await fetchWithTimeout(fetchImpl, url, {
		headers: {
			Accept: "application/json",
			"User-Agent": "ResearchX paper access resolver",
		},
...
export async function resolvePaperAccess(options: PaperAccessOptions): Promise<PaperAccessResult> {
	...
	if (!identifier) throw new Error("Usage: researchx paper <doi|arxiv-id|openalex-id|pmid|pmcid|title>");
	const now = options.now ?? new Date();
	const generatedAt = now.toISOString();
	const outputDir = resolve(options.outputDir ?? "outputs");
...
function renderPaperAccessReport(input: {
	...
	return [
		...
		"## Access Candidates",
		"",
		"| Rank | Source | Kind | Fetchable by ResearchX | Open access | URL/ID | Note |",
		"| --- | --- | --- | --- | --- | --- | --- |",
		...candidates,
		"",
		"## Limits",
...
function buildPaperRankResearchRun(input: {
	...
	const tools = [
		...
		{
			id: "researchx.paper_rank.scoring",
		...
		{
			id: "researchx.paper_rank.field_map",
		...
		{
			id: "researchx.paper_rank.full_text",
		...
		...(input.synthesis.requested ? [{
			id: "researchx.paper_rank.model_synthesis",
			kind: "model" as const,
	...
	return {
		schemaVersion: "researchx.researchRun.v1",
		...
		verification: {
			...
			caveats: [
				...
				"Missing evidence means ResearchX did not see it in fetched sources; it is not proof the paper lacks it.",
...
function buildScoreCalibrationTemplate(input: {
	...
	return {
		schemaVersion: "researchx.paperRank.preferenceTemplate.v1",
		...
		instructions: [
			...
			`Rerun with: researchx rank ${shellQuoteArg(input.topic)} --preference-file ${input.slug}-calibration-template.json`,
		],
		rankedPaperIds: [],
		preferences: [],
...
function renderCalibrationGuide(input: {
	...
	return [
		...
		"",
		"```bash",
		`researchx rank ${shellQuoteArg(input.topic)} --preference-file ${input.slug}-calibration-template.json`,
		"```",
		"",
		"## Candidate Papers",
		"",
...
function buildReproductionNotesTemplate(input: {
	...
	return {
		schemaVersion: "researchx.paperRank.reproductionNotesTemplate.v1",
		...
		instructions: [
			...
			`Rerun with: researchx rank ${shellQuoteArg(input.topic)} --reproduction-notes ${input.slug}-reproduction-notes-template.json`,
		],
		notes: [],
...
function renderGraphExplorer(input: {
	...
	return [
		...
		"<header>",
		"<div>",
		"<p class=\"eyebrow\">ResearchX PaperRank Graph Explorer</p>",
