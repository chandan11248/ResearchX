...
function createPackageContext(workingDir: string, agentDir: string) {
	applyResearchXPackageManagerEnv(agentDir);
...
export function reconcileManagedCorePackageInstalls(
	...
	const bundledNodeModulesRoot = resolve(appRoot, ".researchx", "npm", "node_modules");
	...
	const globalNodeModulesRoot = getResearchXNpmGlobalNodeModulesPath(agentDir);
...
function resolveRuntimePeerSpec(packageName: string): string | undefined {
	...
	for (const packageRoot of [
		...
		resolve(APP_ROOT, ".researchx", "npm", "node_modules", packageName),
	]) {
		try {
...
function ensureLocalInstallRoot(installRoot: string): string {
	...
	if (!existsSync(packageJsonPath)) {
		writeFileSync(packageJsonPath, JSON.stringify({ name: "researchx-packages", private: true }, null, 2) + "\n", "utf8");
	}
...
function defaultNpmInstallTarget(workingDir: string, agentDir: string, scope: PackageScope): NpmInstallTarget {
	if (scope === "project") {
		return {
			scope,
			installRoot: resolve(workingDir, ".researchx", "npm"),
			global: false,
			cwd: workingDir,
		};
	}
	...
	return {
		scope,
		installRoot: getResearchXNpmPrefixPath(agentDir),
		global: true,
		cwd: agentDir,
	};
}
...
function isBundledWorkspacePackagePath(installedPath: string | undefined, appRoot: string): boolean {
	...
	const bundledRoot = resolve(appRoot, ".researchx", "npm", "node_modules");
...
export function seedBundledWorkspacePackages(
	...
	const globalNodeModulesRoot = getResearchXNpmGlobalNodeModulesPath(agentDir, platform);
	const seeded: string[] = [];
