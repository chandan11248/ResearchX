...
export async function launchPiChat(options: PiRuntimeOptions): Promise<void> {
	...
	patchPiRuntimeNodeModules(options.appRoot, options.researchxAgentDir);
	...
	if (!useBuiltWrapper && !useDevWrapper) {
		throw new Error(`ResearchX Pi CLI wrapper not found: ${piCliWrapperPath}`);
	}
	...
	ensureResearchXCommandShim(options.appRoot, options.researchxAgentDir);
	ensureResearchXWorkspaceScaffold(options.workingDir);
	...
	await new Promise<void>((resolvePromise, reject) => {
		...
		child.on("exit", (code, signal) => {
			if (signal) {
				console.error(`researchx terminated because the Pi child exited with ${signal}.`);
