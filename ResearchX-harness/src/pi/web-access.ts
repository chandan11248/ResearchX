...
export function getPiWebSearchConfigPath(home?: string): string {
	if (!home) {
		const configuredPath = (process.env.RESEARCHX_WEB_SEARCH_CONFIG)?.trim();
		if (configuredPath) return resolve(configuredPath);
	}
	const researchxHome = home ? resolve(home, ".researchx") : getResearchXHome();
	return resolve(researchxHome, "web-search.json");
}
