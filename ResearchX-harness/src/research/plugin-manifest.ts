...
export const researchxPluginSlotValues = [
	"source_adapters",
	"access_resolvers",
	"entity_extractors",
	"rank_scorers",
...
export type ResearchXPluginSlot = (typeof researchxPluginSlotValues)[number];

export type ResearchXPluginManifest = {
	manifest_version: 1;
	name: string;
	version?: string;
	description?: string;
	research_jobs: ResearchJob[];
	slots?: Partial<Record<ResearchXPluginSlot, string[]>>;
	pi?: {
		extensions?: string[];
		skills?: string[];
		prompts?: string[];
...
export function validateResearchXPluginManifest(value: unknown): PluginManifestValidationResult {
	const errors: string[] = [];
	...
	if (!Array.isArray(researchJobs) || researchJobs.length === 0) {
		...
		for (const [index, job] of researchJobs.entries()) {
			if (typeof job !== "string" || !researchJobValues.includes(job as ResearchJob)) {
				errors.push(`research_jobs[${index}] is not a supported ResearchX research job`);
			}
		}
	}
	...
	if (value.slots !== undefined) {
		if (!isRecord(value.slots)) {
			...
			for (const [slot, paths] of Object.entries(value.slots)) {
				if (!researchxPluginSlotValues.includes(slot as ResearchXPluginSlot)) {
