...
for line in sys.stdin:
    ...
    try:
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            exec(compile(code, "<researchx-notebook-cell>", "exec"), namespace, namespace)
...
class PersistentBashKernel implements NotebookKernelSession {
	...
	private runOnce(code: string, timeoutMs: number, maxOutputChars: number): Promise<KernelProcessResult> {
		...
		const id = randomUUID();
		const marker = `__RESEARCHX_NOTEBOOK_${id.replace(/-/g, "")}_DONE__`;
		return new Promise<KernelProcessResult>((resolveRun) => {
			const timeout = setTimeout(() => {
...
class PersistentRKernel implements NotebookKernelSession {
	...
	private runOnce(code: string, timeoutMs: number, maxOutputChars: number): Promise<KernelProcessResult> {
		...
		const wrappedCode = [
			".researchx_status <- 0",
			`tryCatch({ eval(parse(text = ${rStringLiteral(code)}), envir = .GlobalEnv) }, error = function(e) { .researchx_status <<- 1; message(conditionMessage(e)) })`,
			`cat("\\n${marker}:", .researchx_status, "\\n", sep = "")`,
		].join("\n");
		return new Promise<KernelProcessResult>((resolveRun) => {
			const timeout = setTimeout(() => {
				this.current = undefined;
