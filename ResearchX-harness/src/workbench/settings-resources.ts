...
const CLAUDE_SCIENCE_REFERENCE_ENV = "RESEARCHX_DEBUG_CLAUDE_SCIENCE_REFERENCE";
...
function envDescription(envVar: string, configuredText: string, missingText: string): string {
	return process.env[envVar]?.trim()
		...
		: `${missingText} Set ${envVar} or use the matching ResearchX CLI command.`;
}
...
function kaggleCredentialStatus(): WorkbenchResource["status"] {
...
function globalKaggleCliPath(): string | undefined {
	const result = spawnSync("sh", ["-lc", "command -v kaggle"], { encoding: "utf8", timeout: 3000 });
	const value = result.stdout.trim();
	return value || undefined;
}

function kaggleCliDiagnostic(): string {
	const global = globalKaggleCliPath();
	if (global) return `CLI: using kaggle from PATH at ${global}.`;
	return "CLI: kaggle command is not installed; run `pip install kaggle` to enable pushes.";
}
...
function scienceDomainResource(id: string, name: string, detail: string, tags: string[]): WorkbenchResource {
	return {
		id: `network-${id}`,
		name,
		description: `${name} endpoints are grouped for science workflows; ResearchX currently uses the host network and connector-specific settings rather than an enforced domain allowlist.`,
		status: "available",
...
const RESEARCHX_BIO_TOOL_SOURCE_NAMES = [
	"PubMed",
	"Europe PMC",
	"OpenAlex",
	"Crossref",
...
const SCIENCE_CONNECTOR_PRESETS: ScienceConnectorPreset[] = [
	...
	{ id: "human-genetics", name: "Human Genetics", description: "Connect human genetic association, phenotype, and variant interpretation resources.", section: "Featured", source: "Science connector preset", tags: ["mcp", "human genetics", "phenotype"], builtInDatabaseSource: "gwascatalog" },
	{
		...
		tools: [
			{ name: "researchx_open_chemistry_sketcher", description: "Create a ResearchX-owned chemistry sketch artifact for the local Ketcher editor." },
		],
	},
...
function buildResearchXBioToolsResource(): WorkbenchResource {
	return {
		id: "researchx-bio-tools",
		name: "ResearchX Bio Tools",
			description: "ResearchX-owned read-only science database connector for papers, open-access full-text sections, citation graphs, authors, venues, datasets, clinical trials, BioMart, CellGuide, Antibody Registry, metabolomics, genome tracks, TF-DNA binding, proteins, structures, EM maps, complexes, interactions, genes, variants, expression, ontology, pathway, ChEMBL molecular pharmacology, chemistry, and purchasable-compound lookup.",
		status: "configured",
		source: "ResearchX built-in science tools",
		connectorKind: "featured",
		section: "Featured",
			detail: `${RESEARCHX_BIO_TOOL_SOURCE_NAMES.length} public or credential-aware sources through researchx_science_database_search.`,
		diagnostics: [
			"Runtime: implemented inside the ResearchX repository and available to Pi chat without a local reference-app dependency.",
			`Sources: ${RESEARCHX_BIO_TOOL_SOURCE_NAMES.join(", ")}.`,
			"Approval boundary: built-in read-only search is enabled through ResearchX's own permission grants.",
		],
		tags: ["researchx", "bio tools", "science databases", "built-in", "read-only"],
		tools: [{
			name: "researchx_science_database_search",
			description: `Search ${RESEARCHX_BIO_TOOL_SOURCE_NAMES.length} public or credential-aware science sources with ResearchX-owned provenance output.`,
		}],
	};
}
	...
	return SCIENCE_CONNECTOR_PRESETS
		...
		.map((preset) => {
			const builtInTool = preset.builtInDatabaseSource
				? { name: "researchx_science_database_search", description: `Built-in read-only ${preset.name} search source: ${preset.builtInDatabaseSource}.` }
				: undefined;
			const executablePreset = Boolean(builtInTool || preset.tools?.some((tool) => tool.name.startsWith("researchx_")));
			return {
				...
				diagnostics: [
					"Catalog state: ResearchX-owned preset backed by the local workbench settings and tool registry.",
					executablePreset
						? "Execution: built-in ResearchX tool is available in Pi chat without connector setup."
						: "Execution: available after you add a matching MCP URL/local command or enable a Pi package that exposes the tool.",
					"Approval boundary: custom MCP calls still use ResearchX's connector grants, chat approval cards, assignment, and excluded-tool policy.",
				],
...
function buildSpecialistResources(workingDir: string): WorkbenchResource[] {
	return directMarkdownFiles(resolve(workingDir, ".researchx", "agents"))
		.map((path) => markdownResource(workingDir, path, {
			source: "ResearchX specialist",
	...
	return [
		{
			...
			status: existsSync(resolve(workingDir, ".researchx", "settings.json")) ? "configured" : "available",
			source: "Pi security model",
			section: "Trust boundary",
			path: ".researchx/settings.json",
	...
	return [
		...WORKBENCH_CREDENTIAL_PROVIDERS.map((provider) => {
			...
			return {
				...
				name: provider.name,
				description: isKaggle && kaggleCredentialStatus() === "configured"
					? "Credential detected. Kaggle credentials are available through the local Kaggle credential path; values are not displayed."
					: envDescription(provider.envVar, "Credential detected.", "Credential not detected."),
				status: isKaggle ? kaggleCredentialStatus() : envStatus(provider.envVar),
				source: provider.source,
				section: provider.section,
				detail: isKaggle ? kaggleCredentialDetail() : provider.envVar,
				...(isKaggle ? { diagnostics: [
					process.env.KAGGLE_USERNAME?.trim() && process.env.KAGGLE_KEY?.trim()
						? "Auth: KAGGLE_USERNAME and KAGGLE_KEY are present in the server environment."
						: kaggleConfigExists()
							? "Auth: ~/.kaggle/kaggle.json exists and is the active detected Kaggle credential source."
							: "Auth: Kaggle credentials were not detected. Set KAGGLE_USERNAME + KAGGLE_KEY or add ~/.kaggle/kaggle.json.",
					kaggleCliDiagnostic(),
				] } : {}),
				tags: provider.tags,
			} satisfies WorkbenchResource;
...
function buildCustomConnectorResources(settings: WorkbenchSettings, workingDir: string): WorkbenchResource[] {
	return settings.customConnectors.map((connector) => {
		...
		return {
			...
			diagnostics: [
					"Config: stored in the ResearchX app data root.",
				connector.transport === "local"
					? "Runtime: local command connectors can be discovered with researchx_connector_tools and called with researchx_connector_call after approval."
					: connector.transport === "streamable_http"
						? "Runtime: Streamable HTTP connectors can be discovered with researchx_connector_tools and called with researchx_connector_call after approval."
						: "Runtime: SSE connectors can be discovered with researchx_connector_tools and called with researchx_connector_call after approval.",
...
function buildStorageResources(workingDir: string, artifacts: WorkbenchArtifact[]): WorkbenchResource[] {
	...
	const piSessionCount = countFiles(resolve(workingDir, ".researchx", "sessions"), (name) => name.endsWith(".jsonl"));
	...
	return [
		{
			...
			name: "Data location",
			description: "ResearchX stores app-owned workbench state under the ResearchX app data root.",
			status: "configured",
			source: "ResearchX app data",
			section: "Local data",
		...
		},
		{
			...
			description: `${pluralize(chatCount, "session")} persisted under the ResearchX app data root.`,
...
function buildUsageResources(
	...
	return [
		...
		},
		{
			...
			description: "ResearchX exposes local run evidence here; provider billing and subscription limits stay with the selected model provider.",
			status: "available",
...
function buildGeneralResources(workingDir: string): WorkbenchResource[] {
	...
	const settings = readJsonObject(resolve(workingDir, ".researchx", "settings.json"));
	...
	return [
		...
		},
		{
			...
			description: "The local UI uses ResearchX's blue research theme and the Codex-style split control-plane layout.",
			status: "configured",
			source: "Workbench UI",
			section: "Appearance",
...
export function buildWorkbenchSettingsResourceGroups(options: {
	...
	const researchxBioTools = buildResearchXBioToolsResource();
	const connectors = [
		researchxBioTools,
		...buildScienceConnectorCatalogResources([...packageConnectors, ...customConnectors, researchxBioTools]),
		...packageConnectors,
		...customConnectors,
	];
	return [
		...
		},
		{
			id: "credentials",
			title: "Credentials",
			description: "Credential surfaces ResearchX can use for model, evidence, code, and compute providers.",
