...
export function buildWorkbenchOAuthTokenRecords(
	...
	return readWorkbenchOAuthTokens(workingDir).tokens.flatMap((token) => {
		...
		return [{
			id: token.id || stableUuid("researchx-oauth-token", token.connectorId),
			userId: LOCAL_USER_ID,
			mcpServerId: server.id,
			encryptedAccessToken: `researchx-oauth-ref:${token.connectorId}:access`,
			...(token.refreshToken ? { encryptedRefreshToken: `researchx-oauth-ref:${token.connectorId}:refresh` } : {}),
