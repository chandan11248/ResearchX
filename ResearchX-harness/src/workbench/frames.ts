...
function agentNameFromSession(session: WorkbenchChatSession): string {
	return (specialistName(session) ?? "ResearchX").replace(/\s+/g, "_").toUpperCase();
}
...
function frameFromSession(session: WorkbenchChatSession, rootSeq: number): WorkbenchFrameRecord {
	...
	return {
		...
		isHidden: false,
		statusDescription: status === "completed" ? "ResearchX workbench session is complete." : `ResearchX workbench session is ${status}.`,
...
function frameFromRun(run: WorkbenchRun, rootSeq: number): WorkbenchFrameRecord {
	...
	return {
		...
		isHidden: false,
		statusDescription: status === "completed" ? "ResearchX artifact frame is complete." : `ResearchX artifact frame is ${status}.`,
		rootSeq,
		source: "artifact-run",
	};
}
...
function uploadFrameFromProject(project: WorkbenchProject): WorkbenchFrameRecord {
	...
	return {
		...
		isHidden: false,
		statusDescription: "ResearchX project upload frame is available.",
		rootSeq: 1,
		source: "project-uploads",
	};
}
