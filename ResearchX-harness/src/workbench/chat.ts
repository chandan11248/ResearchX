...
import {
	abortResearchXWorkbenchPrompt,
	runResearchXWorkbenchPrompt,
	runResearchXWorkbenchPromptStream,
	steerResearchXWorkbenchPrompt,
...
export async function submitWorkbenchChatMessage(
	...
	try {
		const executor = options.executor ?? runResearchXWorkbenchPrompt;
		const result = await executor({ ...options, session, message, viewportContext: input.viewportContext });
		const assistantMessage = createMessage(
			"assistant",
			result.content.trim() || "ResearchX finished without text output.",
...
export async function steerWorkbenchChatMessage(
	...
	const queuedMessage = createMessage("user", message, "queued", [{
		...
		status: "running",
		output: "This message will steer the running ResearchX turn.",
	}]);
	session = {
		...session,
		status: "running",
	...
	try {
		await steerResearchXWorkbenchPrompt({ ...options, session, message, viewportContext: input.viewportContext });
...
export async function abortWorkbenchChatMessage(
	...
	let session = ensureWorkbenchChatSession(options, input);
	await abortResearchXWorkbenchPrompt({
		...options,
		session,
		message: "Stop the running ResearchX turn.",
	});
...
export async function streamWorkbenchChatMessage(
	...
	const path = chatPath(options.workingDir, session.id);
	const assistantMessage = createMessage("assistant", "Starting ResearchX inside this workspace...", "running", [{
		id: randomUUID(),
	...
	try {
		...
		const result = options.executor
			...
			: await runResearchXWorkbenchPromptStream(request, async (update) => {
		...
		session = updateMessage(session, assistantMessage.id, {
			content: result.content.trim() || "ResearchX finished without text output.",
			status: result.status ?? "complete",
