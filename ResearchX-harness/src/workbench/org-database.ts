...
import { getResearchXOrgDatabasePath } from "../config/paths.js";
...
function execSchema(database: DatabaseSync): void {
	database.exec(`
CREATE TABLE IF NOT EXISTS __researchx_mirror_meta (
	key TEXT PRIMARY KEY,
	value TEXT NOT NULL
);

CREATE TABLE IF NOT EXISTS researchx_state_payloads (
...
function clearMirroredTables(database: DatabaseSync): void {
	...
	database.exec("DELETE FROM researchx_state_payloads");
}

function insertPayload(database: DatabaseSync, tableName: string, rowId: string, payload: unknown, updatedAt: number): void {
	const insert = database.prepare(`
INSERT OR REPLACE INTO researchx_state_payloads (table_name, row_id, payload_json, updated_at)
VALUES (?, ?, ?, ?)
`) as RunStatement;
...
export function ensureWorkbenchOrgDatabase(path = getResearchXOrgDatabasePath()): string {
	mkdirSync(dirname(path), { recursive: true });
	const database = new DatabaseSync(path);
	try {
		execSchema(database);
		database.prepare("INSERT OR REPLACE INTO __researchx_mirror_meta (key, value) VALUES (?, ?)").run("schema_version", String(ORG_DATABASE_SCHEMA_VERSION));
	} finally {
		database.close();
	}
	return path;
...
export function materializeWorkbenchOrgDatabase(state: WorkbenchState, path = getResearchXOrgDatabasePath()): WorkbenchOrgDatabaseSummary {
	mkdirSync(dirname(path), { recursive: true });
	...
	try {
		...
		try {
			...
			const meta = database.prepare("INSERT OR REPLACE INTO __researchx_mirror_meta (key, value) VALUES (?, ?)") as RunStatement;
