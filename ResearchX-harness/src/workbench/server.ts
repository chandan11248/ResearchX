...
import { closeWorkbenchPiRpcClients, listResearchXWorkbenchCommands } from "./chat-runtime.js";
...
function isAuthorized(request: IncomingMessage, url: URL, token: string): boolean {
	...
	const headerToken = request.headers["x-researchx-token"];
	const cookieToken = requestCookie(request.headers.cookie, "researchx_workbench");
	return queryToken === token || headerToken === token || cookieToken === token;
}

function tokenCookie(token: string): string {
	return `researchx_workbench=${encodeURIComponent(token)}; HttpOnly; SameSite=Strict; Path=/`;
}
...
async function handleWorkbenchRequest(
	...
	response: ServerResponse,
		...
		if (url.pathname === "/api/connectors/oauth/callback") {
			try {
				...
				send(response, 200, [
					"<!doctype html>",
					"<meta charset=\"utf-8\">",
					"<title>ResearchX OAuth connected</title>",
					"<body style=\"font:14px system-ui;background:#0f140d;color:#f1f4e9;padding:24px\">",
					"<h1>OAuth connected</h1>",
					`<p>Connector ${token.connectorId} is connected. You can close this tab and return to ResearchX.</p>`,
					"</body>",
				...
				send(response, 400, [
					"<!doctype html>",
					"<meta charset=\"utf-8\">",
					"<title>ResearchX OAuth failed</title>",
					"<body style=\"font:14px system-ui;background:#0f140d;color:#f1f4e9;padding:24px\">",
					"<h1>OAuth failed</h1>",
					"<p>OAuth connection failed. Return to ResearchX and try again.</p>",
					"</body>",
				].join(""), { "content-type": "text/html; charset=utf-8" });
			}
			return;
		...
		if (options.requireAuth) {
			...
			if (!authorized) {
				send(response, 401, "Unauthorized. Open the URL printed by `researchx serve`.");
				return;
			}
		...
		try {
			...
			if (url.pathname === "/api/chat/message" && request.method === "POST") {
				...
				sendJson(response, 200, {
					session: await submitWorkbenchChatMessage({
						...
						sessionDir: options.sessionDir,
						researchxAgentDir: options.researchxAgentDir,
						researchxVersion: options.version,
			...
			if (url.pathname === "/api/chat/commands" && request.method === "POST") {
				...
				const commands = await listResearchXWorkbenchCommands({
					workingDir: options.workingDir,
					appRoot: options.appRoot,
					sessionDir: options.sessionDir,
					researchxAgentDir: options.researchxAgentDir,
...
export async function serveWorkbench(options: ServeWorkbenchOptions): Promise<void> {
	...
	console.log("ResearchX workbench running");
