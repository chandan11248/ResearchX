...
const WORKBENCH_PLAN_SCHEMA = "researchx.workbenchPlan.v1";
...
function buildProjects(runs: WorkbenchRun[], artifacts: WorkbenchArtifact[], customProjects: WorkbenchStoredProject[]): WorkbenchProject[] {
	...
	const builtInProjects = [
		...
		buildProject(
			...
			"ResearchX-owned enzyme engineering, extremophile protein, CRISPR screen, and immunotherapy fixtures for open-science parity and smoke tests.",
			"seeds",
			seedRuns,
	...
	return [
		{
			...
			description: "Provider credentials are managed through ResearchX's model login flow and reused by Pi sessions.",
			status: "available",
			source: "ResearchX CLI",
			command: "researchx model login [id]",
			tags: ["models", "oauth", "api keys"],
		},
		{
			id: "search-provider-keys",
			name: "Search provider keys",
			description: "Web evidence providers are configured through ResearchX's search settings.",
			status: "available",
			source: "ResearchX CLI",
			command: "researchx search set <provider> [api-key]",
...
function buildWorkbenchResourceGroups(
	...
	return [
		...
		},
		{
			id: "credentials",
			title: "Credentials",
			description: "Credential surfaces ResearchX can use for model and evidence providers.",
		...
		},
		{
			...
			description: "Slash-command research workflows loaded from ResearchX's prompt templates.",
