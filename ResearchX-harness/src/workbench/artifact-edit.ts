...
import { runResearchXWorkbenchPrompt } from "./chat-runtime.js";
...
function refinementPrompt(input: {
	...
	if (input.mode === "ask") {
		return [
			"Answer a question about a selected passage in a ResearchX research artifact.",
	...
	return [
		"Suggest an edit for a selected passage in a ResearchX research artifact.",
...
export async function suggestWorkbenchArtifactRefinement(
	...
	const executor: WorkbenchPromptExecutor = options.executor ?? runResearchXWorkbenchPrompt;
	try {
