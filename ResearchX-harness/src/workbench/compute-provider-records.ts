...
function providerProbeTimestamp(workingDir: string, provider: WorkbenchComputeProvider): { iso?: string; ms?: number } {
	...
	const root = provider.id === "local-workspace" ? workingDir : resolve(workingDir, ".researchx");
...
function providerInferConfig(provider: WorkbenchComputeProvider): Record<string, string> | undefined {
	...
	return { hosted: "ESMFold", selfHosted: "AlphaFold2 NIM", tool: "researchx_model_endpoint_call" };
}
...
export function buildWorkbenchComputeProviderRecords(
	...
	return compute.map((provider) => {
		...
		return {
			...
			...(provider.id === "nvidia-bionemo" ? { inferConfig: providerInferConfig(provider), appName: "researchx-bionemo" } : {}),
			...(provider.id === "modal" ? { appName: "researchx-modal", modalEnvironment: process.env.MODAL_ENVIRONMENT || "main" } : {}),
			priorAppNames: [],
