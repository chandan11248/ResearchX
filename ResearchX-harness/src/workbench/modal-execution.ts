...
const MODAL_RESULT_MARKER = "__RESEARCHX_MODAL_RESULT__";
...
export function resolveModalCliCommand(workingDir: string): string {
	const override = process.env.RESEARCHX_MODAL_CLI?.trim();
...
function modalScriptSource(code: string, jobId: string, timeoutMs: number): string {
	...
	return [
		...
		'ARTIFACT_ROOT = "/tmp/researchx-modal-artifacts"',
		"TRACKED_ARTIFACT_ROOTS = {'outputs', 'papers', 'notes'}",
		"MAX_ARTIFACT_FILES = 16",
		"MAX_ARTIFACT_BYTES = 65536",
		"MAX_ARTIFACT_TOTAL_BYTES = 262144",
		`app = modal.App(${JSON.stringify(`researchx-notebook-${normalizeId(jobId)}`)})`,
		...
		"    os.environ['RESEARCHX_MODAL_ARTIFACT_DIR'] = ARTIFACT_ROOT",
		"    with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):",
		"        try:",
		'            exec(compile(USER_CODE, "<researchx-modal-notebook>", "exec"), namespace, namespace)',
