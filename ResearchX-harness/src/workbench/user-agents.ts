...
const DEFAULT_SYSTEM_PROMPT = "You are ResearchX, a general-purpose AI assistant built on the Pi coding agent. Never call yourself ResearchX.";
...
function readSystemPrompt(workingDir: string): { prompt: string; createdAtMs: number; updatedAtMs: number } {
	const path = resolve(workingDir, ".researchx", "SYSTEM.md");
...
export function buildWorkbenchUserAgents(workingDir: string, customSkills: WorkbenchCustomSkill[]): WorkbenchUserAgent[] {
	...
	return [{
		id: stableUuid("researchx-user-agent", "local-workbench:RESEARCHX"),
		userId: LOCAL_USER_ID,
		name: "RESEARCHX",
		displayName: "ResearchX",
		description: "Default ResearchX general-purpose agent profile",
		systemPrompt: system.prompt,
		iconKey: "lightning",
		colorKey: "researchx-green",
