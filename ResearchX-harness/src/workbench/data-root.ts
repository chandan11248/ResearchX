...
import { ensureResearchXActiveOrg, getResearchXActiveOrgDir, getResearchXHome } from "../config/paths.js";

const WORKBENCH_DATA_SCHEMA = "researchx.workbenchDataRoot.v1";
const WORKBENCH_INDEX_SCHEMA = "researchx.workbenchWorkspaceIndex.v1";
...
export function getWorkbenchDataHome(): string {
	const override = (process.env.RESEARCHX_WORKBENCH_HOME)?.trim();
	return override ? resolve(override) : resolve(getResearchXActiveOrgDir(), "workbench");
}

export function getLegacyWorkbenchDataHome(home = getResearchXHome()): string {
...
export function getLegacyWorkbenchDataRoot(workingDir: string): string {
	return resolve(workingDir, ".researchx", "workbench");
}
...
function writeWorkspaceManifest(workingDir: string, root: string): void {
	const activeOrg = ensureResearchXActiveOrg();
