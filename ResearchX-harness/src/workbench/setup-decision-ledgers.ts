...
const CONTACT_NOTICE_VERSION = "researchx-contact-email-v1";
const CONTACT_NOTICE_TEXT = "Allow ResearchX Bio Tools to send this contact email to public scientific APIs that request a contact address.";
...
export function buildWorkbenchContactEmailDecisions(): WorkbenchContactEmailDecision[] {
	...
	for (const envVar of CONTACT_EMAIL_ENV_VARS) {
		...
		rows.push({
			id: stableUuid("researchx-contact-email-decision", `${LOCAL_USER_ID}:${key}`),
...
function credentialDecisionForSecrets(provider: string, secrets: WorkbenchUserSecret[]): WorkbenchCredentialAskDecision {
	...
	return {
		id: stableUuid("researchx-credential-ask-decision", `${LOCAL_USER_ID}:${provider}`),
		userId: LOCAL_USER_ID,
		provider,
