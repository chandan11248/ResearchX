...
export type WorkbenchNotebookExecutionRecord = {
	schema: "researchx.notebookExecution.v1";
	id: string;
	sessionId: string;
	projectId: string;
	title: string;
...
const NOTEBOOK_EXECUTION_SCHEMA = "researchx.notebookExecution.v1";
...
function normalizeEnvironmentSnapshot(value: unknown): WorkbenchEnvironmentSnapshot | undefined {
	...
	if (!record || record.schema !== "researchx.workbenchEnvironmentSnapshot.v1") return undefined;
