...
const DEFAULT_AGENT_NAME = "researchx";
...
function skillRecord(workingDir: string, path: string): WorkbenchCustomSkill {
	...
	return {
		id: stableUuid("researchx-custom-skill", relPath),
...
function agentPromptRecord(workingDir: string, path: string): WorkbenchCustomAgentPrompt {
	...
	const timestamps = fileTimestamps(path);
	return {
		id: stableUuid("researchx-custom-agent-prompt", relPath),
		userId: LOCAL_USER_ID,
		agentName,
		promptText: content,
		path: relPath,
...
function skillAssignment(skill: WorkbenchCustomSkill): WorkbenchAgentSkillAssignment {
	return {
		id: stableUuid("researchx-agent-skill-assignment", `${skill.id}:${DEFAULT_AGENT_NAME}`),
		skillId: skill.id,
...
export function buildWorkbenchSkillLedgers(workingDir: string): {
	...
	const customAgentPrompts = directMarkdownFiles(resolve(workingDir, ".researchx", "agents"))
