...
const NOTICE_VERSION = "researchx-owned-skill-pack-v1";
const NOTICE_TEXT = "ResearchX-owned local project skill loaded from this workspace under the package license.";
...
function marketplaceSource(workingDir: string, customSkills: WorkbenchCustomSkill[]): WorkbenchMarketplaceSource[] {
	...
	const slug = "researchx-science-skill-pack";
	...
	return [{
		id: stableUuid("researchx-marketplace-source", `${LOCAL_USER_ID}:${slug}`),
		userId: LOCAL_USER_ID,
		slug,
		kind: "local",
		marketplaceName: "ResearchX Science Skill Pack",
...
function skillLicenseAssent(skill: WorkbenchCustomSkill, license: string): WorkbenchSkillLicenseAssent {
	...
	return {
		id: stableUuid("researchx-skill-license-assent", `${LOCAL_USER_ID}:${resourceKey}`),
		userId: LOCAL_USER_ID,
		orgId: LOCAL_ORG_ID,
		resourceKey,
