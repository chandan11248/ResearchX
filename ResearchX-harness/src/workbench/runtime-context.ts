...
export type WorkbenchEnvironmentSnapshot = {
	schema: "researchx.workbenchEnvironmentSnapshot.v1";
	capturedAt: string;
	cwd: string;
	platform: string;
	arch: string;
...
const SNAPSHOT_SCHEMA = "researchx.workbenchEnvironmentSnapshot.v1" as const;
const ENVIRONMENT_FILE_CANDIDATES: Array<{ path: string; kind: string }> = [
	...
	{ path: ".researchx/npm/package.json", kind: "pi runtime manifest" },
	{ path: ".researchx/npm/package-lock.json", kind: "pi runtime lockfile" },
];
...
export function readWorkbenchRuntimeResources(workingDir: string): WorkbenchRuntimeResources {
	...
	return {
		...
		modelEndpoints: [{
			...
			tool: "researchx_model_endpoint_call",
			description: nvidiaEndpointEnabled
				...
				: "Disabled in ResearchX compute provider settings for this workspace.",
		}],
...
export function formatWorkbenchRuntimeContextForPrompt(
	...
	if (config) {
		...
		if (config.specialist !== "None") {
			lines.push(`- Pass specialist "${config.specialist}" to researchx_connector_tools and researchx_connector_call so connector assignment rules are enforced for this chat.`);
		}
	}
