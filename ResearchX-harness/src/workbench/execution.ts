	...
	for (const session of sessions) {
		...
		for (const rawMessage of messages) {
			...
			records.push({
				...
				title: role === "assistant" ? "ResearchX response" : role === "user" ? "User prompt" : `${role} message`,
				kind,
				status,
				origin: "chat",
				createdAt: timestamp.iso,
...
function buildNotebookExecutionRecords(
	...
	return readNotebookExecutionRecords(workingDir).map((record) => {
		...
		return {
			...
			environment: `ResearchX notebook ${record.executionMode === "session" ? "session kernel" : record.executionMode === "modal" ? "Modal cloud" : "isolated process"} / ${record.cwd || "local workspace"}`,
			sourceId: record.id,
			code: record.code,
