...
function ensureRuntimeReady(request: WorkbenchPromptRequest) {
	if (!request.appRoot || !request.sessionDir || !request.researchxAgentDir) {
		throw new Error("Workbench chat is not connected to the ResearchX runtime. Start it with `researchx serve`.");
	}

	ensureSupportedNodeVersion();
	patchPiRuntimeNodeModules(request.appRoot, request.researchxAgentDir);
	...
	if (!existsSync(paths.piMainPath) || (!useBuiltWrapper && !useDevWrapper) || (!useBuiltPolyfill && !useDevPolyfill)) {
		throw new Error("ResearchX Pi runtime files are missing. Run `npm run build` and try again.");
	}
...
async function spawnWorkbenchPi(request: WorkbenchPromptRequest, mode: ChildMode): Promise<WorkbenchPiChild> {
	...
	const runtimeOptions: PiRuntimeOptions = {
		...
		researchxAgentDir: request.researchxAgentDir!,
		researchxVersion: request.researchxVersion,
	...
	ensureResearchXCommandShim(request.appRoot!, request.researchxAgentDir!);
...
function rpcTimeoutMs(): number {
	return Number(process.env.RESEARCHX_WORKBENCH_TIMEOUT_MS ?? "480000");
}

function rpcCommandTimeoutMs(): number {
	return Number(process.env.RESEARCHX_WORKBENCH_RPC_COMMAND_TIMEOUT_MS ?? "30000");
}
...
class WorkbenchPiRpcClient {
	...
	async steer(request: WorkbenchPromptRequest): Promise<void> {
		if (!this.activeRun) {
			throw new Error("No active ResearchX Pi turn is running.");
		}
		...
		if (response.success === false) {
			throw new Error(response.error || "ResearchX Pi RPC steering message was rejected.");
		}
	}
	...
	async abortActiveRun(): Promise<void> {
		...
		if (response.success === false) {
			throw new Error(response.error || "ResearchX Pi RPC abort was rejected.");
		}
	}
	...
	async listCommands(request: WorkbenchPromptRequest): Promise<WorkbenchPiCommand[]> {
		...
		if (response.success === false) {
			throw new Error(response.error || "ResearchX Pi RPC command discovery failed.");
		}
	...
	private async ensureStarted(request: WorkbenchPromptRequest): Promise<void> {
		...
		if (!child.stdin || !child.stdout || !child.stderr) {
			throw new Error("ResearchX Pi RPC session did not expose stdio pipes.");
		}
		...
		child.once("exit", (code, signal) => {
			...
			const error = new Error(`ResearchX Pi RPC session exited (code=${code} signal=${signal}).${this.stderr ? ` ${stripAnsi(this.stderr).trim()}` : ""}`);
		...
		child.once("error", (error) => {
			if (this.child !== child) return;
			const wrapped = new Error(`ResearchX Pi RPC session failed: ${error.message}.${this.stderr ? ` ${stripAnsi(this.stderr).trim()}` : ""}`);
		...
		if (child.exitCode !== null) {
			throw this.exitError ?? new Error(`ResearchX Pi RPC session exited before it was ready. ${stripAnsi(this.stderr).trim()}`);
		}
	}
	...
	private async runPrompt(
		...
		const done = new Promise<void>((resolve, reject) => {
			...
			activeRun.timeout = setTimeout(() => {
				reject(new Error("ResearchX chat turn timed out."));
			}, rpcTimeoutMs());
		});
		...
		try {
			...
			if (response.success === false) {
				throw new Error(response.error || "ResearchX Pi RPC prompt was rejected.");
			}
			await done;
		...
		if (!activeRun.toolEvents.size) {
			...
			activeRun.toolEvents.set(id, {
				id,
				label: "ResearchX Pi RPC session",
				status: "complete",
			});
		}
		return {
			content: activeRun.content.trim() || "ResearchX finished without text output.",
	...
	private async send(command: Record<string, unknown>): Promise<RpcResponse> {
		...
		if (!child || !stdin) {
			throw new Error("ResearchX Pi RPC session is not running.");
		}
		if (this.exitError) throw this.exitError;
		if (child.exitCode !== null) {
			throw new Error(`ResearchX Pi RPC session already exited. ${stripAnsi(this.stderr).trim()}`);
		}
...
function getActiveWorkbenchPiRpcClient(request: WorkbenchPromptRequest): WorkbenchPiRpcClient {
	...
	if (!client?.isUsable() || !client.hasActiveRun()) {
		throw new Error("No active ResearchX Pi turn is running for this workbench session.");
	}
	return client;
}
...
export async function runResearchXWorkbenchPrompt(request: WorkbenchPromptRequest): Promise<WorkbenchPromptResult> {
	return runResearchXWorkbenchPromptStream(request, () => undefined);
}

export async function listResearchXWorkbenchCommands(request: WorkbenchPromptRequest): Promise<WorkbenchPiCommand[]> {
...
export async function runResearchXWorkbenchPromptStream(
	request: WorkbenchPromptRequest,
	onUpdate: (update: WorkbenchPromptStreamUpdate) => void | Promise<void>,
): Promise<WorkbenchPromptResult> {
...
export async function steerResearchXWorkbenchPrompt(request: WorkbenchPromptRequest): Promise<void> {
	await getActiveWorkbenchPiRpcClient(request).steer(request);
}

export async function abortResearchXWorkbenchPrompt(request: WorkbenchPromptRequest): Promise<void> {
