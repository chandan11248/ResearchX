...
export function resolvePythonRuntimeCommand(workingDir?: string): WorkbenchRuntimeCommand {
	...
	if (managed) return { command: managed, detail: "ResearchX app data managed Python venv", source: "managed" };
	return { command: "python3", detail: "python3 on PATH", source: "path" };
}

export function resolveRRuntimeCommand(): WorkbenchRuntimeCommand {
	const r = configured(process.env.RESEARCHX_R) ?? configured(process.env.R);
	if (r) return { command: r, detail: process.env.RESEARCHX_R?.trim() ? "RESEARCHX_R" : "R", source: "configured" };
	return { command: "R", detail: "R on PATH", source: "path" };
}

export function resolveRscriptRuntimeCommand(): WorkbenchRuntimeCommand {
	const configuredRscript = configured(process.env.RESEARCHX_RSCRIPT) ?? configured(process.env.RSCRIPT) ?? siblingRscript(configured(process.env.RESEARCHX_R) ?? configured(process.env.R));
	if (configuredRscript) {
		const source = process.env.RESEARCHX_RSCRIPT?.trim() ? "RESEARCHX_RSCRIPT" : process.env.RSCRIPT?.trim() ? "RSCRIPT" : "R sibling";
