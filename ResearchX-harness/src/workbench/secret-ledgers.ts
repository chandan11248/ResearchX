...
function encryptedRef(source: SecretSource, key: string): string {
	return `researchx-${source}-ref:${key}`;
}
...
function secretRow(input: {
	...
	return {
		id: stableUuid("researchx-user-secret", `${LOCAL_USER_ID}:${input.source}:${input.key}`),
...
function anthropicKeyRows(userSecrets: WorkbenchUserSecret[]): WorkbenchAnthropicApiKey[] {
	return userSecrets
		...
		.map((secret) => ({
			id: stableUuid("researchx-anthropic-api-key", secret.id),
			userId: LOCAL_USER_ID,
			encryptedApiKey: secret.encryptedValue.replace(/^researchx-/, "researchx-anthropic-"),
